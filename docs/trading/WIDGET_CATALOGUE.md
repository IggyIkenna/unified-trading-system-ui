...
| `book-order-form`         | Book Order Entry     | form         | —         | `book/book-order-form-widget.tsx`         | Core order form: mode toggle, category, venue, instrument, side, qty, price |
| `book-algo-config`        | Algo Configuration   | form         | —         | `book/book-algo-config-widget.tsx`        | Algorithm selector and conditional params (TWAP/VWAP, iceberg, benchmark)   |
| `book-record-details`     | Record Details       | form         | —         | `book/book-record-details-widget.tsx`     | Counterparty, source reference, fee fields for record-only mode             |
| `book-preview-compliance` | Preview & Compliance | detail-panel | —         | `book/book-preview-compliance-widget.tsx` | Order preview grid and pre-trade compliance checks with pass/fail badges    |
...
| **2**    | `FormWidget`                                       | form           | 16            | order-entry, book-order-form, book-algo-config, book-record-details, defi-lending, defi-swap, defi-liquidity, defi-staking, defi-flash-loans, defi-transfer, defi-strategy-config, defi-staking-rewards, accounts-transfer, options-trade-panel, futures-trade-panel, pred-trade-panel, cefi-strategy-config                                                                                                                                  | Most are submit-with-validation forms. Base handles submit/loading/error state, validation, submit button chrome.         |
...
| **4**    | `DetailPanelWidget`                                | detail-panel   | 10            | book-preview-compliance, instr-detail-panel, sports-fixture-detail, pred-market-detail, options-scenario, bundle-steps, defi-health-factor, defi-reward-pnl, sports-clv, sports-ml-status, commodity-regime                                                                                                                                                                                                                                   | Master-detail with header + body + optional actions. Base handles selection state, empty state, back navigation.          |
