...
- **Shared primitives in active use:** `ServiceTabs` + tab constants (`components/shell/service-tabs.tsx`), `TradingVerticalNav` (trading layout only), `FilterBar` + `FilterBarWidget`, `KpiStrip` (`components/widgets/shared/kpi-strip.tsx`) consumed by 13 domain KPI widgets, `MetricCard` (`components/shared/metric-card.tsx`), `DataTable` / `DataTableWidget`, `StatusBadge`, `Skeleton`, Sonner/toast patterns, `Breadcrumbs` in shell.
- **Barrel:** `components/widgets/shared/index.ts` exports `KpiStrip`, `FilterBarWidget`, `DataTableWidget`, `CollapsibleSection` — KPI strip centralization is largely achieved for widgets that follow the registry pattern.
...
| ------------------------------------ | ----------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------ |
| Page header (title + desc + actions) | _None_                                    | 0                                                                            | **~89** `<h1>` occurrences across `app/` (138 `page.tsx` files); only niche `strategies-page-header` | 🟡 High   | Add `components/platform/page-header.tsx` (title, optional description, actions slot); migrate top templates first |
...
| KPI strip (row)                      | `components/widgets/shared/kpi-strip.tsx` | **13** widgets import `KpiStrip` via `../shared`                             | **0** duplicate strip primitives found in audited `*-kpi*` widgets                                   | ✅        | Keep; document as mandatory for new KPI widgets                                                                    |
| Status badge                         | `components/trading/status-badge.tsx`     | **26** files reference `StatusBadge`                                         | **~23** files with ad-hoc badge/pill styling (heuristic: `Badge` + status-ish layout)                | 🟢 Medium | Route new status UI through `StatusBadge`                                                                          |
| Empty state                          | `components/ui/empty-state.tsx`           | **4** pages (strategy backtests/results/compare, ML registry)                | **10** files with “No data” / “No results” strings (not all pages)                                   | 🟡 High   | Replace inline copy with `EmptyState` + consistent CTA                                                             |
...
| Data table                           | `components/ui/data-table.tsx`            | **9** imports                                                                | **43** files with `<table`                                                                           | 🟡 High   | Prefer `DataTable` or `DataTableWidget` when tabular data is standard                                              |
| Filter bar                           | `components/platform/filter-bar.tsx`      | **11** imports; **10** files name `FilterBar`                                | Per-page one-off filters outside this set                                                            | 🟢 Medium | Extend `FilterBar` / `FilterBarWidget` before new filter UIs                                                       |
...
| Error boundary                       | `components/ui/error-boundary.tsx`        | **8** service layouts + `research-family-shell.tsx`                          | No `app/**/error.tsx` route handlers found                                                           | 🟢 Medium | Consider Next `error.tsx` per segment for consistent error UI                                                      |
...
| --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| Duplicate **names** for “filter bar”                                                    | `components/platform/filter-bar.tsx` exports `FilterBar`; `components/widgets/shared/filter-bar-widget.tsx` wraps it (compositional, not a duplicate implementation)                                            | ✅ OK                                                    |
| Duplicate **EmptyState** export                                                         | `components/trading/sports/shared.tsx` exports `EmptyState` in addition to `components/ui/empty-state.tsx`                                                                                                      | 🟡 High — consolidate on `components/ui/empty-state.tsx` |
| **`components/ui/*` never imported** (heuristic: zero `@/components/ui/<file>` imports) | **~28** files flagged (e.g. `carousel`, `drawer`, `chart`, `pagination`, `sidebar`, `empty.tsx` vs `empty-state.tsx`, `breadcrumb.tsx` vs shell)                                                                | 🟡 High — prune or wire; risk of drift and bundle noise  |
...
| ---------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| Metric strip           | **14** widget files tied to KPI/metric naming; **13** use `KpiStrip` from `widgets/shared` | ✅ `kpi-strip.tsx` (done)                                                            |
...
| Data table shell       | **6** `*-table-widget.tsx` files + `shared/data-table-widget.tsx`                          | Optional `components/ui/data-table-shell.tsx` if wrappers diverge                    |
...
4. **Duplicate `EmptyState`** — `components/trading/sports/shared.tsx` vs `components/ui/empty-state.tsx`.
...
3. **Migrate** “No data / No results” surfaces to `components/ui/empty-state.tsx`; **delete** sports `EmptyState` after re-export shim removal (no long-term dual exports per project rules).
