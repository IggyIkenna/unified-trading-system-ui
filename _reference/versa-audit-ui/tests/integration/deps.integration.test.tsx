/**
 * Functional integration tests — verifies batch-audit-ui works correctly
 ...
 * contract) directly, and verify RequireAuth's export exists. The actual
 * RequireAuth rendering is covered by App.test.tsx (which mocks the dep).
 *
...
const skipAuthConfig: AuthProviderConfig = {
  ...
  skipAuth: true,
  serviceName: "batch-audit-ui-test",
};
...
describe("@unified-trading/ui-auth integration", () => {
  ...
  it("type AuthProviderConfig includes all required fields", () => {
    // Compile-time type check: these fields must exist on the config
  ...
  it("Card composition renders header and content", () => {
    render(
      <Card>
        ...
        <CardContent>5 total jobs</CardContent>
      </Card>,
    );
...
describe("auth + ui-kit composition (full stack)", () => {
  it("AuthProvider > ErrorBoundary > Card tree renders end-to-end", () => {
    render(
      <AuthProvider config={skipAuthConfig}>
        <ErrorBoundary>
          <Card>
