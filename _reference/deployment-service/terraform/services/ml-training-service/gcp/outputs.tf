# Outputs for ml-training-service Terraform configuration
...
output "backfill_workflow_id" {
  description = "The ID of the full training Cloud Workflow"
  value       = module.backfill_workflow.workflow_id
}

output "backfill_workflow_name" {
  description = "The name of the full training Cloud Workflow"
  value       = module.backfill_workflow.workflow_name
}

output "backfill_execution_url" {
  description = "URL to execute the full training workflow manually"
  value       = module.backfill_workflow.execution_url
}

output "backfill_example_command" {
  description = "Example command to run full training workflow"
  value       = "gcloud workflows run ${var.backfill_workflow_name} --location=${var.region} --data='{\"start_date\":\"2020-01-01\",\"end_date\":\"2025-12-31\",\"instruments\":[\"BTC\",\"ETH\",\"SOL\",\"SPY\"],\"timeframes\":[\"1h\",\"4h\"],\"target_types\":[\"swing_high\",\"swing_low\"]}'"
}
