# Outputs for ml-inference-service Terraform configuration
...
output "backfill_workflow_id" {
  description = "The ID of the batch inference Cloud Workflow"
  value       = module.backfill_workflow.workflow_id
}

output "backfill_workflow_name" {
  description = "The name of the batch inference Cloud Workflow"
  value       = module.backfill_workflow.workflow_name
}

output "backfill_execution_url" {
  description = "URL to execute the batch inference workflow manually"
  value       = module.backfill_workflow.execution_url
}

output "backfill_example_command" {
  description = "Example command to run batch inference workflow"
  value       = "gcloud workflows run ${var.backfill_workflow_name} --location=${var.region} --data='{\"start_date\":\"2020-01-01\",\"end_date\":\"2026-01-25\",\"instruments\":[\"BTC-USDT-PERPETUAL\",\"ETH-USDT-PERPETUAL\"],\"timeframes\":[\"1h\",\"4h\"]}'"
}
