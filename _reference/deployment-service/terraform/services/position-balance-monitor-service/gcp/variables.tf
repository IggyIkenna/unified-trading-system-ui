# Variables for position-balance-monitor-service Terraform configuration
...
variable "job_name" {
  ...
  type        = string
  default     = "position-balance-monitor-service-job"
}
...
variable "live_workflow_name" {
  description = "Name of the live monitoring Cloud Workflow"
  type        = string
  default     = "position-balance-monitor-service-live"
}

variable "live_schedule" {
  description = "Cron schedule for live monitoring (every 5 minutes)"
  type        = string
  default     = "*/5 * * * *"
}
