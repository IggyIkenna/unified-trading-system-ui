# Variables for features-volatility-service Terraform configuration
...
variable "job_name" {
  ...
  type        = string
  default     = "features-volatility-service-job"
}
...
variable "backfill_workflow_name" {
  ...
  type        = string
  default     = "features-volatility-service-backfill"
}

variable "schedule" {
  description = "Cron schedule (default: 11:00 AM UTC - parallel with delta-one)"
  type        = string
  default     = "0 11 * * *"
}
