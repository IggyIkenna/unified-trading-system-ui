...
| `bloxroute-auth-header`                                         | `BLOXROUTE_AUTH_HEADER` | bloXroute mempool streaming                                                                                                                                                   | ❌ No                       | `KEY_NOT_IN_SM`                                           |
...
| `arkham-api-key`       | `ARKHAM_API_KEY`       | Arkham intelligence on-chain analytics       | ❌ No      | `KEY_NOT_IN_SM`                                                 |
...
> **⚠️ endpoint_registry.py correction needed:** `ibkr` and `bloxroute` are marked `PENDING_CASSETTE_AWAITING_AUTH` with
> notes claiming credentials confirmed — neither key exists in SM.
