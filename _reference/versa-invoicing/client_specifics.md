...
This document is the **authoritative narrative** of what the numbers _should_ be, based strictly on what we agreed in this chat.
...
_(Authoritative reference – overrides spreadsheet confusion)_
...
1. **Two separate High-Water Marks (HWMs) exist**
   - **Odum HWM** → derived from _client invoices_ (or deposits if never invoiced)
   - **Trader HWM** → derived from _trader invoices paid_ (reverse-engineered)
