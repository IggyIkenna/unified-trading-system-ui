...
async function handleRoute(url: string, init?: RequestInit): Promise<Response> {
  ...
  // Deployment action endpoints (POST) — must come before the generic /{id} GET handler
  if (path.match(/^\/api\/deployments\/(.+)\/cancel$/) && method === "POST") {
    const id = path.split("/")[3];
    return json({ success: true, deployment_id: id, status: "cancelled", message: "Deployment cancelled successfully" });
  }
  if (path.match(/^\/api\/deployments\/(.+)\/resume$/) && method === "POST") {
    const id = path.split("/")[3];
    return json({ success: true, deployment_id: id, status: "running", message: "Deployment resumed" });
  }
  if (path.match(/^\/api\/deployments\/(.+)\/verify-completion$/) && method === "POST") {
    const id = path.split("/")[3];
    return json({ success: true, deployment_id: id, completed: true, verified_shards: 24, total_shards: 24, message: "All shards verified complete" });
  }
  if (path.match(/^\/api\/deployments\/(.+)\/retry-failed$/) && method === "POST") {
    const id = path.split("/")[3];
    return json({ success: true, deployment_id: id, shards_to_retry: 3, shards_retried: 3, status: "running" });
  }
  if (path.match(/^\/api\/deployments\/(.+)\/shards\/(.+)\/cancel$/) && method === "POST") {
    const parts = path.split("/");
    const shardId = parts[parts.length - 2];
    return json({ success: true, shard_id: shardId, status: "cancelled" });
  }
  if (path === "/api/deployments/bulk-delete" && method === "POST") {
    return json({ success: true, deleted: 2, message: "Deployments deleted" });
  }
  if (path.match(/^\/api\/deployments\/(.+)\/rerun-commands$/) && method === "GET") {
    return json({ commands: ["bash rerun.sh --shard 1 --region us-central1", "bash rerun.sh --shard 2 --region us-central1"], shard_count: 2 });
  }
  if (path.match(/^\/api\/deployments\/(.+)\/rollback$/) && method === "POST") {
    const id = path.split("/")[3];
    return json({ success: true, deployment_id: id, rollback_version: "v1.2.3", status: "rolling_back", message: "Rollback initiated" });
  }
  ...
  if (path.match(/^\/api\/cloud-builds\/(.+)\/trigger$/) && method === "POST") {
    const triggerId = path.split("/")[3];
    return json(
      {
        success: true,
        build_id: `build-mock-${Date.now()}`,
        trigger_id: triggerId,
        status: "QUEUED",
        message: "Build triggered successfully",
        log_url: "https://console.cloud.google.com/cloud-build/builds/mock-build",
      },
      201,
    );
  }

  // Execution service data status endpoints
  if (path === "/api/service-status/execution-services/data-status" && method === "GET") {
    return json({
      config_path: "configs/v2",
      version: "V2",
      total_configs: 48,
      configs_with_results: 32,
      missing_count: 16,
      completion_pct: 66.7,
      strategy_count: 4,
      strategies: [
        { strategy: "StatArb", total: 12, with_results: 10, completion_pct: 83.3 },
        { strategy: "MeanReversion", total: 12, with_results: 8, completion_pct: 66.7 },
        { strategy: "TrendFollowing", total: 12, with_results: 8, completion_pct: 66.7 },
        { strategy: "MarketMaking", total: 12, with_results: 6, completion_pct: 50.0 },
      ],
    });
  }
  if (path === "/api/service-status/execution-services/missing-shards" && method === "POST") {
    return json({
      missing_shards: [
        { config_gcs: "configs/v2/statarb_day.json", date: "2024-01-15", strategy: "StatArb", mode: "day", timeframe: "1h", algo: "TWAP" },
        { config_gcs: "configs/v2/mr_day.json", date: "2024-01-16", strategy: "MeanReversion", mode: "day", timeframe: "4h", algo: "VWAP" },
      ],
      total_missing: 16,
      total_configs: 48,
      total_dates: 30,
      breakdown: {
        by_strategy: { StatArb: 3, MeanReversion: 4, TrendFollowing: 5, MarketMaking: 4 },
        by_mode: { day: 8, night: 8 },
        by_timeframe: { "1h": 4, "4h": 6, "8h": 6 },
        by_algo: { TWAP: 6, VWAP: 5, ADAPTIVE_TWAP: 5 },
      },
    });
  }
