...
describe("ScopeSummary", () => {
  ...
  it("shows organization name when single org is selected", () => {
    render(<ScopeSummary {...defaultProps} organizations={[{ id: "odum", name: "Odum Capital" }]} />);
  ...
  it("shows client name when single client selected", () => {
    render(<ScopeSummary {...defaultProps} clients={[{ id: "delta-one", name: "Delta One Desk" }]} />);
  ...
  it("shows Batch indicator when mode is batch", () => {
    render(<ScopeSummary {...defaultProps} mode="batch" asOfDatetime="2026-03-17T10:00:00Z" />);
