...
test.describe("Tier 1: Reports Navigation", () => {
  ...
  test("executive report page loads", async ({ page }) => {
    ...
    expect(
      response?.status() === 200 || response?.status() === 304 || response?.status() === 307,
  ...
  test("reports tab bar is visible", async ({ page }) => {
    ...
    const tabs = page.locator(
      'nav a[href*="/services/reports"], ' + '[role="tablist"] [role="tab"], ' + 'a[href*="/reports/"]',
    );
    await expect(async () => {
...
test.describe("Tier 2: Reports Data", () => {
  ...
  test("settlement page shows settlements data", async ({ page }) => {
    ...
    const rows = page.locator('table tbody tr, [role="row"], [data-testid*="settlement"]');
    await expect(async () => {
      const bodyText = await page.textContent("body");
      expect((await rows.count()) > 0 || (bodyText?.length ?? 0) > 200).toBeTruthy();
    }).toPass({ timeout: 10000 });
  });
  ...
  test("reconciliation page shows drift analysis", async ({ page }) => {
    ...
    const content = page.locator(
      'table, [role="grid"], canvas, svg, ' + '[data-testid*="reconciliation"], [data-testid*="drift"]',
    );
    await expect(async () => {
      const bodyText = await page.textContent("body");
      expect((await content.count()) > 0 || (bodyText?.length ?? 0) > 200).toBeTruthy();
...
test.describe("Tier 3: Reports Journey", () => {
  test("navigate P&L → settlement → reconciliation via tabs", async ({ page }) => {
    await page.goto("/services/reports/overview");
    await page.waitForLoadState("networkidle");

    // Click Settlement tab
    const settlementTab = page.locator('a:has-text("Settlement"), [role="tab"]:has-text("Settlement")');
    ...
    const reconTab = page.locator('a:has-text("Reconciliation"), [role="tab"]:has-text("Reconciliation")');
  ...
  test("navigate to regulatory tab", async ({ page }) => {
    ...
    const regulatoryTab = page.locator('a:has-text("Regulatory"), [role="tab"]:has-text("Regulatory")');
