...
test.describe("Strategy Scale — 30+ Strategies", () => {
  test("dashboard Strategy Performance table has data rows", async ({ page }) => {
    ...
    const strategyRows = page.locator('table tbody tr, [data-testid*="strategy"]');
    await expect(async () => {
  ...
  test("strategies page lists 30+ strategies", async ({ page }) => {
    ...
    const strategyItems = page.locator('table tbody tr, [role="row"], [data-testid*="strategy"], .strategy-card');
    await expect(async () => {
  ...
  test("research strategy overview renders strategy list", async ({ page }) => {
    ...
    const strategyContent = page.locator('table, [data-testid*="strategy"], text=Strategy');
    await expect(async () => {
