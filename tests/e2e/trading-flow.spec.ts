...
test.describe("Tier 1: Trading Navigation", () => {
  ...
  test("trading tab bar is visible on terminal page", async ({ page }) => {
    ...
    const tabs = page.locator(
      'nav a[href*="/services/trading"], ' + '[role="tablist"] [role="tab"], ' + 'a[href*="/trading/"]',
    );
    await expect(async () => {
...
test.describe("Tier 2: Trading Data", () => {
  test("positions table renders with data rows", async ({ page }) => {
    ...
    const tableRows = page.locator('table tbody tr, [role="row"], [data-testid*="position"]');
    await expect(async () => {
  ...
  test("orders table renders with data rows", async ({ page }) => {
    ...
    const tableRows = page.locator('table tbody tr, [role="row"], [data-testid*="order"]');
    await expect(async () => {
      expect(await tableRows.count()).toBeGreaterThan(0);
    }).toPass({ timeout: 10000 });
  });
  ...
  test("terminal page renders chart or data content", async ({ page }) => {
    ...
    const dataContent = page.locator('table, [role="grid"], [data-testid*="terminal"]');
  ...
  test("strategies page shows strategy list", async ({ page }) => {
    ...
    const content = page.locator('table tbody tr, [role="row"], [data-testid*="strategy"], .strategy-card');
    await expect(async () => {
...
test.describe("Tier 3: Trading Journey", () => {
  test("navigate terminal → positions → orders via tabs", async ({ page }) => {
    ...
    // Click Positions tab
    const positionsTab = page.locator('a:has-text("Positions"), [role="tab"]:has-text("Positions")');
    ...
    // Click Orders tab
    const ordersTab = page.locator('a:has-text("Orders"), [role="tab"]:has-text("Orders")');
  ...
  test("batch/live toggle changes displayed data", async ({ page }) => {
    ...
    if ((await batchToggle.count()) > 0) {
      ...
      const batchIndicator = page.locator(':text("Batch"), :text("batch"), :text("Viewing Batch")');
      await expect(async () => {
        const bodyAfter = await page.textContent("body");
        // Something should change or batch indicator appears
        expect((await batchIndicator.count()) > 0 || bodyAfter !== initialBody).toBeTruthy();
      }).toPass({ timeout: 5000 });
    }
  });
  ...
  test("manual trade button opens trade form", async ({ page }) => {
    ...
    if ((await tradeBtn.count()) > 0) {
      ...
      const tradeForm = page.locator(
        '[role="dialog"], ' + '[data-testid="trade-form"], ' + '[data-testid="order-entry"], ' + "form",
      );
  ...
  test("navigate to dashboard from trading works", async ({ page }) => {
    ...
    if ((await dashLink.count()) > 0) {
      ...
      await page.waitForLoadState("networkidle");
      expect(page.url().includes("/dashboard") || page.url().endsWith("/")).toBeTruthy();
    }
  });
});
