...
test.describe("API Reset Flow", () => {
  ...
  test("create order → reset → order gone, seed preserved", async ({ request }) => {
    ...
    const beforeReset = await request.get(`${API}/execution/orders?venue=e2e-reset-test`);
    ...
    const afterReset = await request.get(`${API}/execution/orders?venue=e2e-reset-test`);
...
test.describe("UI Reset Flow", () => {
  ...
  test("reset via debug footer clears mutations", async ({ page, request }) => {
    ...
    const resetBtn = page.locator(
      'button:has-text("Reset Demo"), ' + 'button:has-text("Reset"), ' + '[data-testid="reset-demo"]',
    );
    ...
    if ((await resetBtn.count()) > 0) {
      ...
      const afterReset = await request.get(`${API}/execution/orders?venue=e2e-ui-reset`);
      const afterBody = await afterReset.json();
      expect(afterBody.data.length).toBe(0);
    } else {
...
test.describe("Full Reset Lifecycle", () => {
  test("place trade → verify in UI → reset → verify gone", async ({ page, request }) => {
    // Step 1: Clean state
    await request.post(`${API}/admin/reset`);
    ...
    const afterReset = await request.get(`${API}/execution/orders?venue=binance&instrument=LIFECYCLE-RESET`);
    const afterBody = await afterReset.json();
    // The custom LIFECYCLE-RESET order should be gone
    const customOrders = afterBody.data.filter((o: Record<string, unknown>) => o.instrument === "LIFECYCLE-RESET");
