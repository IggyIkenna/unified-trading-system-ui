...
test.describe("Tier 0 behavior audit", () => {
  ...
  test("admin approve → deny round-trip preserves state", async ({ page }) => {
    ...
    await expect(deniedBadge, "TIER0_AUDIT: after Deny, request should show denied status").toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("Reset Demo button is visible in mock footer", async ({ page }) => {
    ...
    await expect(resetBtn, "TIER0_AUDIT: Reset Demo should be visible in mock mode footer").toBeVisible({
      timeout: 5000,
    });
  });
});
