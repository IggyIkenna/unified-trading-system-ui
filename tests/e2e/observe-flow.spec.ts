...
test.describe("Tier 2: Observe Data", () => {
  ...
  test("alerts page shows alerts list", async ({ page }) => {
    ...
    const alertRows = page.locator('table tbody tr, [role="row"], [data-testid*="alert"], .alert-item');
    await expect(async () => {
      const bodyText = await page.textContent("body");
      expect((await alertRows.count()) > 0 || (bodyText?.length ?? 0) > 200).toBeTruthy();
    }).toPass({ timeout: 10000 });
  });
  ...
  test("system health page shows service health grid", async ({ page }) => {
    ...
    const healthContent = page.locator('table, [role="grid"], [data-testid*="health"], .health-card, .service-card');
    await expect(async () => {
      const bodyText = await page.textContent("body");
      expect((await healthContent.count()) > 0 || (bodyText?.length ?? 0) > 200).toBeTruthy();
    }).toPass({ timeout: 10000 });
  });
});
...
test.describe("Tier 3: Observe Journey", () => {
  test("navigate risk → alerts → system health", async ({ page }) => {
    ...
    // Click Alerts tab
    const alertsTab = page.locator('a:has-text("Alerts"), [role="tab"]:has-text("Alerts")');
    ...
    const healthTab = page.locator('a:has-text("Health"), a:has-text("System"), [role="tab"]:has-text("Health")');
  ...
  test("risk dashboard has interactive controls", async ({ page }) => {
    ...
    const controls = page.locator('button:visible, select:visible, [role="combobox"], input[type="range"]');
    await expect(async () => {
