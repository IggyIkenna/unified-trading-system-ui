...
test.describe("Tier 1: Data Navigation", () => {
  ...
  test("data tab bar is visible", async ({ page }) => {
    ...
    const tabs = page.locator(
      'nav a[href*="/services/data"], ' + '[role="tablist"] [role="tab"], ' + 'a[href*="/data/"]',
    );
    await expect(async () => {
...
test.describe("Tier 3: Data Journey", () => {
  test("navigate pipeline status → coverage → markets via tabs", async ({ page }) => {
    await page.goto("/services/data/overview");
    await page.waitForLoadState("networkidle");

    // Click Coverage Matrix tab
    const coverageTab = page.locator('a:has-text("Coverage"), [role="tab"]:has-text("Coverage")');
    ...
    // Click Markets tab
    const marketsTab = page.locator('a:has-text("Markets"), [role="tab"]:has-text("Markets")');
  ...
  test("navigate to venue health and back", async ({ page }) => {
    ...
    const venueTab = page.locator('a:has-text("Venue"), [role="tab"]:has-text("Venue")');
