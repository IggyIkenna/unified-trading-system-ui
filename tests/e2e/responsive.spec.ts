...
test.describe("Tablet viewport (768x1024)", () => {
  ...
  test("trading terminal renders at tablet width", async ({ page }) => {
    ...
    const chartContainer = page.locator('canvas, svg, [data-testid*="chart"], .tv-lightweight-charts');
    // Charts may or may not render at tablet — just verify page loads
    expect(body).not.toContain("404");
  });

  test("data tables have horizontal scroll at narrow width", async ({ page }) => {
