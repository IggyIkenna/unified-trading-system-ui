...
const DOCS_ROOT = path.join(process.cwd(), ".local-dev-cache", "onboarding-docs")
...
export async function POST(request: NextRequest) {
  ...
  if (!file || !orgId || !applicationId || !docType) {
    return NextResponse.json({ error: "Missing file, org_id, application_id, or doc_type" }, { status: 400 })
  }
