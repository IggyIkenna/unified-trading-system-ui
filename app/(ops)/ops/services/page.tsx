...
const serviceShards: Record<string, ShardConfig> = {
  "features-delta-one-service": {
    ...
    totalShards: 42,
    shards: [
      { key: "cefi/binance/funding_rates", status: "running", lastUpdate: "3m ago", latency: "12ms" },
      { key: "cefi/okx/order_book", status: "running", lastUpdate: "1m ago", latency: "8ms" },
      { key: "defi/aave_v3/lending_rates", status: "running", lastUpdate: "5m ago", latency: "45ms" },
      { key: "tradfi/databento/equity_bars", status: "stale", lastUpdate: "47m ago" },
      { key: "sports/odds_api/match_odds", status: "failed", lastUpdate: "2h ago", error: "timeout" },
      { key: "defi/uniswap_v3/pool_state", status: "running", lastUpdate: "2m ago", latency: "22ms" },
    ]
  },
  "market-tick-data-service": {
    ...
    totalShards: 28,
    shards: [
      { key: "cefi/binance", status: "running", lastUpdate: "0.3s ago", latency: "2ms" },
      { key: "cefi/okx", status: "running", lastUpdate: "0.5s ago", latency: "3ms" },
      { key: "defi/hyperliquid", status: "running", lastUpdate: "1s ago", latency: "5ms" },
      { key: "cefi/deribit", status: "running", lastUpdate: "0.8s ago", latency: "4ms" },
    ]
  },
  "market-data-processing-service": {
    ...
    totalShards: 156,
    shards: [
      { key: "cefi/binance/btc", status: "running", lastUpdate: "1s ago", latency: "15ms" },
      { key: "cefi/binance/eth", status: "running", lastUpdate: "1s ago", latency: "14ms" },
      { key: "defi/aave/weth", status: "running", lastUpdate: "5s ago", latency: "32ms" },
  ...
  "position-balance-monitor-service": {
    ...
    totalShards: 18,
    shards: [
      { key: "binance/main", status: "running", lastUpdate: "30s ago", latency: "120ms" },
      { key: "hyperliquid/trading", status: "running", lastUpdate: "15s ago", latency: "45ms" },
      { key: "aave_v3/defi", status: "running", lastUpdate: "1m ago", latency: "200ms" },
    ]
  },
}
            ...
            return (
              <React.Fragment key={service.id}>
                <TableRow 
                  ...
                  onClick={() => hasShard && setExpandedService(isExpanded ? null : service.id)}
                >
                  ...
                  <TableCell>
                    <Badge
                      ...
                      className={cn(
                        health === "healthy" && "bg-[var(--pnl-positive)]/10 text-[var(--pnl-positive)] border-[var(--pnl-positive)]/20",
                        health === "degraded" && "bg-[var(--status-warning)]/10 text-[var(--status-warning)] border-[var(--status-warning)]/20"
                      )}
                    >
                      {health}
                    </Badge>
                  ...
                  <TableCell className="text-right font-mono">
                    <span className={cn(
                      service.healthyInstances < service.instances && "text-[var(--status-warning)]"
                    )}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <span className="font-mono">{service.cpu.toFixed(0)}%</span>
                      {service.cpu > 70 && <ArrowUpRight className="size-3 text-[var(--status-warning)]" />}
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <span className="font-mono">{service.memory.toFixed(0)}%</span>
                  ...
                  <TableCell className="text-right font-mono text-xs">
                    {service.p50Latency.toFixed(1)}ms / {service.p99Latency.toFixed(0)}ms
                  </TableCell>
                  <TableCell className="text-right">
                    ...
                    <TableCell colSpan={11} className="p-4">
                      <div className="space-y-3">
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          <code className="bg-background px-2 py-0.5 rounded text-xs font-mono">{shardConfig.dimensions}</code>
                          ...
                          <TableBody>
                            {shardConfig.shards.map((shard) => (
                              <TableRow key={shard.key}>
                                <TableCell>
                                  {shard.status === "running" && <CheckCircle2 className="size-4 text-[var(--pnl-positive)]" />}
                                  {shard.status === "stale" && <AlertTriangle className="size-4 text-[var(--status-warning)]" />}
                                  {shard.status === "failed" && <XCircle className="size-4 text-[var(--pnl-negative)]" />}
                                </TableCell>
                                <TableCell className="font-mono text-sm">{shard.key}</TableCell>
                                <TableCell className="text-right text-sm text-muted-foreground">{shard.lastUpdate}</TableCell>
                                <TableCell className="text-right font-mono text-sm">
                                  {shard.latency || (shard.error ? <span className="text-destructive">{shard.error}</span> : "—")}
                                </TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
