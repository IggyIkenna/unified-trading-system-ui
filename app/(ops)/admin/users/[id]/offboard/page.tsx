...
export default function OffboardUserPage() {
  ...
  const provisionedServices = React.useMemo(() => {
    if (!user) return [];
    return Object.entries(user.services).filter(
      ...
      <Card className="border-destructive">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            This will revoke access across all provisioned services for{" "}
            <strong>{user.name}</strong> ({user.email}). Choose whether to
            deactivate (preserve data) or delete (remove permanently) each
            service.
          </p>

          {provisionedServices.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No provisioned services to offboard.
            </p>
          ) : (
