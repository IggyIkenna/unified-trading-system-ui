...
const strategySchemas: Record<string, { params: ConfigParam[]; versions: { version: string; status: string; date: string; author: string; message: string }[] }> = {
  "btc-basis": {
    params: [
      { key: "min_funding_rate", label: "Min Funding Rate", type: "slider", value: 0.0001, min: 0.0001, max: 0.01, step: 0.0001, description: "Minimum funding rate to enter position" },
      { key: "max_leverage", label: "Max Leverage", type: "slider", value: 2.5, min: 1.0, max: 5.0, step: 0.5, description: "Maximum allowed leverage" },
      { key: "fractional_kelly", label: "Fractional Kelly", type: "slider", value: 0.25, min: 0, max: 1, step: 0.05, description: "Kelly criterion fraction" },
      { key: "delta_drift_threshold", label: "Delta Drift Threshold", type: "slider", value: 0.05, min: 0.01, max: 0.10, step: 0.01, description: "Rebalance when delta drifts beyond this" },
      { key: "execution_mode", label: "Execution Mode", type: "select", value: "HUF", options: ["SCE", "HUF", "EVT"], description: "Order execution strategy" },
      { key: "benchmark_type", label: "Benchmark Type", type: "select", value: "ARRIVAL", options: ["ARRIVAL", "ORACLE", "TWAP", "CLOSE"], description: "P&L benchmark reference" },
      { key: "allowed_venues", label: "Allowed Venues", type: "multiselect", value: ["BINANCE", "OKX", "BYBIT"], options: ["BINANCE", "OKX", "BYBIT", "COINBASE", "KRAKEN", "DERIBIT"], description: "Venues enabled for trading" },
      { key: "initial_capital", label: "Initial Capital", type: "number", value: 500000, description: "Starting capital allocation" },
      { key: "position_limit_usd", label: "Position Limit USD", type: "number", value: 1000000, description: "Maximum position size" },
      { key: "sor_enabled", label: "SOR Enabled", type: "switch", value: true, description: "Enable smart order routing" },
      { key: "post_only", label: "Post Only", type: "switch", value: false, description: "Only post maker orders" },
    ],
    versions: [
      { version: "v3.2", status: "Active", date: "Mar 15, 2026", author: "quant@odum.io", message: "Increase funding threshold" },
      { version: "v3.1", status: "Previous", date: "Mar 10, 2026", author: "quant@odum.io", message: "Initial config" },
      { version: "v3.0", status: "Archived", date: "Mar 5, 2026", author: "auto-deploy", message: "Grid sweep winner #42" },
    ],
  },
}
...
const venues = [
  // CeFi CLOB venues
  { id: "BINANCE-FUTURES", name: "Binance Futures", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["BINANCE-FUTURES"]], status: "connected" as const, latency: 2.1, rateLimit: "85%" },
  { id: "HYPERLIQUID", name: "Hyperliquid", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["HYPERLIQUID"]], status: "connected" as const, latency: 0.8, rateLimit: "45%" },
  { id: "DERIBIT", name: "Deribit", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["DERIBIT"]], status: "connected" as const, latency: 3.2, rateLimit: "62%" },
  { id: "OKX", name: "OKX", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["OKX"]], status: "degraded" as const, latency: 8.4, rateLimit: "92%" },
  { id: "BYBIT", name: "Bybit", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["BYBIT"]], status: "connected" as const, latency: 1.8, rateLimit: "55%" },
  // DeFi
  { id: "AAVE_V3_ETH", name: "Aave V3 (ETH)", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["AAVE_V3_ETH"]], status: "connected" as const, latency: 1.2, rateLimit: "30%" },
  { id: "UNISWAPV3-ETH", name: "Uniswap V3", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["UNISWAPV3-ETH"]], status: "connected" as const, latency: 2.1, rateLimit: "25%" },
  // Sports
  { id: "PINNACLE", name: "Pinnacle", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["PINNACLE"]], status: "connected" as const, latency: 45, rateLimit: "70%" },
  { id: "BETFAIR", name: "Betfair", type: VENUE_CATEGORIES[VENUE_CATEGORY_MAP["BETFAIR"]], status: "connected" as const, latency: 32, rateLimit: "60%" },
]
...
export default function ConfigPage() {
  ...
  const openConfigEditor = (strategyId: string) => {
    ...
    if (config) {
      ...
      config.params.forEach(p => { initialParams[p.key] = p.value })
  ...
  const getNewButtonLabel = () => {
    switch (activeTab) {
      ...
      case "strategies": return "New Strategy"
      case "venues": return "New Venue"
      case "risk": return "New Risk Config"
      case "credentials": return "New Credential"
      default: return "New"
    }
  }
                    ...
                    <div className="flex items-center gap-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Clients: {strategy.clients.join(", ")} &bull; Published{" "}
                          {strategy.lastPublished}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground max-w-md mx-auto">
                    API keys, wallet addresses, and authentication credentials for all connected
                    venues and data sources. Encrypted at rest with audit logging.
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  <div key={param.key} className="space-y-2">
                    ...
                    {param.type === "select" && param.options && (
                      <Select
                        ...
                        onValueChange={(v) => setEditedParams(prev => ({ ...prev, [param.key]: v }))}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    {param.type === "multiselect" && param.options && (
                      <div className="flex flex-wrap gap-2">
                        {param.options.map((opt) => {
                          const selected = Array.isArray(editedParams[param.key]) && (editedParams[param.key] as string[]).includes(opt)
                          return (
                            <label key={opt} className="flex items-center gap-1.5 text-sm cursor-pointer">
                              <Checkbox
                                checked={selected}
                                onCheckedChange={(checked) => {
                                  const current = Array.isArray(editedParams[param.key]) ? [...editedParams[param.key] as string[]] : []
                                  if (checked) {
                                    setEditedParams(prev => ({ ...prev, [param.key]: [...current, opt] }))
                                  } else {
                                    setEditedParams(prev => ({ ...prev, [param.key]: current.filter(v => v !== opt) }))
                                  }
                                }}
                              />
                              {opt}
                    ...
                    {param.type === "number" && (
                      <Input
                        ...
                        onChange={(e) => setEditedParams(prev => ({ ...prev, [param.key]: parseFloat(e.target.value) || 0 }))}
                        className="font-mono"
                      />
                    )}
                    ...
                    {param.type === "switch" && (
                      <Switch
                        ...
                        onCheckedChange={(checked) => setEditedParams(prev => ({ ...prev, [param.key]: checked }))}
                      />
                    )}
              ...
              <div className="space-y-3">
                ...
                {getChangedParams().length > 0 ? (
                  <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
                    <p className="text-sm font-medium">Changed parameters ({getChangedParams().length} of {strategySchemas[editingStrategy].params.length}):</p>
