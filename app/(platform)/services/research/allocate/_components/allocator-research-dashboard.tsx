...
function modeBadge(mode: AllocatorInstance["mode"]): string {
  return mode === "PRIMARY" ? "text-foreground border-border" : "text-violet-400 border-violet-500/40 bg-violet-500/10";
}

export function AllocatorResearchDashboard() {
  return (
      ...
      <div className="platform-page-width space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="text-body text-muted-foreground max-w-2xl">
            Per-client allocators decide how capital flows between strategy instances. 8 allocator archetypes (FIXED,
            PNL_WEIGHTED, SHARPE_WEIGHTED, RISK_PARITY, KELLY, MIN_CVAR, REGIME_AWARE, MANUAL) emit{" "}
            <code>AllocationDirective</code> events; shadow instances run alongside without routing capital.
          </p>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Card>
            <CardContent className="pt-5">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Instances</p>
              <p className="text-2xl font-bold font-mono">{MOCK_ALLOCATOR_INSTANCES.length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-5">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Managed NAV</p>
          ...
          <Card>
            <CardContent className="pt-5">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Shadow vs primary</p>
              <p className="text-2xl font-bold font-mono">
          ...
          <Card>
            <CardContent className="pt-5">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Awaiting approval</p>
        ...
        <Tabs defaultValue="instances" className="space-y-4">
                  ...
                  <TableBody>
                    {MOCK_ALLOCATOR_INSTANCES.map((inst) => (
                      ...
                      >
                        ...
                        <TableCell className="font-mono text-sm">{inst.managed_strategy_instance_ids.length}</TableCell>
                        <TableCell>
                          <Badge variant="outline" className={cn("text-[10px]", statusBadge(inst.status))}>
                            {inst.status}
                          </Badge>
                        </TableCell>
                      </TableRow>
                  ...
                  <TableBody>
                    {MOCK_SHADOW_COMPARE.map((r) => (
                      <TableRow key={r.strategy_instance_id} className="border-border/30">
                        ...
                        <TableCell className="font-mono text-xs">{formatNumber(r.primary_weight * 100, 1)}%</TableCell>
                        <TableCell className="font-mono text-xs">{formatNumber(r.shadow_weight * 100, 1)}%</TableCell>
                        <TableCell
                          className={cn(
                            "font-mono text-xs",
          ...
          <TabsContent value="approvals">
            <Card>
              <CardContent className="space-y-3 pt-5">
                {MOCK_APPROVAL_QUEUE.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No MANUAL allocator directives awaiting approval.</p>
                ) : (
