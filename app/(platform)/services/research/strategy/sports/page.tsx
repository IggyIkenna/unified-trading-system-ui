...
const MODEL_FAMILIES: ModelFamilySummary[] = [
  { id: "pregame_fundamental", name: "Pre-Game xG", targetType: "xG regression", accuracy: 0.684, clvBps: 3.1, fixtures: 2840, status: "active" },
  { id: "pregame_market", name: "Pre-Game CLV", targetType: "CLV classification", accuracy: 0.627, clvBps: 2.4, fixtures: 2840, status: "active" },
  { id: "ht_fundamental", name: "Half-Time xG", targetType: "2H xG regression", accuracy: 0.591, clvBps: 1.8, fixtures: 1420, status: "stale" },
  { id: "ht_market", name: "HT Market CLV", targetType: "HT CLV bps", accuracy: 0.553, clvBps: 1.2, fixtures: 1420, status: "stale" },
  { id: "meta", name: "Bet Quality", targetType: "ROI flag", accuracy: 0.715, clvBps: 4.2, fixtures: 2840, status: "active" },
];
...
const LEAGUE_RESEARCH: LeagueResearchRow[] = [
  { league: "EPL", seasons: 7, fixtures: 2660, features: 635, modelAccuracy: 0.68, avgClvBps: 3.1, roiPct: 7.2, edgeDecay: "42d" },
  { league: "La Liga", seasons: 7, fixtures: 2660, features: 612, modelAccuracy: 0.65, avgClvBps: 2.2, roiPct: 5.8, edgeDecay: "38d" },
  { league: "Bundesliga", seasons: 7, fixtures: 2142, features: 608, modelAccuracy: 0.64, avgClvBps: 1.8, roiPct: 4.9, edgeDecay: "35d" },
  { league: "Serie A", seasons: 7, fixtures: 2660, features: 618, modelAccuracy: 0.63, avgClvBps: 1.9, roiPct: 5.1, edgeDecay: "36d" },
  { league: "Ligue 1", seasons: 7, fixtures: 2660, features: 595, modelAccuracy: 0.61, avgClvBps: 1.1, roiPct: 3.2, edgeDecay: "30d" },
  { league: "UCL", seasons: 7, fixtures: 875, features: 635, modelAccuracy: 0.66, avgClvBps: 4.2, roiPct: 8.4, edgeDecay: "48d" },
  { league: "UEL", seasons: 7, fixtures: 665, features: 620, modelAccuracy: 0.62, avgClvBps: 2.8, roiPct: 6.1, edgeDecay: "40d" },
];
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
        ...
        <MetricCard tone="grid" density="compact" label="Active Models" primary={`${MODEL_FAMILIES.filter((m) => m.status === "active").length}`} primaryClassName="text-emerald-400" />
        ...
        <MetricCard tone="grid" density="compact" label="Avg CLV" primary={`${avgClv.toFixed(1)}%`} primaryClassName="text-emerald-400" />
        <MetricCard tone="grid" density="compact" label="Total P&L" primary={`$${(totalPnl / 1000).toFixed(1)}k`} primaryClassName="text-emerald-400" />
                ...
                <tbody>
                  {LEAGUE_RESEARCH.map((l) => (
                    <tr key={l.league} className="border-b border-muted/30 hover:bg-muted/30">
                      ...
                      <td className={cn("text-right py-1.5 font-mono", l.modelAccuracy >= 0.65 ? "text-emerald-500" : "text-amber-500")}>{(l.modelAccuracy * 100).toFixed(1)}%</td>
                      <td className={cn("text-right py-1.5 font-mono", l.avgClvBps >= 2 ? "text-emerald-500" : "text-amber-500")}>{l.avgClvBps}</td>
                      <td className={cn("text-right py-1.5 font-mono", l.roiPct >= 5 ? "text-emerald-500" : l.roiPct >= 0 ? "text-amber-500" : "text-rose-500")}>{l.roiPct}%</td>
            ...
            {clvRecords.map((r, i) => (
              <div key={i} className="flex items-center justify-between rounded-md bg-muted/50 px-3 py-2 text-sm">
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  <span className="text-emerald-500 font-mono">{r.clvHitRate > 0 ? (r.clvHitRate * 100).toFixed(1) : 0}% hit</span>
