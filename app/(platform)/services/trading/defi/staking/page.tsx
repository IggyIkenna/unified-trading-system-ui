  ...
  return (
    <DeFiDataProvider>
      <WidgetScroll className="h-full bg-background">
        <div className="p-6 max-w-[1600px] mx-auto space-y-6" data-testid="staking-dashboard">
          <PageHeader
            title={
          ...
          {/* Stake / Unstake execution widget — SSOT YIELD_STAKING_SIMPLE archetype.
            Mounted inline so e2e specs can drive STAKE + UNSTAKE actions end-to-end. */}
          <Card>
            <CardHeader>
                    ...
                    <TableBody>
                      {VALIDATORS.map((v) => (
                        <TableRow key={v.id} data-testid="staking-validators-row">
                          ...
                          <TableCell className="text-xs text-right font-mono">
                            <span
                              className={v.slashingEvents > 0 ? "text-rose-400 font-bold" : "text-muted-foreground"}
                            >
                              {v.slashingEvents}
                            </span>
                          </TableCell>
