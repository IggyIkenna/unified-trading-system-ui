...
const CEFI_VENUES = ["Binance", "OKX", "Deribit", "Bybit", "Hyperliquid"] as const
const SUB_ACCOUNT_VENUES = ["Binance", "OKX", "Bybit"] as const
const TRANSFER_ASSETS = ["USDC", "BTC", "ETH", "SOL"] as const
const NETWORKS = ["Ethereum", "Arbitrum", "Optimism", "Base", "Polygon"] as const
const SUB_ACCOUNTS = ["Trading-1", "Trading-2", "MM-Alpha", "Arb-Beta"] as const
...
const MOCK_TRANSFER_HISTORY: TransferHistoryEntry[] = [
  { timestamp: "2026-03-23 14:32", type: "Venue to Venue", from: "Binance", to: "OKX", asset: "USDC", amount: 50_000, status: "Completed", txHash: "0x3a7f...e2b1" },
  { timestamp: "2026-03-23 13:15", type: "Withdraw", from: "Bybit", to: "0x7a23...4f91", asset: "ETH", amount: 12.5, status: "Processing", txHash: "0x91cd...8f3a" },
  { timestamp: "2026-03-23 11:48", type: "Sub-account", from: "Trading-1", to: "Main", asset: "BTC", amount: 2.0, status: "Completed", txHash: "-" },
  { timestamp: "2026-03-22 22:05", type: "Venue to Venue", from: "OKX", to: "Deribit", asset: "BTC", amount: 5.0, status: "Completed", txHash: "0x6b4e...d102" },
  { timestamp: "2026-03-22 19:30", type: "Deposit", from: "0x8b12...c4a3", to: "Binance", asset: "USDC", amount: 100_000, status: "Pending", txHash: "0xf2a8...7e59" },
]
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Select value={transferType} onValueChange={(v) => setTransferType(v as TransferType)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="sub-account">Sub-account &harr; Main</SelectItem>
          ...
          <div className="space-y-4">
              ...
              <Select value={asset} onValueChange={setAsset}>
                ...
                <SelectContent>
                  {TRANSFER_ASSETS.map((a) => (
                    <SelectItem key={a} value={a}><span className="font-mono">{a}</span></SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="p-3 rounded-lg border bg-muted/30 space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono">{availableBalance.toLocaleString()} {asset}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Est. Arrival</span>
                <span className="flex items-center gap-1"><Clock className="size-3" />~30 min (blockchain confirmation)</span>
              </div>
            </div>
            <Button className="w-full" disabled={amountNum <= 0 || amountNum > availableBalance || fromVenue === toVenue}>
              Initiate Transfer
            </Button>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-lg border bg-muted/30">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-emerald-400">Instant (no blockchain confirmation)</span>
              </div>
            </div>
        ...
        {transferType === "withdraw" && (
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input placeholder="0x... or ENS name" value={toAddress} onChange={(e) => setToAddress(e.target.value)} className="font-mono text-xs" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">Network / Chain</label>
        ...
        {transferType === "deposit" && (
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">Deposit to Venue</label>
              <Select value={fromVenue} onValueChange={(v) => setFromVenue(v)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {CEFI_VENUES.map((v) => (
            ...
            <div className="p-3 rounded-lg border bg-muted/30 space-y-3">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <code className="text-xs font-mono bg-background px-2 py-1 rounded flex-1 truncate">0x7a23b8c1d9e4f6a2b3c5d7e8f0a1b2c3d4e5f691</code>
                  <Button variant="outline" size="sm" className="h-7 gap-1 text-xs shrink-0" onClick={handleCopyAddress}>
          ...
          <TableBody>
            {MOCK_TRANSFER_HISTORY.map((tx, idx) => (
              <TableRow key={`${tx.txHash}-${idx}`}>
                <TableCell className="text-xs font-mono whitespace-nowrap">{tx.timestamp}</TableCell>
                ...
                <TableCell className="text-right text-xs font-mono">{tx.amount.toLocaleString()}</TableCell>
                <TableCell>
                  <Badge
                    variant="outline"
                    className={cn(
                      ...
                      tx.status === "Processing" && "border-amber-500/50 text-amber-400",
                      tx.status === "Pending" && "border-blue-500/50 text-blue-400",
                    )}
                  >
                    {tx.status}
                  </Badge>
                </TableCell>
                <TableCell className="text-xs font-mono text-muted-foreground">{tx.txHash}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            Transfer
            <ChevronDown className={cn("size-3.5 transition-transform", transferOpen && "rotate-180")} />
          </Button>
