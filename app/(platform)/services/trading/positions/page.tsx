...
function PositionsPageContent() {
  ...
  const [sideFilter, setSideFilter] = React.useState<"all" | "LONG" | "SHORT">(
    "all",
  );
  const [strategyFilter, setStrategyFilter] = React.useState(
    strategyIdFilter || "all",
  );
  ...
  const handleWsMessage = React.useCallback(
    (msg: Record<string, unknown>) => {
      if (msg.channel === "positions" && msg.type === "pnl_update") {
        const updatedPositions = (msg.data as Record<string, unknown>)
          ?.positions as PositionRecord[] | undefined;
        if (updatedPositions) {
          queryClient.setQueryData(["positions", undefined], (old: unknown) => {
            ...
            const oldPositions = (oldData.positions ??
              oldData) as PositionRecord[];
            if (!Array.isArray(oldPositions)) return old;
            const updateMap = new Map(
              updatedPositions.map((p) => [p.instrument + p.venue, p]),
            );
  ...
  const positions: PositionRecord[] = React.useMemo(() => {
    ...
    if (globalScope.strategyIds.length > 0) {
      result = result.filter((p) =>
        globalScope.strategyIds.includes(p.strategy_id),
      );
    }
    return result;
  ...
  const filteredPositions = React.useMemo(() => {
    ...
    if (instrumentTypeFilter !== "All") {
      result = result.filter(
        (p) => classifyInstrument(p.instrument) === instrumentTypeFilter,
      );
    }
  ...
  const summary = React.useMemo(
    () => ({
      ...
      totalNotional: filteredPositions.reduce(
  ...
  const positionFilterDefs: FilterDefinition[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        options: uniqueStrategies.map(([id, name]) => ({
          value: id,
          label: name,
        })),
      },
  ...
  const handleFilterChange = React.useCallback(
    (key: string, value: unknown) => {
      switch (key) {
        ...
        case "side":
          setSideFilter(
            ((value as string) || "all") as "all" | "LONG" | "SHORT",
          );
          break;
      }
    },
  ...
  return (
    <div className="p-6">
        ...
        <Card className="bg-card/50">
          <CardContent className="p-4">
            ...
            >
              ...
              {formatCurrency(Math.abs(summary.unrealizedPnL))}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card/50">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <Select
          value={sideFilter}
          onValueChange={(v) => setSideFilter(v as "all" | "LONG" | "SHORT")}
        >
                ...
                <TableRow key={pos.id} className="hover:bg-muted/30">
                  <TableCell>
                    <div className="flex flex-col">
                      <Link
                        href={getInstrumentRoute(
                          pos.instrument,
                          classifyInstrument(pos.instrument),
                        )}
                      ...
                      <div className="flex flex-col items-end">
                        <span
                          className={cn(
                            ...
                            pos.health_factor >= 1.5 &&
                              pos.health_factor < 2.0 &&
                              "text-amber-500",
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {(
                            ((pos.health_factor - 1.0) / pos.health_factor) *
                            100
                          ).toFixed(0)}
                          % to liq
                        </span>
