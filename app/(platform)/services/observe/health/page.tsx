...
function getSeverityColor(severity: string) {
  switch (severity) {
    ...
    case "error": return "text-[var(--status-error)] bg-[var(--status-error)]/10"
...
const DEPENDENCY_GRAPH: DependencyNode[] = [
  ...
  { name: "unified-internal-contracts", tier: "T0", deps: [], status: "healthy" },
  { name: "unified-api-contracts", tier: "T0", deps: ["unified-trading-library"], status: "healthy" },
  { name: "unified-cloud-interface", tier: "T1", deps: ["unified-trading-library", "unified-internal-contracts"], status: "healthy" },
  { name: "unified-config-interface", tier: "T1", deps: ["unified-trading-library"], status: "healthy" },
  { name: "unified-events-interface", tier: "T1", deps: ["unified-trading-library", "unified-cloud-interface"], status: "healthy" },
  { name: "unified-market-interface", tier: "T2", deps: ["unified-api-contracts", "unified-cloud-interface"], status: "healthy" },
  { name: "unified-trade-execution-interface", tier: "T2", deps: ["unified-api-contracts", "unified-cloud-interface"], status: "healthy" },
  { name: "execution-service", tier: "T3", deps: ["unified-trade-execution-interface", "unified-events-interface"], status: "healthy" },
  { name: "strategy-service", tier: "T3", deps: ["unified-market-interface", "unified-events-interface"], status: "healthy" },
  { name: "risk-and-exposure-service", tier: "T3", deps: ["unified-events-interface", "unified-cloud-interface"], status: "degraded" },
  { name: "market-tick-data-service", tier: "T3", deps: ["unified-market-interface", "unified-cloud-interface"], status: "healthy" },
  { name: "instruments-service", tier: "T3", deps: ["unified-api-contracts", "unified-cloud-interface"], status: "healthy" },
  { name: "alerting-service", tier: "T3", deps: ["unified-events-interface", "unified-cloud-interface"], status: "healthy" },
]
                  ...
                  <div key={node.name} className={cn("p-3 rounded-lg border text-sm", TIER_COLORS[tier])}>
                    ...
                    {node.deps.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {node.deps.map(dep => (
                          <span key={dep} className="inline-flex items-center gap-0.5 text-[10px] text-muted-foreground bg-muted/50 px-1.5 py-0.5 rounded">
  ...
  if (servicesWithResources.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">Resource data is populated when services report CPU/memory usage</p>
      </div>
    )
  }
        ...
        <Card key={service.name} className="bg-card/50">
          ...
          <CardContent className="space-y-3">
            {service.cpuPct != null && (
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-muted-foreground flex items-center gap-1"><Cpu className="size-3" /> CPU</span>
                  <span className={cn(
                    "font-mono",
                    service.cpuPct > 80 ? "text-[var(--status-error)]" :
                    service.cpuPct > 60 ? "text-[var(--status-warning)]" :
                ...
                <Progress
                  ...
                  className={cn(
                    "h-1.5",
                    service.cpuPct > 80 ? "[&>div]:bg-[var(--status-error)]" :
                    service.cpuPct > 60 ? "[&>div]:bg-[var(--status-warning)]" :
            ...
            {service.memoryPct != null && (
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-muted-foreground flex items-center gap-1"><MemoryStick className="size-3" /> Memory</span>
                  <span className={cn(
                    "font-mono",
                    service.memoryPct > 80 ? "text-[var(--status-error)]" :
                    service.memoryPct > 60 ? "text-[var(--status-warning)]" :
                    "text-[var(--status-live)]"
                  )}>{service.memoryPct}%</span>
                </div>
                <Progress
                  value={service.memoryPct}
                  className={cn(
                    "h-1.5",
                    service.memoryPct > 80 ? "[&>div]:bg-[var(--status-error)]" :
                    service.memoryPct > 60 ? "[&>div]:bg-[var(--status-warning)]" :
                    "[&>div]:bg-[var(--status-live)]"
                  )}
                />
              </div>
            ...
            <TableBody>
              {filtered.slice(0, 50).map(log => (
                <TableRow key={log.id} className="font-mono text-xs">
                  ...
                  <TableCell>
                    <Badge variant="outline" className={cn("text-[10px] uppercase px-1.5", getSeverityColor(log.severity))}>
                      {log.severity}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-muted-foreground truncate max-w-[120px]">{log.correlationId ?? "-"}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export default function HealthPage() {
  ...
  const { data: healthData, isLoading: healthLoading, isError: healthError, refetch: refetchHealth } = useServiceHealth()
            ...
            <TabsContent value="services" className="space-y-6">
              {healthLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Services will appear when the API reports health data</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* Critical Tier */}
                        ...
                        <DropdownMenuItem
                          onClick={() => {
                            const cols: ExportColumn[] = [
                              ...
                              { key: "freshness", header: "Freshness (s)", format: "number" },
                              { key: "sla", header: "SLA (s)", format: "number" },
