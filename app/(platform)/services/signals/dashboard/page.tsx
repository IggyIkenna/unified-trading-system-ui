...
export default function CounterpartyDashboardPage() {
  ...
  const [familyFilter, setFamilyFilter] = useState<StrategyFamily | undefined>(undefined);
  const [archetypeFilter, setArchetypeFilter] = useState<StrategyArchetype | undefined>(undefined);

  const anyMock = emissions.isMock || backtest.isMock || health.isMock || pnl.isMock;
  ...
  const filteredEmissions = useMemo(() => {
    ...
    return raw.filter((em) => {
      ...
      if (familyFilter) {
        const family =
          archetypeToken in ARCHETYPE_TO_FAMILY ? ARCHETYPE_TO_FAMILY[archetypeToken as StrategyArchetype] : undefined;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" data-testid="signal-broadcast-dashboard-page">
      <div className="platform-page-width space-y-6 p-6">
        <header className="space-y-3">
          <div className="flex items-center gap-2">
            <h1 className="text-page-title font-semibold tracking-tight">Signals dashboard</h1>
          ...
          <p className="text-body text-muted-foreground max-w-3xl">
            Observability for the signals Odum delivers to your webhook endpoint. Odum does not see your fills,
            positions, or P&amp;L: this dashboard is a delivery-audit surface plus a backtest-paper-live parity view.
            The full signal payload flows only through your HTTPS endpoint.
          </p>
        </header>
          ...
          <CardContent>
            <dl className="grid grid-cols-1 gap-3 text-sm md:grid-cols-4">
              <div>
                ...
                <dd className="font-mono text-xs" data-testid="counterparty-profile-id">
                  {cp.id}
                </dd>
              </div>
              <div>
              ...
              <div>
                <dt className="text-xs text-muted-foreground">Entitled slots</dt>
                <dd className="font-mono text-xs" data-testid="counterparty-profile-slot-count">
                  {cp.allowed_slots.length}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-muted-foreground">Rate limit (per strategy)</dt>
                <dd className="font-mono text-xs">{cp.rate_limit_per_strategy_per_sec}/s</dd>
              </div>
            </dl>
          </CardContent>
        </Card>
        ...
        {health.error !== null && <FetchErrorBanner label="delivery health" detail={health.error} />}
        ...
        >
          <span className="font-medium uppercase tracking-wide">Emission filter</span>
          ...
          {(familyFilter || archetypeFilter) && (
            <span className="font-mono text-[10px] text-amber-500" data-testid="signals-dashboard-filter-count">
        ...
        {emissions.data !== null && (
          <SignalHistoryTable emissions={filteredEmissions} entitledSlots={cp.allowed_slots} />
        )}
        {emissions.error !== null && <FetchErrorBanner label="signal history" detail={emissions.error} />}

        {backtest.data !== null && <BacktestComparisonPanel rows={backtest.data} />}
        {backtest.error !== null && <FetchErrorBanner label="backtest vs paper vs live" detail={backtest.error} />}

        <PnlAttributionPanel enabled={cp.pnl_reporting_enabled} rows={pnl.data ?? []} />
        ...
        <section className="rounded-lg border border-dashed border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">
          ...
          <p className="mt-1">
            Delivery state is refreshed on page load. If deliveries appear to be failing, verify that your webhook
            endpoint is reachable and that the HMAC signature validation in your consumer matches the shared-secret
            rotation window. Contact Odum operations to rotate a compromised secret.
          </p>
        </section>
      </div>
    </div>
...
function FetchErrorBanner({ label, detail }: { readonly label: string; readonly detail: string }) {
  return (
    <div
    ...
    >
      ...
      <span className="text-muted-foreground">{label}</span> <span className="font-mono text-xs">({detail})</span>
    </div>
  );
}
