    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="flex justify-end font-mono">
          {formatNumeric(row.original.liveValue)}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <span className="flex justify-end font-mono">
          {formatNumeric(row.original.batchValue)}
        </span>
      ),
    },
    {
            ...
            {record.status !== "resolved" && (
              <>
                ...
                <Button
                  ...
                  title="Investigate"
                  onClick={() =>
                    handlers.onResolveAction(record, "investigate")
                  }
                >
...
function breakTypeBadge(bt: BreakType) {
  ...
  return (
    <Badge
      variant="outline"
      className={`text-[10px] uppercase ${style.className}`}
    >
      {bt}
    </Badge>
  );
...
export default function ReconciliationPage() {
  ...
  const [resolvingBreak, setResolvingBreak] =
    React.useState<ReconciliationRecord | null>(null);
  const [resolveAction, setResolveAction] = React.useState<
  ...
  const filtered = React.useMemo(() => {
    return history.filter((r) => {
      if (breakTypeFilter !== "all" && r.breakType !== breakTypeFilter)
        return false;
  ...
  const handleBookCorrection = React.useCallback(
    (record: ReconciliationRecord) => {
      ...
      router.push(
        `/services/trading/book?prefill=${encodeURIComponent(JSON.stringify(prefill))}`,
      );
    },
    [router],
  );
  ...
  const positionBreaks = history.filter(
    (r) => r.breakType === "position" && r.status !== "resolved",
  ).length;
  const pnlDiscrepancies = history.filter(
    (r) => r.breakType === "pnl" && r.status !== "resolved",
  ).length;
  const feeMismatches = history.filter(
    (r) => r.breakType === "fee" && r.status !== "resolved",
  ).length;
          ...
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Could not reach the reporting API. Check connectivity and try
                again.
              </p>
            </div>
            <Button
        ...
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                There are no reconciliation records yet. Records will appear
                after the first batch/live comparison cycle.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      >
        <DialogContent>
            ...
            <div className="space-y-1.5">
              ...
              {resolveNote.length > 0 && resolveNote.length < 10 && (
                <p className="text-[10px] text-rose-500">
                  {10 - resolveNote.length} more characters required
                </p>
              )}
            </div>
          </div>
          <DialogFooter>
            ...
            >
              Confirm{" "}
              {resolveAction.charAt(0).toUpperCase() + resolveAction.slice(1)}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
