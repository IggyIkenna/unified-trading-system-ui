...
const slides = [
  ...
  // Slide 4: Operational Lanes (renamed from Domain Lanes)
  {
    ...
    subtitle:
      "Six operational lanes flow through the lifecycle with varying intensity at each stage.",
    stages: [
      "Acquire",
      "Build",
      "Promote",
  ...
  // Slide 5: Commercial Packaging
  {
    ...
    subtitle:
      "Each service maps to specific lifecycle stages. Clients can start at any point and expand over time.",
    services: [
      {
        name: "Data Provision",
        stages: ["Acquire"],
  ...
  // Slide 6: Why One Platform
  {
    ...
    conclusion:
      "Clients enter at any stage - but they're entering the same system we run our own capital through.",
    differentiators: [
  ...
  },
  {
    ...
    instrumentNote:
      "instruments registered in our database (including historical/expired)",
    differentiator:
  ...
  },
  {
    ...
    disclosure:
      "Investment Management note: +34.2% since Feb 2026 on client accounts on Binance and OKX.",
  },
  {
    id: 11,
    type: "flywheel",
    ...
    subtitle:
      "Clients enter at one service, naturally expand to others. Each step increases contract value 3-10x.",
    funnel: [
  ...
  return (
    <div
      className={cn(
        ...
        colorStyles[market.color as keyof typeof colorStyles],
      )}
    >
      ...
      {market.count && (
        <div
          className={cn(
            ...
            textColorStyles[market.color as keyof typeof textColorStyles],
          )}
        >
          {market.count}
        </div>
      ...
      <main className="flex-1 flex items-center justify-center p-8 overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {slide.type === "doctrine" && (
              <div>
                <h2 className="text-3xl font-bold text-primary border-b border-border pb-2 mb-2">
                  {slide.title}
                </h2>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  {slide.points?.map(
                    (
                      point: { problem: string; solution: string },
                      i: number,
                    ) => (
                      <motion.div
                        key={i}
                      ...
                      >
                        <div className="flex-1">
                          <div className="text-xs text-destructive uppercase tracking-wider mb-1">
                            Problem
                          </div>
                ...
                {slide.differentiators && (
                  <div className="grid grid-cols-3 gap-3 mb-6">
                    {(slide.differentiators as string[]).map(
            ...
            {slide.type === "lifecycle-new" && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  {slide.stages?.map(
                    (stage: { name: string; desc: string }, i: number) => (
            ...
            {slide.type === "lanes" && (
              <div>
                ...
                <div className="space-y-3">
                  {slide.lanes?.map(
                    (
                      lane: { name: string; desc: string; color: string },
                      i: number,
                    ) => {
                      const colorClasses: Record<string, string> = {
                        sky: "border-sky-400/30 bg-sky-400/5 text-sky-400",
                        violet:
                          "border-violet-400/30 bg-violet-400/5 text-violet-400",
                        amber:
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-xs text-muted-foreground w-64 text-right">
                            {lane.desc}
                          </div>
                        </motion.div>
                      );
                ...
                <div className="space-y-3">
                  {(
                    ...
                    const colorMap: Record<
                      string,
                      { node: string; nodeDim: string; glow: string }
                    > = {
                      sky: {
                        node: "bg-sky-400",
                            ...
                            >
                              <div
                                className={cn(
                                  ...
                                  isEmphasized
                                    ? cn("size-3", colors.node, colors.glow)
                                    : cn("size-1.5", colors.nodeDim),
                                )}
                              />
                            </div>
                          );
                      ...
                      return (
                        <motion.div
                          ...
                          className={cn(
                            ...
                            colors[gap.color as keyof typeof colors],
                          )}
                        >
            ...
            {slide.type === "coverage" && (
              <div>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h2 className="text-3xl font-bold text-primary border-b border-border pb-2 mb-1">
                      {slide.title}
                    </h2>
                    <p className="text-sm text-muted-foreground">
                      Trading 24/7/365 — same instruments, multiple venues,
                    ...
                    {[...VENUE_LIST, ...VENUE_LIST].map((venue, i) => {
                      ...
                      return (
                        <span
                          key={`${venue.name}-${i}`}
                          className={cn(
                            "text-xs px-2 py-1 rounded border bg-background/50",
                            colors[venue.color],
                          )}
                        >
                          {venue.name}
            ...
            {slide.type === "demo" && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  {slide.sections?.map(
                    (section: { name: string; desc: string }, i: number) => (
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
            ...
            {slide.type === "lifecycle" && (
              <div>
                <h2 className="text-3xl font-bold text-primary border-b border-border pb-2 mb-8">
                  {slide.title}
                </h2>
            ...
            {slide.type === "revenue" && (
              <div>
                <h2 className="text-3xl font-bold text-primary border-b border-border pb-2 mb-6">
                  {slide.title}
                </h2>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-card">
                        <th className="text-left p-3 text-primary font-semibold border-b border-primary">
                          Service
                        </th>
                        <th className="text-left p-3 text-primary font-semibold border-b border-primary">
                          Model
            ...
            {slide.type === "flywheel" && (
              <div>
                <h2 className="text-3xl font-bold text-primary border-b border-border pb-2 mb-4">
                  {slide.title}
                </h2>
                <p className="text-muted-foreground mb-6">{slide.subtitle}</p>
                <div className="flex items-center gap-2 mb-4">
                  {slide.funnel?.map(
                    (
                      step: { name: string; sub: string; active?: boolean },
                      i: number,
                    ) => (
                      <React.Fragment key={i}>
                        <motion.div
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 rounded-lg border border-primary/30 bg-primary/5 text-center">
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Data clients become backtesting clients.
                      <br />
                      Backtesting clients want execution.
                      <br />
                      Execution clients want the full platform.
                    </p>
                    <p className="mt-4 text-primary font-medium">
                      Every entry point is a gateway to deeper engagement.
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border border-border bg-card">
                    <h3 className="text-sm font-semibold text-primary uppercase tracking-wider mb-3">
            ...
            {slide.type === "operations" && (
              <div>
                <div className="flex items-start justify-between mb-4">
                  <h2 className="text-3xl font-bold text-primary border-b border-border pb-2">
                    {slide.title}
                  </h2>
            ...
            {slide.type === "traction" && (
              <div>
                ...
                <div className="mt-6 p-4 rounded-lg border border-primary/30 bg-primary/5 text-center">
                  <p className="text-sm text-muted-foreground">
                    This is a{" "}
                    <span className="text-primary font-medium">checkpoint</span>
                    . {slide.checkpoint?.split("checkpoint.")[1]}
                  </p>
                </div>
              </div>
            )}
            ...
            {slide.type === "ask" && (
              <div className="text-center">
                ...
                <div className="grid grid-cols-3 gap-6 mt-10">
                  {slide.asks?.map(
                    (ask: { title: string; items: string[] }, i: number) => (
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                ...
                >
                  Odum Research Ltd | FCA 975797 | {slide.contact} |
                  odum-research.com
                </motion.p>
              </div>
            )}
