...
function generateValidationCurves(): { epoch: number; accuracy: number; valAccuracy: number }[] {
  const data = []
...
function generateRegimePerformance(): { regime: string; sharpe: number; accuracy: number; drawdown: number }[] {
  return [
...
function generateRadarData(metrics: ExperimentMetrics | null) {
  ...
  return [
    ...
    { metric: "Dir. Acc", value: metrics.directionalAccuracy * 100, fullMark: 100 },
    ...
    { metric: "1-MaxDD", value: (1 - metrics.maxDrawdown) * 100, fullMark: 100 },
  ]
}
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-[60vh]">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mt-2">The experiment {experimentId} does not exist.</p>
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        {experiment.status === "running" && (
          <Card className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-muted-foreground">{experiment.progress}% complete</span>
            </div>
            <Progress value={experiment.progress} className="h-2" />
            <div className="flex items-center justify-between mt-2 text-xs text-muted-foreground">
              <span>Started {experiment.startedAt ? new Date(experiment.startedAt).toLocaleTimeString() : "N/A"}</span>
        ...
        <Tabs defaultValue="training" className="space-y-6">
                  ...
                  <div className="h-[280px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={validationCurves}>
                        ...
                        <XAxis dataKey="epoch" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
                        <YAxis tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} domain={[0.4, 1]} />
                        <Tooltip
                          contentStyle={{
                        ...
                        />
                        <Legend />
                        <Line type="monotone" dataKey="accuracy" stroke="var(--surface-ml)" strokeWidth={2} dot={false} name="Train Accuracy" />
                        <Line type="monotone" dataKey="valAccuracy" stroke="var(--status-warning)" strokeWidth={2} dot={false} name="Val Accuracy" />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
                  ...
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart data={radarData}>
                        ...
                        <Radar name="Performance" dataKey="value" stroke="var(--surface-ml)" fill="var(--surface-ml)" fillOpacity={0.3} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
                ...
                <div className="h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={featureImportance} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" opacity={0.5} />
                      <XAxis type="number" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} domain={[0, 0.2]} />
                      <YAxis dataKey="feature" type="category" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} width={150} />
                      <Tooltip
                        contentStyle={{
                          backgroundColor: "var(--background)",
                          border: "1px solid var(--border)",
          ...
          <TabsContent value="regimes" className="space-y-6">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Regime-Specific Performance</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {regimePerformance.map((regime) => (
                        <tr key={regime.regime} className="border-b border-border/50">
                          <td className="py-3 px-4 font-medium">{regime.regime}</td>
                          <td className={`py-3 px-4 text-right font-mono ${regime.sharpe >= 2.0 ? "text-[var(--status-live)]" : ""}`}>
                            {regime.sharpe.toFixed(2)}
                          </td>
                          <td className="py-3 px-4 text-right font-mono">{regime.accuracy}%</td>
                          <td className="py-3 px-4 text-right font-mono text-[var(--status-warning)]">{regime.drawdown}%</td>
                  ...
                  <div className="space-y-2">
                    {Object.entries(experiment.hyperparameters).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                        ...
                        <code className="font-mono font-medium">{String(value)}</code>
                      </div>
                    ))}
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between py-2 border-b border-border/50">
                      ...
                      <code className="font-mono">{experiment.trainingConfig.epochs}</code>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-border/50">
                      <span className="text-sm text-muted-foreground">Batch Size</span>
                      <code className="font-mono">{experiment.trainingConfig.batchSize}</code>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-border/50">
                      <span className="text-sm text-muted-foreground">Optimizer</span>
                      <code className="font-mono">{experiment.trainingConfig.optimizer}</code>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-border/50">
                      <span className="text-sm text-muted-foreground">Loss Function</span>
                      <code className="font-mono text-xs">{experiment.trainingConfig.lossFunction}</code>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-border/50">
                      <span className="text-sm text-muted-foreground">GPU Type</span>
                      <code className="font-mono">{experiment.trainingConfig.gpuType} x{experiment.trainingConfig.numGpus}</code>
                    </div>
                    <div className="flex items-center justify-between py-2">
                      <span className="text-sm text-muted-foreground">Early Stopping</span>
                      <Badge variant="outline">{experiment.trainingConfig.earlyStopping ? "Enabled" : "Disabled"}</Badge>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <TabsContent value="lineage" className="space-y-6">
                  ...
                  {dataset ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <code className="block text-xs font-mono">{dataset.id}</code>
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">Date Range</span>
                        <p className="text-sm">{dataset.dateRange.start} to {dataset.dateRange.end}</p>
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">Rows</span>
                        <p className="font-mono">{dataset.rowCount.toLocaleString()}</p>
                      </div>
                    </div>
                  ) : (
                ...
                <CardContent>
                  {featureSet ? (
                    <div className="space-y-3">
                      <div>
                        <span className="text-xs text-muted-foreground">Name</span>
                        <p className="font-medium">{featureSet.name} v{featureSet.version}</p>
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">ID</span>
                        <code className="block text-xs font-mono">{featureSet.id}</code>
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">Features</span>
                        <p className="font-mono">{featureSet.features.length}</p>
                      </div>
                      <div>
                    ...
                    <p className="text-muted-foreground text-sm">Feature set not found</p>
                  )}
                </CardContent>
              </Card>
                ...
                <CardContent>
                  {modelFamily ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        <span className="text-xs text-muted-foreground">Linked Strategies</span>
                        <p className="font-mono">{modelFamily.linkedStrategies.length}</p>
                      </div>
                    </div>
                  ) : (
                    <p className="text-muted-foreground text-sm">Model family not found</p>
                  )}
                </CardContent>
              </Card>
            </div>
            ...
            {modelFamily && modelFamily.linkedStrategies.length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">Related Live Strategies</CardTitle>
                </CardHeader>
                <CardContent>
