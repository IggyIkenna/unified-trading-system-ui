...
const regimePerformance = [
  ...
  { regime: "Trending", championSharpe: 2.6, challengerSharpe: 2.9, samples: 15200, description: "ADX > 25" },
...
const walkForwardResults = [
  ...
  { window: "W4", trainStart: "2025-09", trainEnd: "2025-12", testStart: "2026-01", testEnd: "2026-02", champSharpe: 2.52, challSharpe: 2.71, champAcc: 0.856, challAcc: 0.871 },
...
const statisticalTests = [
  ...
  { test: "Bootstrap Sharpe Diff", statistic: "0.17 [0.08, 0.26]", pValue: 0.0023, significant: true, conclusion: "95% CI excludes zero" },
...
const featureImportance = [
  ...
  { feature: "spot_perp_spread", champion: 0.10, challenger: 0.09 },
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Champion vs Challenger backtesting and statistical validation</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Select value={selectedComparison} onValueChange={setSelectedComparison}>
                ...
                <SelectContent>
                  <SelectItem value="funding-pred">Funding Rate Predictor</SelectItem>
                  <SelectItem value="vol-forecast">Vol Forecaster</SelectItem>
                  <SelectItem value="liq-detect">Liquidation Detector</SelectItem>
                </SelectContent>
              </Select>
              <Select value={validationPeriod} onValueChange={setValidationPeriod}>
        ...
        <div className="grid grid-cols-2 gap-6">
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Badge className="bg-[var(--status-success)] text-white">CHAMPION</Badge>
                  <span className="text-lg font-semibold">{champion.name}</span>
                </div>
                <Badge variant="outline">v{champion.version}</Badge>
              </div>
              <CardDescription>Currently deployed in production</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-4 gap-4">
                <div>
                  <p className="text-xs text-muted-foreground">Sharpe Ratio</p>
                  <p className="text-2xl font-bold">{champion.metrics.sharpe.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Accuracy</p>
                  <p className="text-2xl font-bold">{(champion.metrics.accuracy * 100).toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Max Drawdown</p>
                  <p className="text-2xl font-bold text-[var(--status-error)]">{(champion.metrics.maxDrawdown * 100).toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">AUC-ROC</p>
                  <p className="text-2xl font-bold">{champion.metrics.auc.toFixed(3)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-[var(--status-warning)]/30 bg-[var(--status-warning)]/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Badge className="bg-[var(--status-warning)] text-black">CHALLENGER</Badge>
                  <span className="text-lg font-semibold">{challenger.name}</span>
                </div>
            ...
            <CardContent>
              <div className="grid grid-cols-4 gap-4">
                <div>
                  <p className="text-xs text-muted-foreground">Sharpe Ratio</p>
                  <p className="text-2xl font-bold">{challenger.metrics.sharpe.toFixed(2)}</p>
                  <p className="text-xs text-[var(--status-success)]">+{getMetricDiff("sharpe").pctDiff.toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Accuracy</p>
                  <p className="text-2xl font-bold">{(challenger.metrics.accuracy * 100).toFixed(1)}%</p>
                  <p className="text-xs text-[var(--status-success)]">+{getMetricDiff("accuracy").pctDiff.toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Max Drawdown</p>
                  <p className="text-2xl font-bold text-[var(--status-error)]">{(challenger.metrics.maxDrawdown * 100).toFixed(1)}%</p>
                  <p className="text-xs text-[var(--status-success)]">{getMetricDiff("maxDrawdown").pctDiff.toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">AUC-ROC</p>
                  <p className="text-2xl font-bold">{challenger.metrics.auc.toFixed(3)}</p>
                  <p className="text-xs text-[var(--status-success)]">+{getMetricDiff("auc").pctDiff.toFixed(1)}%</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <CardContent className="py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold">Recommendation: Promote Challenger to Champion</p>
                  <p className="text-sm text-muted-foreground">
                    Challenger outperforms on 4/5 statistical tests with significantly better Sharpe ratio (+7.1%) across all regimes
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Out-of-sample backtest performance</CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <RechartsLine data={timeSeriesComparison}>
                        ...
                        />
                        <Legend />
                        <ReferenceLine y={2} stroke="hsl(var(--muted-foreground))" strokeDasharray="5 5" />
                        <Line 
                          type="monotone" 
                    ...
                    {Object.entries(champion.metrics).map(([key, value]) => {
                      ...
                      return (
                        <TableRow key={key}>
                          ...
                          <TableCell className={`text-right font-mono ${challengerWins ? "text-[var(--status-success)]" : "text-[var(--status-error)]"}`}>
                          ...
                          <TableCell>
                            <Badge variant={challengerWins ? "default" : "secondary"} className={challengerWins ? "bg-[var(--status-warning)]" : ""}>
          ...
          <TabsContent value="regimes" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>How models perform under different market conditions</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={regimePerformance} layout="vertical">
                      ...
                      <YAxis type="category" dataKey="regime" tick={{ fontSize: 12 }} width={80} />
                      <Tooltip 
                        contentStyle={{ 
                          backgroundColor: "hsl(var(--card))", 
                          border: "1px solid hsl(var(--border))" 
                      ...
                      />
                      <Legend />
                      <Bar dataKey="championSharpe" name="Champion" fill="var(--status-success)" />
                      <Bar dataKey="challengerSharpe" name="Challenger" fill="var(--status-warning)" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                <div className="mt-4 grid grid-cols-5 gap-4">
                  {regimePerformance.map(r => (
                    <div key={r.regime} className="text-center p-3 rounded-lg bg-muted/30">
                      ...
                      <p className="text-sm mt-1">{r.samples.toLocaleString()} samples</p>
                      <p className={`text-xs mt-1 ${r.challengerSharpe > r.championSharpe ? "text-[var(--status-success)]" : "text-[var(--status-error)]"}`}>
                        Challenger {r.challengerSharpe > r.championSharpe ? "+" : ""}{((r.challengerSharpe - r.championSharpe) / r.championSharpe * 100).toFixed(1)}%
                      </p>
                    </div>
                  ))}
                </div>
          ...
          <TabsContent value="walkforward" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Rolling window out-of-sample testing to detect overfitting</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">Champion Sharpe</TableHead>
                      <TableHead className="text-right">Challenger Sharpe</TableHead>
                      <TableHead className="text-right">Champion Acc</TableHead>
                      <TableHead className="text-right">Challenger Acc</TableHead>
                      <TableHead>Winner</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {walkForwardResults.map(w => (
                      <TableRow key={w.window}>
                        ...
                        <TableCell className="text-muted-foreground">{w.trainStart} → {w.trainEnd}</TableCell>
                        <TableCell>{w.testStart} → {w.testEnd}</TableCell>
                        <TableCell className="text-right font-mono">{w.champSharpe.toFixed(2)}</TableCell>
                        <TableCell className="text-right font-mono">{w.challSharpe.toFixed(2)}</TableCell>
                        <TableCell className="text-right font-mono">{(w.champAcc * 100).toFixed(1)}%</TableCell>
                        <TableCell className="text-right font-mono">{(w.challAcc * 100).toFixed(1)}%</TableCell>
                        <TableCell>
                          <Badge className="bg-[var(--status-warning)]">Challenger</Badge>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                ...
                <div className="mt-4 p-4 bg-muted/30 rounded-lg">
                  <p className="text-sm">
                    <span className="font-semibold">Summary:</span> Challenger wins 5/5 windows with average Sharpe improvement of +0.16 and accuracy improvement of +1.5%. 
                    Consistent outperformance suggests genuine model improvement rather than overfitting.
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="statistical" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Rigorous hypothesis testing to validate performance differences</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {statisticalTests.map(t => (
                      <TableRow key={t.test}>
                        ...
                        <TableCell className="text-right font-mono">{t.statistic}</TableCell>
                        <TableCell className="text-right font-mono">{t.pValue.toFixed(4)}</TableCell>
                        <TableCell>
                          {t.significant ? (
                            <Badge className="bg-[var(--status-success)]">Yes</Badge>
                          ) : (
                            <Badge variant="secondary">No</Badge>
                          )}
                        </TableCell>
                        <TableCell className="text-muted-foreground">{t.conclusion}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div className="p-4 bg-[var(--status-success)]/10 rounded-lg border border-[var(--status-success)]/30">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-semibold">4/5 Tests Significant</span>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      Strong statistical evidence that challenger outperforms champion on risk-adjusted returns.
                    </p>
                  </div>
                  <div className="p-4 bg-muted/30 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Return distributions are similar (KS test), suggesting performance improvement comes from better timing rather than different risk profile.
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <TabsContent value="features" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>How models weight different input features</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={featureImportance} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis type="number" domain={[0, 0.3]} tick={{ fontSize: 10 }} tickFormatter={(v) => `${(v * 100).toFixed(0)}%`} />
                      ...
                      />
                      <Legend />
                      <Bar dataKey="champion" name="Champion" fill="var(--status-success)" />
                      <Bar dataKey="challenger" name="Challenger" fill="var(--status-warning)" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                <div className="mt-4 p-4 bg-muted/30 rounded-lg">
                  <p className="text-sm">
                    <span className="font-semibold">Key Differences:</span> Challenger places more weight on <code className="bg-muted px-1 rounded">oi_change_1h</code> (+4%) and <code className="bg-muted px-1 rounded">liquidation_ratio</code> (+3%), 
                    while reducing reliance on <code className="bg-muted px-1 rounded">whale_flow</code> (-3%). This suggests better capture of short-term open interest dynamics.
                  </p>
                </div>
              </CardContent>
            </Card>
