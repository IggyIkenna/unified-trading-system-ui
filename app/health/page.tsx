...
import { CheckCircle2, XCircle, Loader2, AlertTriangle, RefreshCw, Clock, Server, Database, Shield, Globe, Terminal } from "lucide-react"
...
const TIER_DESCRIPTIONS: Record<DetectedTier, { label: string; description: string }> = {
  0: { label: "Tier 0 — UI Only", description: "No API gateway detected. UI running with in-browser mock store." },
  1: { label: "Tier 1 — UI + API Gateways", description: "API gateway serving mock data via MockStateStore. No downstream service fleet." },
  2: { label: "Tier 2 — Full Fleet", description: "API gateway + downstream services. Full engine parity with production topology." },
}
...
async function checkEndpoint(url: string, timeoutMs: number = 5000): Promise<{ ok: boolean; latencyMs: number; detail: string | null; data?: Record<string, unknown> }> {
...
function useHealthChecks() {
  ...
  const runChecks = React.useCallback(async () => {
    ...
    const definitions: Omit<HealthCheck, "status" | "latencyMs" | "detail">[] = [
      ...
      { name: "GET /alerts/list", description: "Alerts domain", group: "API Domains", required: true },
      { name: "GET /risk/limits", description: "Risk domain", group: "API Domains", required: true },
      { name: "GET /instruments/list", description: "Instruments domain", group: "API Domains", required: true },
      { name: "GET /ml/model-families", description: "ML domain", group: "API Domains", required: false },
    ...
    const initial: HealthCheck[] = definitions.map(d => ({ ...d, status: "checking", latencyMs: null, detail: null }))
    setChecks(initial)
    ...
    if (apiHealth.ok && apiData) {
      ...
      results[0] = { ...results[0], status: "down", latencyMs: apiHealth.latencyMs, detail: apiHealth.detail }
    }
    setChecks([...results])
    ...
    if (!apiHealth.ok) {
      ...
      for (let i = 1; i < results.length; i++) {
        results[i] = { ...results[i], status: "skipped", detail: "API gateway unreachable" }
      }
      setChecks([...results])
      setRunning(false)
      setLastRun(new Date())
    ...
    try {
      ...
      const wsResult = await new Promise<{ ok: boolean; latencyMs: number; detail: string | null }>((resolve) => {
        const start = performance.now()
        const timeout = setTimeout(() => { ws.close(); resolve({ ok: false, latencyMs: Math.round(performance.now() - start), detail: "timeout (5s)" }) }, 5000)
        ws.onopen = () => { clearTimeout(timeout); ws.close(); resolve({ ok: true, latencyMs: Math.round(performance.now() - start), detail: null }) }
        ws.onerror = () => { clearTimeout(timeout); resolve({ ok: false, latencyMs: Math.round(performance.now() - start), detail: "connection refused" }) }
      })
      results[wsIdx] = { ...results[wsIdx], status: wsResult.ok ? "ok" : "down", latencyMs: wsResult.latencyMs, detail: wsResult.detail }
    } catch {
      results[wsIdx] = { ...results[wsIdx], status: "down", latencyMs: null, detail: "WebSocket not available" }
    }
    setChecks([...results])
    ...
    <div className="min-h-screen bg-background p-6 max-w-4xl mx-auto">
      ...
      <div className={cn("flex items-center gap-3 mb-4 p-4 rounded-lg border", TIER_COLORS[detectedTier])}>
        ...
        <div className="ml-auto flex gap-2">
          <Badge variant="outline" className="text-[10px]">{env.toUpperCase()}</Badge>
          <Badge variant="outline" className="text-[10px]">{integration}</Badge>
          <Badge variant="outline" className={cn("text-[10px]", isMockMode ? "border-amber-500/30 text-amber-400" : "border-emerald-500/30 text-emerald-400")}>
      ...
      <div className="flex items-center gap-3 mb-6 p-3 rounded-lg border bg-card">
        ...
        <span className="font-semibold text-sm">
          {overallStatus === "healthy" && `All required services operational (${okCount}/${checks.length} checks passed)`}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {check.required && (
                        <Badge variant="outline" className="text-[9px] px-1 py-0 h-4">required</Badge>
                      )}
                    </div>
                    ...
                    {check.detail && (
                      <pre className={cn(
                        ...
                        check.status === "ok" ? "text-emerald-400" : check.status === "down" ? "text-red-300" : "text-amber-400"
                      )}>
                        {check.detail}
                      </pre>
                    )}
                  ...
                  {check.latencyMs !== null && (
                    <span className={cn(
                      ...
                      check.latencyMs < 100 ? "text-emerald-400" : check.latencyMs < 500 ? "text-amber-400" : "text-red-400"
                    )}>
                      {check.latencyMs}ms
                    </span>
                  )}
      ...
      <div className="mt-8 p-4 rounded-lg border border-border/50 bg-card/50 text-xs text-muted-foreground space-y-4">
        <div>
          ...
          <div className="space-y-1.5">
            <p><code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">bash scripts/dev-tiers.sh --tier 0</code> — UI-only. In-browser mock. No Python.</p>
            <p><code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">bash scripts/dev-tiers.sh --tier 1</code> — UI + API gateways. MockStateStore. Demo-ready.</p>
            <p><code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">bash scripts/dev-tiers.sh --tier 2</code> — UI + APIs + all service processes. Full engine parity.</p>
          </div>
        </div>
        <div>
          <p className="font-medium mb-2 text-foreground">Cloud Tiers (deployment-driven)</p>
          <div className="space-y-1.5">
            <p><strong>T3</strong> — UI in cloud, API local. <strong>T4</strong> — UI + API in cloud. <strong>T5</strong> — Full cloud (mock). <strong>T6</strong> — Full cloud (real).</p>
            <p>Cloud tiers driven by Deployment UI or <code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">deploy.py</code> scripts.</p>
          </div>
        </div>
        <div>
          <p className="font-medium mb-1 text-foreground">Other commands</p>
          <div className="space-y-1.5">
            <p><code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">bash scripts/dev-tiers.sh --stop</code> — Stop all processes.</p>
            <p><code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">bash scripts/dev-tiers.sh --status</code> — Show running processes and ports.</p>
            <p>Add <code className="bg-muted px-1.5 py-0.5 rounded text-[10px]">--real</code> to any tier for <code>CLOUD_MOCK_MODE=false</code>.</p>
          </div>
        </div>
      </div>
    </div>
