...
export default function ValidationPage() {
  ...
  const validationResults =
    (rawData as any)?.data?.comparison ?? (rawData as any)?.comparison ?? DEFAULT_VALIDATION_RESULTS;
  const regimePerformance: any[] =
    (rawData as any)?.data?.regimePerformance ?? (rawData as any)?.regimePerformance ?? DEFAULT_REGIME_PERFORMANCE;
  const walkForwardResults: any[] =
    (rawData as any)?.data?.walkForward ?? (rawData as any)?.walkForward ?? DEFAULT_WALK_FORWARD_RESULTS;
  const statisticalTests: any[] =
    (rawData as any)?.data?.statisticalTests ?? (rawData as any)?.statisticalTests ?? DEFAULT_STATISTICAL_TESTS;
  const featureImportance: any[] =
    (rawData as any)?.data?.featureImportance ?? (rawData as any)?.featureImportance ?? DEFAULT_FEATURE_IMPORTANCE;
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Challenger outperforms on 4/5 statistical tests with significantly better Sharpe ratio (+7.1%)
                    across all regimes
                  </p>
                </div>
              </div>
                        ...
                        <TableRow key={key}>
                          ...
                          <TableCell>
                            <Badge
                              ...
                              className={challengerWins ? "bg-[var(--status-warning)]" : ""}
                            >
                ...
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                  {regimePerformance.map((r) => (
                    <div key={r.regime} className="text-center p-3 rounded-lg bg-muted/30">
                      ...
                      >
                        Challenger {r.challengerSharpe > r.championSharpe ? "+" : ""}
                        {(((r.challengerSharpe - r.championSharpe) / r.championSharpe) * 100).toFixed(1)}%
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-4 p-4 bg-muted/30 rounded-lg">
                  <p className="text-sm">
                    <span className="font-semibold">Summary:</span> Challenger wins 5/5 windows with average Sharpe
                    improvement of +0.16 and accuracy improvement of +1.5%. Consistent outperformance suggests genuine
                    model improvement rather than overfitting.
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-muted/30 rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Return distributions are similar (KS test), suggesting performance improvement comes from better
                      timing rather than different risk profile.
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <TabsContent value="features" className="space-y-6">
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-4 p-4 bg-muted/30 rounded-lg">
                  <p className="text-sm">
                    <span className="font-semibold">Key Differences:</span> Challenger places more weight on{" "}
                    <code className="bg-muted px-1 rounded">oi_change_1h</code> (+4%) and{" "}
                    <code className="bg-muted px-1 rounded">liquidation_ratio</code> (+3%), while reducing reliance on{" "}
                    <code className="bg-muted px-1 rounded">whale_flow</code> (-3%). This suggests better capture of
                    short-term open interest dynamics.
                  </p>
                </div>
              </CardContent>
            </Card>
