...
export default function ExecutionTCAPage() {
  ...
  const EXECUTION_TIMELINE: Array<any> =
    (ordersData as any)?.executionTimeline ?? [];

  const SLIPPAGE_DISTRIBUTION: Array<any> =
    (ordersData as any)?.slippageDistribution ?? [];
  ...
  const totalCost = TCA_BREAKDOWN.reduce(
      ...
      <div className="platform-page-width p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Transaction Cost Analysis - understand and optimize execution
              costs
            </p>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold font-mono text-red-500">
                    +{selectedOrder.tca.slippage.toFixed(1)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Slippage (bps)
                  </div>
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold font-mono">
                    {selectedOrder.tca.marketImpact.toFixed(1)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Market Impact (bps)
                  </div>
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold font-mono">
                    {selectedOrder.tca.timingCost.toFixed(1)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Timing Cost (bps)
                  </div>
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold font-mono">
                    ${selectedOrder.tca.arrivalPrice.toLocaleString()}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Arrival Price
                  </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <Card className="col-span-2">
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Price evolution vs benchmarks during execution
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Performance vs standard execution benchmarks
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-4 gap-4">
              <div className="p-4 rounded-lg border">
                ...
                >
                  {selectedOrder.avgPrice > selectedOrder.tca.arrivalPrice
                    ? "+"
                    : ""}
                  {(selectedOrder.tca.arrivalPrice > 0
                    ? ((selectedOrder.avgPrice -
                        selectedOrder.tca.arrivalPrice) /
                        selectedOrder.tca.arrivalPrice) *
                      10000
                    : 0
                  ).toFixed(1)}{" "}
              ...
              <div className="p-4 rounded-lg border">
                ...
                >
                  {selectedOrder.avgPrice > selectedOrder.tca.vwap ? "+" : ""}
                  {(selectedOrder.tca.vwap > 0
                    ? ((selectedOrder.avgPrice - selectedOrder.tca.vwap) /
                        selectedOrder.tca.vwap) *
                      10000
                    : 0
                  ).toFixed(1)}{" "}
                  bps
              ...
              <div className="p-4 rounded-lg border">
                ...
                >
                  {selectedOrder.avgPrice > selectedOrder.tca.twap ? "+" : ""}
                  {(selectedOrder.tca.twap > 0
                    ? ((selectedOrder.avgPrice - selectedOrder.tca.twap) /
                        selectedOrder.tca.twap) *
                      10000
                    : 0
                  ).toFixed(1)}{" "}
                  bps
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Historical slippage across all executions
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Select an order to view detailed TCA
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {MOCK_RECENT_ORDERS.map((order) => (
                  ...
                  >
                    ...
                    >
                      {order.tca.slippage >= 0 ? "+" : ""}
                      {order.tca.slippage.toFixed(1)}
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {order.tca.totalCost.toFixed(1)} bps
                    </TableCell>
                    <TableCell>
