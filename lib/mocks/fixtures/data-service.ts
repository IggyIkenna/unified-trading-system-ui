// ─── Data Provision Service — Mock Data ──────────────────────────────────────
// All data is mock. Uses real venue/instrument names from the Odum system.
// This powers demo mode (always), client portal (scoped), admin view (all orgs).

import type {
  DataOrg,
  CatalogueEntry,
  InstrumentEntry,
  DataSubscription,
  DataQueryLog,
  ShardAvailability,
  DateFreshnessMap,
  FreshnessStatus,
  ETLPipelineConfig,
  ETLPipelineStatus,
  ETLStageStatus,
  ETLAlert,
  VenueCoverage,
  InstrumentCoverage,
  DataGap,
  ETLStage,
  ETLStatus,
  DataAssetGroup,
  DataFolder,
  DataType,
} from "@/lib/types/data-service";

// ─── Organisations ────────────────────────────────────────────────────────────

export const MOCK_ORGS: DataOrg[] = [
  {
    id: "odum",
    name: "Odum Research",
    mode: "admin",
    cloudPreference: "both",
    planTier: "enterprise",
    monthlySpend: 0,
  },
  {
    id: "arcadia-capital",
    name: "Arcadia Capital",
    mode: "client",
    cloudPreference: "aws",
    linkedCloudAccount: "aws:382910476291",
    planTier: "institutional",
    monthlySpend: 399900,
  },
  {
    id: "meridian-quant",
    name: "Meridian Quant",
    mode: "client",
    cloudPreference: "gcp",
    linkedCloudAccount: "gcp:meridian-quant-prod",
    planTier: "professional",
    monthlySpend: 149900,
  },
  {
    id: "demo",
    name: "Demo Org",
    mode: "demo",
    cloudPreference: "gcp",
    planTier: "institutional",
    monthlySpend: 0,
  },
];

export const ADMIN_ORG = MOCK_ORGS[0];
export const DEMO_ORG = MOCK_ORGS[3];

// ─── Instrument registry (subset — real symbols, real venues) ─────────────────

export const MOCK_INSTRUMENTS: InstrumentEntry[] = [
  // CeFi — Perpetuals
  {
    instrumentKey: "binance:perpetuals:BTCUSDT",
    venue: "binance",
    assetGroup: "cefi",
    folder: "perpetuals",
    symbol: "BTCUSDT",
    baseCurrency: "BTC",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5", "funding_rates", "liquidations", "open_interest"],
    availableFrom: "2019-09-08",
  },
  {
    instrumentKey: "binance:perpetuals:ETHUSDT",
    venue: "binance",
    assetGroup: "cefi",
    folder: "perpetuals",
    symbol: "ETHUSDT",
    baseCurrency: "ETH",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5", "funding_rates", "liquidations"],
    availableFrom: "2019-11-10",
  },
  {
    instrumentKey: "bybit:perpetuals:SOLUSDT",
    venue: "bybit",
    assetGroup: "cefi",
    folder: "perpetuals",
    symbol: "SOLUSDT",
    baseCurrency: "SOL",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades", "funding_rates"],
    availableFrom: "2021-06-01",
  },
  {
    instrumentKey: "bybit:perpetuals:BTCUSDT",
    venue: "bybit",
    assetGroup: "cefi",
    folder: "perpetuals",
    symbol: "BTCUSDT",
    baseCurrency: "BTC",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5", "funding_rates"],
    availableFrom: "2019-11-01",
  },
  // CeFi — Options
  {
    instrumentKey: "deribit:options:BTC-PERPETUAL",
    venue: "deribit",
    assetGroup: "cefi",
    folder: "options",
    symbol: "BTC-PERPETUAL",
    baseCurrency: "BTC",
    quoteCurrency: "USD",
    dataTypes: ["ohlcv", "trades", "greeks", "iv_surface", "book_snapshot_5"],
    availableFrom: "2018-01-01",
  },
  {
    instrumentKey: "deribit:options:ETH-PERPETUAL",
    venue: "deribit",
    assetGroup: "cefi",
    folder: "options",
    symbol: "ETH-PERPETUAL",
    baseCurrency: "ETH",
    quoteCurrency: "USD",
    dataTypes: ["ohlcv", "trades", "greeks", "iv_surface"],
    availableFrom: "2019-01-01",
  },
  // CeFi — Spot
  {
    instrumentKey: "coinbase:spot:BTC-USD",
    venue: "coinbase",
    assetGroup: "cefi",
    folder: "spot",
    symbol: "BTC-USD",
    baseCurrency: "BTC",
    quoteCurrency: "USD",
    dataTypes: ["ohlcv", "trades", "tick"],
    availableFrom: "2015-01-01",
  },
  {
    instrumentKey: "okx:spot:ETH-USDT",
    venue: "okx",
    assetGroup: "cefi",
    folder: "spot",
    symbol: "ETH-USDT",
    baseCurrency: "ETH",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades"],
    availableFrom: "2018-01-01",
  },
  // CeFi — Futures
  {
    instrumentKey: "binance:futures:BTCUSDT_250328",
    venue: "binance",
    assetGroup: "cefi",
    folder: "futures",
    symbol: "BTCUSDT_250328",
    baseCurrency: "BTC",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5"],
    availableFrom: "2023-01-01",
    availableTo: "2025-03-28",
  },
  {
    instrumentKey: "bybit:futures:BTCUSDT_250627",
    venue: "bybit",
    assetGroup: "cefi",
    folder: "futures",
    symbol: "BTCUSDT_250627",
    baseCurrency: "BTC",
    quoteCurrency: "USDT",
    dataTypes: ["ohlcv", "trades"],
    availableFrom: "2024-01-01",
  },
  // TradFi
  {
    instrumentKey: "databento:futures:ES.c.0",
    venue: "databento",
    assetGroup: "tradfi",
    folder: "futures",
    symbol: "ES.c.0",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5", "book_snapshot_25"],
    availableFrom: "2018-01-01",
  },
  {
    instrumentKey: "databento:futures:NQ.c.0",
    venue: "databento",
    assetGroup: "tradfi",
    folder: "futures",
    symbol: "NQ.c.0",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5"],
    availableFrom: "2018-01-01",
  },
  {
    instrumentKey: "databento:equity:AAPL",
    venue: "databento",
    assetGroup: "tradfi",
    folder: "equity",
    symbol: "AAPL",
    dataTypes: ["ohlcv", "trades", "book_snapshot_5"],
    availableFrom: "2012-01-01",
  },
  {
    instrumentKey: "ibkr:equity:AAPL",
    venue: "ibkr",
    assetGroup: "tradfi",
    folder: "equity",
    symbol: "AAPL",
    dataTypes: ["ohlcv", "trades"],
    availableFrom: "2010-01-01",
  },
  {
    instrumentKey: "ibkr:equity:MSFT",
    venue: "ibkr",
    assetGroup: "tradfi",
    folder: "equity",
    symbol: "MSFT",
    dataTypes: ["ohlcv", "trades"],
    availableFrom: "2010-01-01",
  },
  {
    instrumentKey: "fred:rates:DFF",
    venue: "fred",
    assetGroup: "tradfi",
    folder: "rates",
    symbol: "DFF",
    dataTypes: ["ohlcv"],
    availableFrom: "1954-07-01",
  },
  {
    instrumentKey: "fred:rates:US10Y",
    venue: "fred",
    assetGroup: "tradfi",
    folder: "rates",
    symbol: "US10Y",
    dataTypes: ["ohlcv"],
    availableFrom: "1962-01-01",
  },
  // DeFi
  {
    instrumentKey: "uniswap_v3:pool_state:USDC-ETH-0.05",
    venue: "uniswap_v3",
    assetGroup: "defi",
    folder: "pool_state",
    symbol: "USDC-ETH-0.05",
    dataTypes: ["pool_state", "swap_events", "price_feeds"],
    availableFrom: "2021-05-05",
  },
  {
    instrumentKey: "uniswap_v3:pool_state:WBTC-ETH-0.3",
    venue: "uniswap_v3",
    assetGroup: "defi",
    folder: "pool_state",
    symbol: "WBTC-ETH-0.3",
    dataTypes: ["pool_state", "swap_events", "price_feeds"],
    availableFrom: "2021-05-05",
  },
  {
    instrumentKey: "aave_v3:lending:USDC",
    venue: "aave_v3",
    assetGroup: "defi",
    folder: "lending",
    symbol: "USDC",
    dataTypes: ["lending_rates", "price_feeds"],
    availableFrom: "2023-01-01",
  },
  {
    instrumentKey: "aave_v3:lending:WETH",
    venue: "aave_v3",
    assetGroup: "defi",
    folder: "lending",
    symbol: "WETH",
    dataTypes: ["lending_rates", "price_feeds"],
    availableFrom: "2023-01-01",
  },
  {
    instrumentKey: "lido:staking:ETH",
    venue: "lido",
    assetGroup: "defi",
    folder: "staking",
    symbol: "stETH",
    dataTypes: ["staking_yields", "price_feeds"],
    availableFrom: "2020-12-18",
  },
  // Onchain Perps
  {
    instrumentKey: "hyperliquid:perpetuals:BTC-USD",
    venue: "hyperliquid",
    assetGroup: "onchain_perps",
    folder: "perpetuals",
    symbol: "BTC-USD",
    baseCurrency: "BTC",
    quoteCurrency: "USD",
    dataTypes: ["ohlcv", "trades", "funding_rates", "open_interest"],
    availableFrom: "2023-06-01",
  },
  {
    instrumentKey: "hyperliquid:perpetuals:ETH-USD",
    venue: "hyperliquid",
    assetGroup: "onchain_perps",
    folder: "perpetuals",
    symbol: "ETH-USD",
    dataTypes: ["ohlcv", "trades", "funding_rates"],
    availableFrom: "2023-06-01",
  },
  // Prediction Markets
  {
    instrumentKey: "polymarket:predictions:TRUMP_WIN_2024",
    venue: "polymarket",
    assetGroup: "prediction_market",
    folder: "predictions",
    symbol: "TRUMP_WIN_2024",
    dataTypes: ["odds", "settlement_prices"],
    availableFrom: "2024-01-01",
    availableTo: "2024-11-06",
  },
  {
    instrumentKey: "polymarket:odds:BTC_100K_2025",
    venue: "polymarket",
    assetGroup: "prediction_market",
    folder: "odds",
    symbol: "BTC_100K_2025",
    dataTypes: ["odds", "settlement_prices"],
    availableFrom: "2024-10-01",
  },
  {
    instrumentKey: "kalshi:predictions:FED_RATE_JUL2025",
    venue: "kalshi",
    assetGroup: "prediction_market",
    folder: "predictions",
    symbol: "FED_RATE_JUL2025",
    dataTypes: ["odds", "settlement_prices"],
    availableFrom: "2024-12-01",
  },
  {
    instrumentKey: "kalshi:odds:US_RECESSION_2025",
    venue: "kalshi",
    assetGroup: "prediction_market",
    folder: "odds",
    symbol: "US_RECESSION_2025",
    dataTypes: ["odds", "settlement_prices"],
    availableFrom: "2025-01-01",
  },
  // Sports
  {
    instrumentKey: "api_football:fixtures:EPL_2025",
    venue: "api_football",
    assetGroup: "sports",
    folder: "fixtures",
    symbol: "EPL_2025",
    dataTypes: ["odds", "game_events"],
    availableFrom: "2023-08-01",
  },
  {
    instrumentKey: "api_football:fixtures:UCL_2025",
    venue: "api_football",
    assetGroup: "sports",
    folder: "fixtures",
    symbol: "UCL_2025",
    dataTypes: ["odds", "game_events"],
    availableFrom: "2023-09-01",
  },
  {
    instrumentKey: "api_football:odds:EPL_2025_MD32",
    venue: "api_football",
    assetGroup: "sports",
    folder: "odds",
    symbol: "EPL_2025_MD32",
    dataTypes: ["odds"],
    availableFrom: "2025-04-01",
  },
  {
    instrumentKey: "footystats:fixtures:EPL_2025",
    venue: "footystats",
    assetGroup: "sports",
    folder: "fixtures",
    symbol: "EPL_2025",
    dataTypes: ["odds", "game_events"],
    availableFrom: "2022-01-01",
  },
  {
    instrumentKey: "footystats:fixtures:LA_LIGA_2025",
    venue: "footystats",
    assetGroup: "sports",
    folder: "fixtures",
    symbol: "LA_LIGA_2025",
    dataTypes: ["odds", "game_events"],
    availableFrom: "2022-01-01",
  },
  {
    instrumentKey: "footystats:game_events:EPL_2025_GW34",
    venue: "footystats",
    assetGroup: "sports",
    folder: "game_events",
    symbol: "EPL_2025_GW34",
    dataTypes: ["game_events"],
    availableFrom: "2025-04-01",
  },
];

// ─── Catalogue entries (what we have in storage) ──────────────────────────────

// Deterministic seeded random to avoid hydration mismatches
function seededRandom(seed: number): number {
  const x = Math.sin(seed * 9999) * 10000;
  return x - Math.floor(x);
}

function makeFreshnessMap(dayCount: number, completeness: number): DateFreshnessMap {
  const map: DateFreshnessMap = {};
  // Generate from today backwards so dates match the heatmap grid
  const today = new Date();
  const baseSeed = 42 + completeness; // deterministic seed
  for (let i = 0; i < dayCount; i++) {
    const d = new Date(today);
    d.setDate(d.getDate() - (dayCount - 1 - i));
    const key = d.toISOString().split("T")[0];
    // Deterministic random based on day index
    const rand = seededRandom(baseSeed + i);
    let status: FreshnessStatus;
    if (rand < completeness / 100) {
      status = "complete";
    } else if (rand < (completeness + 3) / 100) {
      status = "partial";
    } else {
      status = "missing";
    }
    map[key] = status;
  }
  return map;
}

export const MOCK_CATALOGUE: CatalogueEntry[] = [
  {
    instrument: MOCK_INSTRUMENTS[0], // binance:perpetuals:BTCUSDT
    cloud: "both",
    totalDates: 2000,
    datesWithData: 1980,
    freshnessPct: 99,
    lastUpdated: "2026-03-18",
    sizeGb: 847,
    gcpCompleteness: 99,
    awsCompleteness: 97,
  },
  {
    instrument: MOCK_INSTRUMENTS[1], // binance:perpetuals:ETHUSDT
    cloud: "both",
    totalDates: 1900,
    datesWithData: 1881,
    freshnessPct: 99,
    lastUpdated: "2026-03-18",
    sizeGb: 612,
    gcpCompleteness: 99,
    awsCompleteness: 96,
  },
  {
    instrument: MOCK_INSTRUMENTS[2], // bybit:perpetuals:SOLUSDT
    cloud: "gcp",
    totalDates: 1000,
    datesWithData: 960,
    freshnessPct: 96,
    lastUpdated: "2026-03-17",
    sizeGb: 124,
    gcpCompleteness: 96,
    awsCompleteness: 0,
  },
  {
    instrument: MOCK_INSTRUMENTS[4], // deribit:options:BTC-PERPETUAL
    cloud: "both",
    totalDates: 2920,
    datesWithData: 2870,
    freshnessPct: 98,
    lastUpdated: "2026-03-18",
    sizeGb: 2140,
    gcpCompleteness: 98,
    awsCompleteness: 95,
  },
  {
    instrument: MOCK_INSTRUMENTS[8], // databento:futures:ES.c.0
    cloud: "gcp",
    totalDates: 2000,
    datesWithData: 1950,
    freshnessPct: 97,
    lastUpdated: "2026-03-18",
    sizeGb: 1820,
    gcpCompleteness: 97,
    awsCompleteness: 0,
  },
  {
    instrument: MOCK_INSTRUMENTS[12], // uniswap_v3:pool_state
    cloud: "both",
    totalDates: 1100,
    datesWithData: 1078,
    freshnessPct: 98,
    lastUpdated: "2026-03-18",
    sizeGb: 334,
    gcpCompleteness: 98,
    awsCompleteness: 92,
  },
  {
    instrument: MOCK_INSTRUMENTS[15], // hyperliquid:perpetuals:BTC-USD
    cloud: "gcp",
    totalDates: 640,
    datesWithData: 635,
    freshnessPct: 99,
    lastUpdated: "2026-03-18",
    sizeGb: 89,
    gcpCompleteness: 99,
    awsCompleteness: 0,
  },
  // Prediction Markets
  {
    instrument: MOCK_INSTRUMENTS[17], // polymarket:predictions:TRUMP_WIN_2024
    cloud: "gcp",
    totalDates: 310,
    datesWithData: 308,
    freshnessPct: 99,
    lastUpdated: "2026-03-18",
    sizeGb: 12,
    gcpCompleteness: 99,
    awsCompleteness: 0,
  },
  {
    instrument: MOCK_INSTRUMENTS[18], // kalshi:predictions:FED_RATE_JUL2025
    cloud: "gcp",
    totalDates: 120,
    datesWithData: 118,
    freshnessPct: 98,
    lastUpdated: "2026-03-18",
    sizeGb: 8,
    gcpCompleteness: 98,
    awsCompleteness: 0,
  },
  // Sports
  {
    instrument: MOCK_INSTRUMENTS[19], // api_football:fixtures:EPL_2025
    cloud: "gcp",
    totalDates: 950,
    datesWithData: 938,
    freshnessPct: 98,
    lastUpdated: "2026-03-18",
    sizeGb: 45,
    gcpCompleteness: 98,
    awsCompleteness: 0,
  },
  {
    instrument: MOCK_INSTRUMENTS[20], // api_football:fixtures:UCL_2025
    cloud: "gcp",
    totalDates: 900,
    datesWithData: 882,
    freshnessPct: 98,
    lastUpdated: "2026-03-18",
    sizeGb: 38,
    gcpCompleteness: 98,
    awsCompleteness: 0,
  },
  {
    instrument: MOCK_INSTRUMENTS[21], // footystats:fixtures:EPL_2025
    cloud: "gcp",
    totalDates: 1500,
    datesWithData: 1470,
    freshnessPct: 98,
    lastUpdated: "2026-03-17",
    sizeGb: 62,
    gcpCompleteness: 98,
    awsCompleteness: 0,
  },
  {
    instrument: MOCK_INSTRUMENTS[22], // footystats:fixtures:LA_LIGA_2025
    cloud: "gcp",
    totalDates: 1500,
    datesWithData: 1455,
    freshnessPct: 97,
    lastUpdated: "2026-03-17",
    sizeGb: 58,
    gcpCompleteness: 97,
    awsCompleteness: 0,
  },
];

// ─── Subscriptions ────────────────────────────────────────────────────────────

export const MOCK_SUBSCRIPTIONS: DataSubscription[] = [
  {
    id: "sub-arc-001",
    orgId: "arcadia-capital",
    label: "CeFi Core Bundle",
    shardFilters: {
      assetGroups: ["cefi"],
      venues: ["binance", "bybit", "deribit"],
      folders: ["perpetuals", "options"],
      dataTypes: ["ohlcv", "trades", "funding_rates", "book_snapshot_5"],
      dateFrom: "2020-01-01",
      dateTo: "present",
    },
    accessMode: "in_system",
    cloudTarget: "aws",
    status: "active",
    createdAt: "2025-01-15",
    expiresAt: "2027-01-15",
    monthlyCostCents: 299900,
    gbUsedThisMonth: 847,
    gbLimitThisMonth: 2000,
  },
  {
    id: "sub-arc-002",
    orgId: "arcadia-capital",
    label: "TradFi Futures",
    shardFilters: {
      assetGroups: ["tradfi"],
      venues: ["databento"],
      folders: ["futures"],
      dataTypes: ["ohlcv", "trades", "book_snapshot_5"],
      dateFrom: "2018-01-01",
      dateTo: "present",
    },
    accessMode: "download",
    cloudTarget: "aws",
    status: "active",
    createdAt: "2025-03-01",
    expiresAt: "2026-03-01",
    monthlyCostCents: 99900,
    gbUsedThisMonth: 234,
    gbLimitThisMonth: 500,
  },
  {
    id: "sub-mer-001",
    orgId: "meridian-quant",
    label: "DeFi Research Pack",
    shardFilters: {
      assetGroups: ["defi", "onchain_perps"],
      venues: ["uniswap_v3", "aave_v3", "hyperliquid"],
      folders: ["pool_state", "lending", "perpetuals"],
      dataTypes: ["pool_state", "lending_rates", "ohlcv", "funding_rates"],
      dateFrom: "2022-01-01",
      dateTo: "present",
    },
    accessMode: "in_system",
    cloudTarget: "gcp",
    status: "active",
    createdAt: "2025-06-01",
    monthlyCostCents: 149900,
    gbUsedThisMonth: 312,
    gbLimitThisMonth: 500,
  },
  // Demo org subscription
  {
    id: "sub-demo-001",
    orgId: "demo",
    label: "Demo Bundle (Mock)",
    shardFilters: {
      assetGroups: ["cefi", "defi"],
      venues: ["binance", "uniswap_v3"],
      folders: ["perpetuals", "pool_state"],
      dataTypes: ["ohlcv", "trades"],
      dateFrom: "2023-01-01",
      dateTo: "present",
    },
    accessMode: "in_system",
    cloudTarget: "gcp",
    status: "trial",
    createdAt: "2026-03-01",
    expiresAt: "2026-04-01",
    monthlyCostCents: 0,
    gbUsedThisMonth: 14,
    gbLimitThisMonth: 50,
  },
];

// ─── Query log ────────────────────────────────────────────────────────────────

export const MOCK_QUERY_LOG: DataQueryLog[] = [
  {
    id: "qry-001",
    orgId: "arcadia-capital",
    instrumentKey: "binance:perpetuals:BTCUSDT",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    folder: "perpetuals",
    dataType: "ohlcv",
    dateFrom: "2024-01-01",
    dateTo: "2024-12-31",
    accessMode: "in_system",
    cloud: "aws",
    queriedAt: "2026-03-18T14:23:11Z",
    rowsReturned: 525600,
    gbTransferred: 1.2,
    costCents: 60,
    status: "completed",
  },
  {
    id: "qry-002",
    orgId: "arcadia-capital",
    instrumentKey: "databento:futures:ES.c.0",
    assetGroup: "tradfi",
    venue: "DATABENTO",
    folder: "futures",
    dataType: "book_snapshot_5",
    dateFrom: "2025-01-01",
    dateTo: "2026-01-01",
    accessMode: "download",
    cloud: "aws",
    queriedAt: "2026-03-18T11:05:44Z",
    rowsReturned: 18000000,
    gbTransferred: 42.6,
    costCents: 10650,
    status: "completed",
  },
  {
    id: "qry-003",
    orgId: "arcadia-capital",
    instrumentKey: "bybit:perpetuals:SOLUSDT",
    assetGroup: "cefi",
    venue: "BYBIT",
    folder: "perpetuals",
    dataType: "trades",
    dateFrom: "2023-01-01",
    dateTo: "2026-01-01",
    accessMode: "in_system",
    cloud: "aws",
    queriedAt: "2026-03-17T09:30:00Z",
    gbTransferred: 0,
    costCents: 0,
    status: "paywall_blocked",
  },
  {
    id: "qry-004",
    orgId: "meridian-quant",
    instrumentKey: "uniswap_v3:pool_state:USDC-ETH-0.05",
    assetGroup: "defi",
    venue: "UNISWAPV3-ETHEREUM",
    folder: "pool_state",
    dataType: "pool_state",
    dateFrom: "2024-06-01",
    dateTo: "2025-06-01",
    accessMode: "in_system",
    cloud: "gcp",
    queriedAt: "2026-03-18T08:15:22Z",
    rowsReturned: 4380000,
    gbTransferred: 8.4,
    costCents: 420,
    status: "completed",
  },
];

// ─── Shard availability (mock response for DataStatusTab equivalent) ──────────

// Generate shard availability with stats matching the 90-day heatmap
function makeShardAvailability(
  assetGroup: DataAssetGroup,
  venue: string,
  folder: DataFolder,
  dataType: DataType,
  completeness: number,
  cloud: { gcp: number; aws: number },
): ShardAvailability {
  const byDate = makeFreshnessMap(90, completeness);
  const dates = Object.values(byDate);
  const complete = dates.filter((s) => s === "complete").length;
  const partial = dates.filter((s) => s === "partial").length;
  const missing = dates.filter((s) => s === "missing").length;
  const total = complete + partial + missing;
  return {
    assetGroup,
    venue,
    folder,
    dataType,
    dateRange: { start: "last 90 days", end: "today" },
    datesChecked: total,
    datesFound: complete + partial,
    datesMissing: missing,
    completionPct: total > 0 ? Math.round((complete / total) * 100 * 10) / 10 : 0,
    lastFreshnessDate: new Date().toISOString().split("T")[0],
    gcpCompletionPct: cloud.gcp,
    awsCompletionPct: cloud.aws,
    byDate,
  };
}

export const MOCK_SHARD_AVAILABILITY: ShardAvailability[] = [
  makeShardAvailability("cefi", "binance", "perpetuals", "ohlcv", 96, {
    gcp: 96,
    aws: 0,
  }),
  makeShardAvailability("tradfi", "databento", "futures", "trades", 93, {
    gcp: 0,
    aws: 93,
  }),
  makeShardAvailability("defi", "aave_v3", "lending", "pool_state", 91, {
    gcp: 91,
    aws: 88,
  }),
];

// ─── Admin summary stats ──────────────────────────────────────────────────────

export const ADMIN_SUMMARY = {
  totalOrgs: 12,
  activeSubscriptions: 34,
  totalGbStored: 47200,
  gcpGb: 47200,
  awsGb: 31400,
  monthlyRevenueUsd: 68420,
  avgFreshnessPct: 97.4,
  totalInstruments: 18743,
  assetGroupCounts: {
    cefi: 4200,
    tradfi: 9800,
    defi: 3100,
    onchain_perps: 847,
    prediction_market: 796,
    sports: 512,
  },
};

// ─── ETL Pipeline Mock Data (Internal) ────────────────────────────────────────

const ETL_STAGES: ETLStage[] = ["ingest", "validate", "normalise", "enrich", "store_gcp", "store_aws", "index"];

// Deterministic stage status based on stage index and health baseline
function mockStageStatus(stage: ETLStage, baseHealth: number, stageIndex: number = 0): ETLStageStatus {
  // Use deterministic values based on stage index
  const seed = stageIndex * 7 + baseHealth * 100;
  const healthThreshold = seededRandom(seed);
  const isHealthy = healthThreshold < baseHealth;
  const isDegraded = !isHealthy && seededRandom(seed + 1) < 0.7;

  // Fixed timestamps for consistency
  const baseTime = new Date("2026-03-19T00:00:00Z").getTime();
  const offset = Math.floor(seededRandom(seed + 2) * 3600000);

  return {
    stage,
    status: isHealthy ? "healthy" : isDegraded ? "degraded" : "failed",
    lastRun: new Date(baseTime - offset).toISOString(),
    lastSuccess: isHealthy ? new Date(baseTime - offset).toISOString() : undefined,
    lastError: !isHealthy ? "Connection timeout to upstream" : undefined,
    recordsProcessed: Math.floor(seededRandom(seed + 3) * 1000000),
    latencyMs: Math.floor(seededRandom(seed + 4) * 500) + 50,
    errorCount24h: isHealthy ? 0 : Math.floor(seededRandom(seed + 5) * 10) + 1,
  };
}

export const MOCK_ETL_PIPELINES: ETLPipelineStatus[] = [
  // CeFi pipelines
  {
    config: {
      id: "etl-binance-perps",
      assetGroup: "cefi",
      venue: "BINANCE-SPOT",
      folder: "perpetuals",
      dataTypes: ["ohlcv", "trades", "book_snapshot_5", "funding_rates", "liquidations"],
      schedule: "realtime",
      enabled: true,
      priority: "critical",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.95, i)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    nextScheduledRun: "2026-03-19T00:01:00Z",
    dataLagMinutes: 1,
    alerts: [],
  },
  {
    config: {
      id: "etl-binance-spot",
      assetGroup: "cefi",
      venue: "BINANCE-SPOT",
      folder: "spot",
      dataTypes: ["ohlcv", "trades"],
      schedule: "1m",
      enabled: true,
      priority: "high",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.92, i + 10)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 2,
    alerts: [],
  },
  {
    config: {
      id: "etl-bybit-perps",
      assetGroup: "cefi",
      venue: "BYBIT",
      folder: "perpetuals",
      dataTypes: ["ohlcv", "trades", "funding_rates"],
      schedule: "realtime",
      enabled: true,
      priority: "critical",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.88, i + 20)),
    overallStatus: "degraded",
    lastFullSync: "2026-03-18T23:45:00Z",
    dataLagMinutes: 15,
    alerts: [
      {
        id: "alert-1",
        pipelineId: "etl-bybit-perps",
        stage: "ingest",
        severity: "warning",
        message: "Elevated latency on WebSocket connection",
        timestamp: "2026-03-19T00:10:00Z",
        acknowledged: false,
      },
    ],
  },
  {
    config: {
      id: "etl-deribit-options",
      assetGroup: "cefi",
      venue: "DERIBIT",
      folder: "options",
      dataTypes: ["ohlcv", "trades", "greeks", "iv_surface"],
      schedule: "1m",
      enabled: true,
      priority: "high",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.94, i + 30)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 3,
    alerts: [],
  },
  {
    config: {
      id: "etl-coinbase-spot",
      assetGroup: "cefi",
      venue: "COINBASE-SPOT",
      folder: "spot",
      dataTypes: ["ohlcv", "trades", "tick"],
      schedule: "realtime",
      enabled: true,
      priority: "high",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.96, i + 40)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 1,
    alerts: [],
  },
  // TradFi pipelines
  {
    config: {
      id: "etl-databento-futures",
      assetGroup: "tradfi",
      venue: "DATABENTO",
      folder: "futures",
      dataTypes: ["ohlcv", "trades", "book_snapshot_5", "book_snapshot_25"],
      schedule: "realtime",
      enabled: true,
      priority: "critical",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.97, i + 50)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 0,
    alerts: [],
  },
  {
    config: {
      id: "etl-ibkr-equity",
      assetGroup: "tradfi",
      venue: "IBKR",
      folder: "equity",
      dataTypes: ["ohlcv", "trades"],
      schedule: "15m",
      enabled: true,
      priority: "normal",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.9, i + 60)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-18T21:00:00Z",
    dataLagMinutes: 180, // market closed
    alerts: [],
  },
  {
    config: {
      id: "etl-fred-rates",
      assetGroup: "tradfi",
      venue: "FRED",
      folder: "rates",
      dataTypes: ["ohlcv"],
      schedule: "daily",
      enabled: true,
      priority: "low",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.99, i + 70)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-18T06:00:00Z",
    dataLagMinutes: 1080,
    alerts: [],
  },
  // DeFi pipelines
  {
    config: {
      id: "etl-uniswap-v3",
      assetGroup: "defi",
      venue: "UNISWAPV3-ETHEREUM",
      folder: "pool_state",
      dataTypes: ["pool_state", "swap_events", "price_feeds"],
      schedule: "1m",
      enabled: true,
      priority: "high",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.91, i + 80)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 2,
    alerts: [],
  },
  {
    config: {
      id: "etl-aave-v3",
      assetGroup: "defi",
      venue: "AAVEV3-ETHEREUM",
      folder: "lending",
      dataTypes: ["lending_rates", "price_feeds"],
      schedule: "5m",
      enabled: true,
      priority: "normal",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.93, i + 90)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 5,
    alerts: [],
  },
  {
    config: {
      id: "etl-lido-staking",
      assetGroup: "defi",
      venue: "LIDO-ETHEREUM",
      folder: "staking",
      dataTypes: ["staking_yields", "price_feeds"],
      schedule: "1h",
      enabled: true,
      priority: "normal",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.95, i + 100)),
    overallStatus: "healthy",
    lastFullSync: "2026-03-19T00:00:00Z",
    dataLagMinutes: 30,
    alerts: [],
  },
  // Onchain Perps
  {
    config: {
      id: "etl-hyperliquid",
      assetGroup: "onchain_perps",
      venue: "HYPERLIQUID",
      folder: "perpetuals",
      dataTypes: ["ohlcv", "trades", "funding_rates", "open_interest"],
      schedule: "realtime",
      enabled: true,
      priority: "high",
    },
    stages: ETL_STAGES.map((s, i) => mockStageStatus(s, 0.89, i + 110)),
    overallStatus: "degraded",
    lastFullSync: "2026-03-18T23:30:00Z",
    dataLagMinutes: 30,
    alerts: [
      {
        id: "alert-2",
        pipelineId: "etl-hyperliquid",
        stage: "normalise",
        severity: "warning",
        message: "New instrument detected, schema update required",
        timestamp: "2026-03-19T00:05:00Z",
        acknowledged: false,
      },
    ],
  },
];

export const MOCK_VENUE_COVERAGE: VenueCoverage[] = [
  {
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    label: "Binance",
    dataSource: "exchange_api",
    status: "active",
    folders: ["perpetuals", "spot", "futures"],
    instrumentCount: 1247,
    oldestData: "2019-09-08",
    newestData: "2026-03-19",
    dailyVolume: 85000000,
    etlPipelineId: "etl-binance-perps",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "BYBIT",
    assetGroup: "cefi",
    label: "Bybit",
    dataSource: "exchange_api",
    status: "active",
    folders: ["perpetuals", "spot"],
    instrumentCount: 634,
    oldestData: "2019-11-01",
    newestData: "2026-03-19",
    dailyVolume: 42000000,
    etlPipelineId: "etl-bybit-perps",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "degraded",
  },
  {
    venue: "DERIBIT",
    assetGroup: "cefi",
    label: "Deribit",
    dataSource: "exchange_api",
    status: "active",
    folders: ["options", "perpetuals"],
    instrumentCount: 2341,
    oldestData: "2018-01-01",
    newestData: "2026-03-19",
    dailyVolume: 12000000,
    etlPipelineId: "etl-deribit-options",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "COINBASE-SPOT",
    assetGroup: "cefi",
    label: "Coinbase",
    dataSource: "exchange_api",
    status: "active",
    folders: ["spot"],
    instrumentCount: 523,
    oldestData: "2015-01-01",
    newestData: "2026-03-19",
    dailyVolume: 18000000,
    etlPipelineId: "etl-coinbase-spot",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "OKX-SPOT",
    assetGroup: "cefi",
    label: "OKX",
    dataSource: "exchange_api",
    status: "active",
    folders: ["perpetuals", "spot", "options"],
    instrumentCount: 891,
    oldestData: "2018-01-01",
    newestData: "2026-03-19",
    dailyVolume: 35000000,
    etlPipelineId: "etl-okx",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "DATABENTO",
    assetGroup: "tradfi",
    label: "Databento",
    dataSource: "vendor",
    status: "active",
    folders: ["futures", "equity", "options"],
    instrumentCount: 8742,
    oldestData: "2018-01-01",
    newestData: "2026-03-19",
    dailyVolume: 250000000,
    etlPipelineId: "etl-databento-futures",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "IBKR",
    assetGroup: "tradfi",
    label: "Interactive Brokers",
    dataSource: "vendor",
    status: "active",
    folders: ["equity", "options"],
    instrumentCount: 12500,
    oldestData: "2010-01-01",
    newestData: "2026-03-18",
    dailyVolume: 45000000,
    etlPipelineId: "etl-ibkr-equity",
    lastHealthCheck: "2026-03-18T21:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "FRED",
    assetGroup: "tradfi",
    label: "FRED",
    dataSource: "vendor",
    status: "active",
    folders: ["rates"],
    instrumentCount: 847,
    oldestData: "1954-07-01",
    newestData: "2026-03-18",
    dailyVolume: 847,
    etlPipelineId: "etl-fred-rates",
    lastHealthCheck: "2026-03-18T06:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "UNISWAPV3-ETHEREUM",
    assetGroup: "defi",
    label: "Uniswap V3",
    dataSource: "onchain",
    status: "active",
    folders: ["pool_state", "swaps"],
    instrumentCount: 4521,
    oldestData: "2021-05-05",
    newestData: "2026-03-19",
    dailyVolume: 2800000,
    etlPipelineId: "etl-uniswap-v3",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "AAVEV3-ETHEREUM",
    assetGroup: "defi",
    label: "Aave V3",
    dataSource: "onchain",
    status: "active",
    folders: ["lending"],
    instrumentCount: 127,
    oldestData: "2023-01-01",
    newestData: "2026-03-19",
    dailyVolume: 450000,
    etlPipelineId: "etl-aave-v3",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "LIDO-ETHEREUM",
    assetGroup: "defi",
    label: "Lido",
    dataSource: "onchain",
    status: "active",
    folders: ["staking"],
    instrumentCount: 12,
    oldestData: "2020-12-18",
    newestData: "2026-03-19",
    dailyVolume: 24000,
    etlPipelineId: "etl-lido-staking",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "healthy",
  },
  {
    venue: "HYPERLIQUID",
    assetGroup: "onchain_perps",
    label: "Hyperliquid",
    dataSource: "onchain",
    status: "active",
    folders: ["perpetuals"],
    instrumentCount: 147,
    oldestData: "2023-06-01",
    newestData: "2026-03-19",
    dailyVolume: 8500000,
    etlPipelineId: "etl-hyperliquid",
    lastHealthCheck: "2026-03-19T00:00:00Z",
    healthStatus: "degraded",
  },
];

export const MOCK_DATA_GAPS: DataGap[] = [
  {
    id: "gap-1",
    assetGroup: "cefi",
    venue: "BYBIT",
    folder: "perpetuals",
    dataType: "trades",
    instrument: "SOLUSDT",
    gapStart: "2026-03-15",
    gapEnd: "2026-03-15",
    daysAffected: 1,
    severity: "medium",
    cause: "Exchange API maintenance",
    status: "backfilling",
    createdAt: "2026-03-15T12:00:00Z",
  },
  {
    id: "gap-2",
    assetGroup: "defi",
    venue: "UNISWAPV3-ETHEREUM",
    folder: "pool_state",
    dataType: "swap_events",
    gapStart: "2026-03-10",
    gapEnd: "2026-03-10",
    daysAffected: 1,
    severity: "low",
    cause: "RPC node timeout",
    status: "resolved",
    createdAt: "2026-03-10T08:00:00Z",
    resolvedAt: "2026-03-11T02:00:00Z",
  },
  {
    id: "gap-3",
    assetGroup: "tradfi",
    venue: "DATABENTO",
    folder: "futures",
    dataType: "book_snapshot_25",
    gapStart: "2026-02-28",
    gapEnd: "2026-02-28",
    daysAffected: 1,
    severity: "high",
    cause: "Vendor outage",
    status: "open",
    createdAt: "2026-02-28T14:00:00Z",
  },
  {
    id: "gap-4",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    folder: "perpetuals",
    dataType: "liquidations",
    gapStart: "2026-01-15",
    gapEnd: "2026-01-16",
    daysAffected: 2,
    severity: "medium",
    cause: "Schema change not detected",
    status: "resolved",
    createdAt: "2026-01-16T00:00:00Z",
    resolvedAt: "2026-01-17T06:00:00Z",
  },
];

export const ETL_SUMMARY = {
  totalPipelines: 12,
  healthyPipelines: 10,
  degradedPipelines: 2,
  failedPipelines: 0,
  totalVenues: 12,
  totalInstruments: 32532,
  totalDataPointsToday: 847000000,
  avgLatencyMs: 127,
  openGaps: 2,
  backfillingGaps: 1,
  alertsUnacknowledged: 2,
};

// ─── Venue display metadata ───────────────────────────────────────────────────

export const VENUE_DISPLAY: Record<string, { label: string; logo?: string; dataHistory: string }> = {
  binance: { label: "Binance", dataHistory: "Sep 2019" },
  bybit: { label: "Bybit", dataHistory: "Nov 2019" },
  coinbase: { label: "Coinbase", dataHistory: "Jan 2015" },
  okx: { label: "OKX", dataHistory: "Jan 2018" },
  deribit: { label: "Deribit", dataHistory: "Jan 2018" },
  upbit: { label: "Upbit", dataHistory: "Sep 2017" },
  databento: { label: "Databento", dataHistory: "Jan 2018" },
  tardis: { label: "Tardis", dataHistory: "Jan 2019" },
  yahoo_finance: { label: "Yahoo Finance", dataHistory: "Jan 2000" },
  fred: { label: "FRED", dataHistory: "1954" },
  ibkr: { label: "IBKR", dataHistory: "Jan 2010" },
  ecb: { label: "ECB", dataHistory: "Jan 1999" },
  uniswap_v3: { label: "Uniswap V3", dataHistory: "May 2021" },
  uniswap_v4: { label: "Uniswap V4", dataHistory: "Jan 2025" },
  aave_v3: { label: "Aave V3", dataHistory: "Jan 2023" },
  curve: { label: "Curve", dataHistory: "Aug 2020" },
  lido: { label: "Lido", dataHistory: "Dec 2020" },
  morpho: { label: "Morpho", dataHistory: "Jun 2022" },
  ethena: { label: "Ethena", dataHistory: "Feb 2024" },
  hyperliquid: { label: "Hyperliquid", dataHistory: "Jun 2023" },
  polymarket: { label: "Polymarket", dataHistory: "Jan 2024" },
  kalshi: { label: "Kalshi", dataHistory: "Dec 2024" },
  api_football: { label: "API-Football", dataHistory: "Aug 2023" },
  footystats: { label: "FootyStats", dataHistory: "Jan 2022" },
};

// ─── Pipeline Stage Summaries ─────────────────────────────────────────────────

export const MOCK_PIPELINE_STAGES: import("@/lib/types/data-service").PipelineStageSummary[] = [
  {
    stage: "instruments",
    label: "Instruments",
    totalShards: 8_450,
    completedShards: 8_112,
    inProgressShards: 14,
    failedShards: 3,
    completionPct: 96.0,
    lastUpdated: "2026-03-23T08:14:00Z",
    byAssetGroup: [
      {
        assetGroup: "cefi",
        totalShards: 2_400,
        completedShards: 2_400,
        completionPct: 100,
      },
      {
        assetGroup: "tradfi",
        totalShards: 1_560,
        completedShards: 1_520,
        completionPct: 97.4,
      },
      {
        assetGroup: "defi",
        totalShards: 2_000,
        completedShards: 1_870,
        completionPct: 93.5,
      },
      {
        assetGroup: "onchain_perps",
        totalShards: 730,
        completedShards: 730,
        completionPct: 100,
      },
      {
        assetGroup: "prediction_market",
        totalShards: 570,
        completedShards: 412,
        completionPct: 72.3,
      },
      {
        assetGroup: "sports",
        totalShards: 1_190,
        completedShards: 1_180,
        completionPct: 99.2,
      },
    ],
  },
  {
    stage: "raw",
    label: "Raw Data",
    totalShards: 142_000,
    completedShards: 118_500,
    inProgressShards: 84,
    failedShards: 12,
    completionPct: 83.5,
    lastUpdated: "2026-03-23T08:02:00Z",
    byAssetGroup: [
      {
        assetGroup: "cefi",
        totalShards: 52_000,
        completedShards: 48_800,
        completionPct: 93.8,
      },
      {
        assetGroup: "tradfi",
        totalShards: 34_000,
        completedShards: 30_100,
        completionPct: 88.5,
      },
      {
        assetGroup: "defi",
        totalShards: 28_000,
        completedShards: 21_200,
        completionPct: 75.7,
      },
      {
        assetGroup: "onchain_perps",
        totalShards: 9_800,
        completedShards: 9_600,
        completionPct: 98.0,
      },
      {
        assetGroup: "prediction_market",
        totalShards: 6_200,
        completedShards: 2_400,
        completionPct: 38.7,
      },
      {
        assetGroup: "sports",
        totalShards: 12_000,
        completedShards: 6_400,
        completionPct: 53.3,
      },
    ],
  },
  {
    stage: "processing",
    label: "Processed Data",
    totalShards: 118_500,
    completedShards: 94_200,
    inProgressShards: 42,
    failedShards: 8,
    completionPct: 79.5,
    lastUpdated: "2026-03-23T07:45:00Z",
    byAssetGroup: [
      {
        assetGroup: "cefi",
        totalShards: 48_800,
        completedShards: 46_000,
        completionPct: 94.3,
      },
      {
        assetGroup: "tradfi",
        totalShards: 30_100,
        completedShards: 27_200,
        completionPct: 90.4,
      },
      {
        assetGroup: "defi",
        totalShards: 21_200,
        completedShards: 13_800,
        completionPct: 65.1,
      },
      {
        assetGroup: "onchain_perps",
        totalShards: 9_600,
        completedShards: 9_100,
        completionPct: 94.8,
      },
      {
        assetGroup: "prediction_market",
        totalShards: 2_400,
        completedShards: 600,
        completionPct: 25.0,
      },
      {
        assetGroup: "sports",
        totalShards: 6_400,
        completedShards: 3_500,
        completionPct: 54.7,
      },
    ],
  },
  {
    stage: "events",
    label: "Events",
    totalShards: 94_200,
    completedShards: 78_400,
    inProgressShards: 36,
    failedShards: 5,
    completionPct: 83.2,
    lastUpdated: "2026-03-23T07:30:00Z",
    byAssetGroup: [
      {
        assetGroup: "cefi",
        totalShards: 46_000,
        completedShards: 42_100,
        completionPct: 91.5,
      },
      {
        assetGroup: "tradfi",
        totalShards: 27_200,
        completedShards: 23_800,
        completionPct: 87.5,
      },
      {
        assetGroup: "defi",
        totalShards: 13_800,
        completedShards: 8_200,
        completionPct: 59.4,
      },
      {
        assetGroup: "onchain_perps",
        totalShards: 9_100,
        completedShards: 8_600,
        completionPct: 94.5,
      },
      {
        assetGroup: "prediction_market",
        totalShards: 600,
        completedShards: 120,
        completionPct: 20.0,
      },
      {
        assetGroup: "sports",
        totalShards: 3_500,
        completedShards: 1_580,
        completionPct: 45.1,
      },
    ],
  },
];

// ─── Active Jobs ──────────────────────────────────────────────────────────────

export const MOCK_ACTIVE_JOBS: import("@/lib/types/data-service").JobInfo[] = [
  {
    id: "job-dl-bin-0323",
    type: "download",
    status: "running",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    dateRange: { start: "2026-03-18", end: "2026-03-23" },
    shardsTotal: 6,
    shardsCompleted: 4,
    progressPct: 67,
    startedAt: "2026-03-23T06:00:00Z",
    estimatedCompletionAt: "2026-03-23T10:30:00Z",
    workersActive: 3,
    workersMax: 5,
    forceFlag: false,
  },
  {
    id: "job-dl-db-0323",
    type: "download",
    status: "running",
    assetGroup: "tradfi",
    venue: "DATABENTO",
    dateRange: { start: "2026-03-15", end: "2026-03-23" },
    shardsTotal: 9,
    shardsCompleted: 2,
    progressPct: 22,
    startedAt: "2026-03-23T04:00:00Z",
    estimatedCompletionAt: "2026-03-23T22:00:00Z",
    workersActive: 2,
    workersMax: 4,
    forceFlag: false,
  },
  {
    id: "job-proc-cefi-0322",
    type: "process",
    status: "running",
    assetGroup: "cefi",
    venue: "BYBIT",
    dateRange: { start: "2026-03-20", end: "2026-03-22" },
    shardsTotal: 3,
    shardsCompleted: 2,
    progressPct: 78,
    startedAt: "2026-03-23T07:00:00Z",
    estimatedCompletionAt: "2026-03-23T09:15:00Z",
    workersActive: 2,
    workersMax: 2,
    forceFlag: false,
  },
  {
    id: "job-backfill-defi-0301",
    type: "backfill",
    status: "queued",
    assetGroup: "defi",
    venue: "AAVEV3-ETHEREUM",
    dateRange: { start: "2026-02-28", end: "2026-03-05" },
    shardsTotal: 6,
    shardsCompleted: 0,
    progressPct: 0,
    startedAt: "2026-03-23T08:00:00Z",
    workersActive: 0,
    workersMax: 2,
    forceFlag: false,
  },
];

// ─── Job History ──────────────────────────────────────────────────────────────

export const MOCK_JOB_HISTORY: import("@/lib/types/data-service").JobHistoryEntry[] = [
  {
    jobType: "download",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    avgDurationMs: 3_600_000,
    p50DurationMs: 3_200_000,
    p90DurationMs: 5_400_000,
    successRate: 97.2,
    lastRunAt: "2026-03-23T06:00:00Z",
    runCount: 124,
  },
  {
    jobType: "download",
    assetGroup: "cefi",
    venue: "BYBIT",
    avgDurationMs: 2_800_000,
    p50DurationMs: 2_600_000,
    p90DurationMs: 4_200_000,
    successRate: 98.1,
    lastRunAt: "2026-03-22T18:00:00Z",
    runCount: 98,
  },
  {
    jobType: "download",
    assetGroup: "tradfi",
    venue: "DATABENTO",
    avgDurationMs: 7_200_000,
    p50DurationMs: 6_800_000,
    p90DurationMs: 10_800_000,
    successRate: 94.5,
    lastRunAt: "2026-03-23T04:00:00Z",
    runCount: 67,
  },
  {
    jobType: "process",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    avgDurationMs: 1_800_000,
    p50DurationMs: 1_600_000,
    p90DurationMs: 2_800_000,
    successRate: 99.1,
    lastRunAt: "2026-03-23T05:30:00Z",
    runCount: 205,
  },
  {
    jobType: "process",
    assetGroup: "tradfi",
    venue: "DATABENTO",
    avgDurationMs: 2_400_000,
    p50DurationMs: 2_200_000,
    p90DurationMs: 3_600_000,
    successRate: 97.8,
    lastRunAt: "2026-03-22T22:00:00Z",
    runCount: 142,
  },
  {
    jobType: "backfill",
    assetGroup: "defi",
    venue: "UNISWAPV3-ETHEREUM",
    avgDurationMs: 10_800_000,
    p50DurationMs: 9_600_000,
    p90DurationMs: 18_000_000,
    successRate: 88.0,
    lastRunAt: "2026-03-20T12:00:00Z",
    runCount: 25,
  },
];

// ─── Alerts ───────────────────────────────────────────────────────────────────

export const MOCK_ALERTS: import("@/lib/types/data-service").AlertItem[] = [
  {
    id: "alert-001",
    type: "new_instruments",
    severity: "info",
    title: "12 new instruments on Binance",
    message: "12 new perpetual contracts were discovered on Binance during the 08:00 cron run.",
    timestamp: "2026-03-23T08:14:00Z",
    read: false,
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    actionHref: "/services/data/instruments",
  },
  {
    id: "alert-002",
    type: "download_failed",
    severity: "warning",
    title: "Databento download: 2 shards failed",
    message: "2 of 9 shards failed during the TradFi Databento download (Mar 21, Mar 22). Retrying.",
    timestamp: "2026-03-23T07:42:00Z",
    read: false,
    venue: "DATABENTO",
    assetGroup: "tradfi",
    actionHref: "/services/data/raw",
  },
  {
    id: "alert-003",
    type: "gap_detected",
    severity: "critical",
    title: "BTC-PERP data gap: Mar 20",
    message: "Missing OHLCV data for BTC-PERP on Binance for 2026-03-20. Backfill queued.",
    timestamp: "2026-03-23T06:30:00Z",
    read: false,
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    actionHref: "/services/data/gaps",
  },
  {
    id: "alert-004",
    type: "job_completed",
    severity: "info",
    title: "Processing complete: OKX Mar 22",
    message: "All 6 timeframes processed for OKX perpetuals on 2026-03-22.",
    timestamp: "2026-03-23T05:15:00Z",
    read: true,
    venue: "OKX-SPOT",
    assetGroup: "cefi",
  },
  {
    id: "alert-005",
    type: "data_stale",
    severity: "warning",
    title: "Sports data: 18h stale",
    message: "Football match data from api_football has not been updated in 18 hours.",
    timestamp: "2026-03-22T14:00:00Z",
    read: true,
    venue: "API-FOOTBALL",
    assetGroup: "sports",
    actionHref: "/services/data/raw",
  },
];

// ─── Corporate Actions ────────────────────────────────────────────────────────

export const MOCK_CORPORATE_ACTIONS: import("@/lib/types/data-service").CorporateAction[] = [
  {
    id: "ca-001",
    venue: "DATABENTO",
    symbol: "AAPL",
    actionType: "split",
    effectiveDate: "2020-08-31",
    ratio: 4,
    description: "Apple 4-for-1 stock split",
    dataAdjusted: true,
    declarationDate: "2020-07-30",
    recordDate: "2020-08-24",
  },
  {
    id: "ca-002",
    venue: "DATABENTO",
    symbol: "TSLA",
    actionType: "split",
    effectiveDate: "2020-08-31",
    ratio: 5,
    description: "Tesla 5-for-1 stock split",
    dataAdjusted: true,
    declarationDate: "2020-08-11",
    recordDate: "2020-08-21",
  },
  {
    id: "ca-003",
    venue: "DATABENTO",
    symbol: "AMZN",
    actionType: "split",
    effectiveDate: "2022-06-06",
    ratio: 20,
    description: "Amazon 20-for-1 stock split",
    dataAdjusted: true,
    declarationDate: "2022-03-09",
    recordDate: "2022-05-27",
  },
  {
    id: "ca-004",
    venue: "DATABENTO",
    symbol: "GOOGL",
    actionType: "split",
    effectiveDate: "2022-07-15",
    ratio: 20,
    description: "Alphabet 20-for-1 stock split",
    dataAdjusted: true,
    declarationDate: "2022-02-01",
    recordDate: "2022-07-01",
  },
  {
    id: "ca-005",
    venue: "DATABENTO",
    symbol: "NVDA",
    actionType: "split",
    effectiveDate: "2024-06-10",
    ratio: 10,
    description: "NVIDIA 10-for-1 stock split",
    dataAdjusted: true,
    declarationDate: "2024-05-22",
    recordDate: "2024-06-06",
  },
  {
    id: "ca-006",
    venue: "IBKR",
    symbol: "FB",
    newSymbol: "META",
    actionType: "symbol_change",
    effectiveDate: "2022-06-09",
    description: "Facebook renamed to Meta Platforms",
    dataAdjusted: true,
    declarationDate: "2021-10-28",
    reason: "Corporate rebrand to Meta Platforms, Inc.",
  },
  {
    id: "ca-007",
    venue: "NASDAQ",
    symbol: "AAPL",
    actionType: "dividend",
    effectiveDate: "2026-02-07", // ex-date
    description: "Apple quarterly cash dividend",
    dataAdjusted: true,
    declarationDate: "2026-01-30",
    recordDate: "2026-02-10",
    payDate: "2026-02-13",
    amount: 0.26,
    currency: "USD",
    frequency: "quarterly",
    yieldPct: 0.51,
  },
  {
    id: "ca-008",
    venue: "NASDAQ",
    symbol: "MSFT",
    actionType: "earnings",
    effectiveDate: "2026-01-28",
    description: "Microsoft Q2 FY26 earnings",
    dataAdjusted: true,
    declarationDate: "2025-12-18",
    fiscalPeriod: "Q2 FY2026",
    reportTime: "amc",
    actualEps: 3.21,
    estimatedEps: 3.12,
    actualRevenue: 68_900_000_000,
    estimatedRevenue: 68_500_000_000,
    surprisePct: 2.88,
  },
  {
    id: "ca-009",
    venue: "NASDAQ",
    symbol: "NVDA",
    actionType: "earnings",
    effectiveDate: "2026-02-26",
    description: "NVIDIA Q4 FY26 earnings",
    dataAdjusted: true,
    declarationDate: "2026-01-21",
    fiscalPeriod: "Q4 FY2026",
    reportTime: "amc",
    actualEps: 0.89,
    estimatedEps: 0.81,
    actualRevenue: 42_100_000_000,
    estimatedRevenue: 38_300_000_000,
    surprisePct: 9.88,
  },
  {
    id: "ca-010",
    venue: "NYSE",
    symbol: "KO",
    actionType: "dividend",
    effectiveDate: "2026-03-13",
    description: "Coca-Cola quarterly cash dividend",
    dataAdjusted: true,
    declarationDate: "2026-02-20",
    recordDate: "2026-03-16",
    payDate: "2026-04-01",
    amount: 0.51,
    currency: "USD",
    frequency: "quarterly",
    yieldPct: 3.12,
  },
  {
    id: "ca-011",
    venue: "NASDAQ",
    symbol: "GOOGL",
    actionType: "earnings",
    effectiveDate: "2026-02-04",
    description: "Alphabet Q4 FY25 earnings",
    dataAdjusted: true,
    declarationDate: "2026-01-09",
    fiscalPeriod: "Q4 FY2025",
    reportTime: "amc",
    actualEps: null,
    estimatedEps: 2.15,
    actualRevenue: null,
    estimatedRevenue: 96_700_000_000,
    surprisePct: null,
  },
  {
    id: "ca-012",
    venue: "NYSE",
    symbol: "BRK.B",
    actionType: "reverse_split",
    effectiveDate: "2025-11-12",
    ratio: 2,
    description: "Illustrative 1-for-2 reverse split (demo row)",
    dataAdjusted: true,
    declarationDate: "2025-10-14",
    recordDate: "2025-11-07",
  },
  {
    id: "ca-013",
    venue: "NYSE",
    symbol: "GE",
    newSymbol: "GEV",
    actionType: "spinoff",
    effectiveDate: "2024-04-02",
    ratio: 4,
    description: "GE Vernova spin-off from General Electric (1 GEV per 4 GE)",
    dataAdjusted: true,
    declarationDate: "2023-11-09",
    recordDate: "2024-03-19",
    parentSymbol: "GE",
    targetSymbol: "GEV",
  },
];

// ─── Instrument Counts per Venue (realistic) ──────────────────────────────────

export const MOCK_INSTRUMENT_COUNTS: Record<
  string,
  {
    total: number;
    active: number;
    assetGroup: import("@/lib/types/data-service").DataAssetGroup;
  }
> = {
  binance: { total: 412, active: 398, assetGroup: "cefi" },
  bybit: { total: 284, active: 276, assetGroup: "cefi" },
  okx: { total: 198, active: 192, assetGroup: "cefi" },
  coinbase: { total: 87, active: 84, assetGroup: "cefi" },
  deribit: { total: 2840, active: 2840, assetGroup: "cefi" }, // options
  upbit: { total: 156, active: 149, assetGroup: "cefi" },
  hyperliquid: { total: 64, active: 64, assetGroup: "onchain_perps" },
  databento: { total: 4200, active: 4200, assetGroup: "tradfi" }, // CME + NASDAQ + ICE
  ibkr: { total: 12500, active: 12100, assetGroup: "tradfi" }, // stocks
  yahoo_finance: { total: 8400, active: 8200, assetGroup: "tradfi" },
  fred: { total: 820_000, active: 820_000, assetGroup: "tradfi" }, // macro series
  uniswap_v3: { total: 8920, active: 6400, assetGroup: "defi" },
  uniswap_v4: { total: 1240, active: 1240, assetGroup: "defi" },
  aave_v3: { total: 48, active: 42, assetGroup: "defi" },
  curve: { total: 342, active: 298, assetGroup: "defi" },
  balancer: { total: 287, active: 241, assetGroup: "defi" },
  lido: { total: 3, active: 3, assetGroup: "defi" },
  morpho: { total: 62, active: 62, assetGroup: "defi" },
  polymarket: { total: 4200, active: 1840, assetGroup: "prediction_market" },
  kalshi: { total: 1600, active: 890, assetGroup: "prediction_market" },
  api_football: { total: 14_000, active: 2400, assetGroup: "sports" }, // all fixtures
  footystats: { total: 11_500, active: 2100, assetGroup: "sports" },
};

// ─── Coverage Matrix Rows ─────────────────────────────────────────────────────

export const MOCK_COVERAGE_ROWS: import("@/lib/types/data-service").CoverageRow[] = [
  {
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    date: "2026-03-22",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "complete", completionPct: 100 },
    processing: { status: "complete", completionPct: 100 },
    features: { status: "complete", completionPct: 89 },
  },
  {
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    date: "2026-03-21",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "complete", completionPct: 100 },
    processing: { status: "complete", completionPct: 100 },
    features: { status: "complete", completionPct: 92 },
  },
  {
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    date: "2026-03-20",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "partial", completionPct: 98 },
    processing: { status: "complete", completionPct: 100 },
    features: { status: "partial", completionPct: 71 },
  },
  {
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    date: "2026-03-19",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "complete", completionPct: 100 },
    processing: { status: "partial", completionPct: 83 },
    features: { status: "missing", completionPct: 0 },
  },
  {
    venue: "DATABENTO",
    assetGroup: "tradfi",
    date: "2026-03-22",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "partial", completionPct: 94 },
    processing: { status: "complete", completionPct: 100 },
    features: { status: "missing", completionPct: 0 },
  },
  {
    venue: "DATABENTO",
    assetGroup: "tradfi",
    date: "2026-03-21",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "complete", completionPct: 100 },
    processing: { status: "complete", completionPct: 100 },
    features: { status: "complete", completionPct: 95 },
  },
  {
    venue: "AAVEV3-ETHEREUM",
    assetGroup: "defi",
    date: "2026-03-22",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "complete", completionPct: 100 },
    processing: { status: "partial", completionPct: 72 },
    features: { status: "missing", completionPct: 0 },
  },
  {
    venue: "AAVEV3-ETHEREUM",
    assetGroup: "defi",
    date: "2026-03-21",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "partial", completionPct: 87 },
    processing: { status: "missing", completionPct: 0 },
    features: { status: "missing", completionPct: 0 },
  },
  {
    venue: "API-FOOTBALL",
    assetGroup: "sports",
    date: "2026-03-22",
    instruments: { status: "complete", completionPct: 100 },
    rawData: { status: "partial", completionPct: 78 },
    processing: { status: "missing", completionPct: 0 },
    features: { status: "missing", completionPct: 0 },
  },
  {
    venue: "POLYMARKET",
    assetGroup: "prediction_market",
    date: "2026-03-22",
    instruments: { status: "partial", completionPct: 44 },
    rawData: { status: "partial", completionPct: 38 },
    processing: { status: "missing", completionPct: 0 },
    features: { status: "missing", completionPct: 0 },
  },
];

// ─── Timeframe Processing Status ─────────────────────────────────────────────

export const MOCK_TIMEFRAME_STATUS: import("@/lib/types/data-service").TimeframeStatus[] = [
  {
    timeframe: "1m",
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    completionPct: 98.2,
    totalDays: 2400,
    completedDays: 2357,
    pendingDays: 43,
    lastProcessedDate: "2026-03-22",
  },
  {
    timeframe: "5m",
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    completionPct: 97.8,
    totalDays: 2400,
    completedDays: 2347,
    pendingDays: 53,
    lastProcessedDate: "2026-03-22",
  },
  {
    timeframe: "15m",
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    completionPct: 97.4,
    totalDays: 2400,
    completedDays: 2338,
    pendingDays: 62,
    lastProcessedDate: "2026-03-22",
  },
  {
    timeframe: "1h",
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    completionPct: 99.1,
    totalDays: 2400,
    completedDays: 2378,
    pendingDays: 22,
    lastProcessedDate: "2026-03-22",
  },
  {
    timeframe: "4h",
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    completionPct: 99.3,
    totalDays: 2400,
    completedDays: 2383,
    pendingDays: 17,
    lastProcessedDate: "2026-03-22",
  },
  {
    timeframe: "1d",
    venue: "BINANCE-SPOT",
    assetGroup: "cefi",
    completionPct: 99.8,
    totalDays: 2400,
    completedDays: 2395,
    pendingDays: 5,
    lastProcessedDate: "2026-03-22",
  },
  {
    timeframe: "1m",
    venue: "DATABENTO",
    assetGroup: "tradfi",
    completionPct: 91.2,
    totalDays: 1560,
    completedDays: 1423,
    pendingDays: 137,
    lastProcessedDate: "2026-03-21",
  },
  {
    timeframe: "5m",
    venue: "DATABENTO",
    assetGroup: "tradfi",
    completionPct: 91.0,
    totalDays: 1560,
    completedDays: 1420,
    pendingDays: 140,
    lastProcessedDate: "2026-03-21",
  },
  {
    timeframe: "1h",
    venue: "DATABENTO",
    assetGroup: "tradfi",
    completionPct: 93.5,
    totalDays: 1560,
    completedDays: 1459,
    pendingDays: 101,
    lastProcessedDate: "2026-03-21",
  },
  {
    timeframe: "1d",
    venue: "DATABENTO",
    assetGroup: "tradfi",
    completionPct: 95.8,
    totalDays: 1560,
    completedDays: 1495,
    pendingDays: 65,
    lastProcessedDate: "2026-03-22",
  },
];

// ─── Enhanced Data Gaps (with severity multi-dimensions) ────────────────────

export const MOCK_ENHANCED_GAPS: import("@/lib/types/data-service").DataGap[] = [
  {
    id: "gap-001",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    folder: "perpetuals",
    dataType: "ohlcv",
    instrument: "BTC-PERP",
    gapStart: "2026-03-20",
    gapEnd: "2026-03-20",
    daysAffected: 1,
    severity: "critical",
    cause: "API rate limit exceeded",
    status: "backfilling",
    createdAt: "2026-03-21T06:00:00Z",
  },
  {
    id: "gap-002",
    assetGroup: "cefi",
    venue: "BINANCE-SPOT",
    folder: "perpetuals",
    dataType: "trades",
    instrument: "ETH-PERP",
    gapStart: "2026-03-19",
    gapEnd: "2026-03-19",
    daysAffected: 1,
    severity: "high",
    cause: "Provider downtime 2h window",
    status: "open",
    createdAt: "2026-03-20T08:00:00Z",
  },
  {
    id: "gap-003",
    assetGroup: "tradfi",
    venue: "DATABENTO",
    folder: "equity",
    dataType: "ohlcv",
    instrument: "AAPL",
    gapStart: "2026-03-15",
    gapEnd: "2026-03-17",
    daysAffected: 3,
    severity: "high",
    cause: "Network timeout during batch download",
    status: "backfilling",
    createdAt: "2026-03-18T04:00:00Z",
  },
  {
    id: "gap-004",
    assetGroup: "defi",
    venue: "AAVEV3-ETHEREUM",
    folder: "lending",
    dataType: "lending_rates",
    gapStart: "2026-02-28",
    gapEnd: "2026-03-05",
    daysAffected: 6,
    severity: "medium",
    cause: "RPC node sync issue",
    status: "open",
    createdAt: "2026-03-06T10:00:00Z",
  },
  {
    id: "gap-005",
    assetGroup: "sports",
    venue: "API-FOOTBALL",
    folder: "game_events",
    dataType: "game_events",
    gapStart: "2026-03-10",
    gapEnd: "2026-03-12",
    daysAffected: 3,
    severity: "low",
    cause: "API quota reset",
    status: "resolved",
    createdAt: "2026-03-13T09:00:00Z",
    resolvedAt: "2026-03-14T12:00:00Z",
  },
  {
    id: "gap-006",
    assetGroup: "cefi",
    venue: "OKX-SPOT",
    folder: "spot",
    dataType: "ohlcv",
    gapStart: "2026-03-22",
    gapEnd: "2026-03-22",
    daysAffected: 1,
    severity: "medium",
    cause: "Scheduled maintenance window",
    status: "open",
    createdAt: "2026-03-23T00:15:00Z",
  },
  {
    id: "gap-007",
    assetGroup: "tradfi",
    venue: "IBKR",
    folder: "equity",
    dataType: "trades",
    instrument: "TSLA",
    gapStart: "2026-01-15",
    gapEnd: "2026-01-15",
    daysAffected: 1,
    severity: "low",
    status: "wont_fix",
    createdAt: "2026-01-16T08:00:00Z",
  },
  {
    id: "gap-008",
    assetGroup: "defi",
    venue: "UNISWAPV3-ETHEREUM",
    folder: "pool_state",
    dataType: "pool_state",
    gapStart: "2026-03-18",
    gapEnd: "2026-03-20",
    daysAffected: 3,
    severity: "medium",
    cause: "RPC endpoint rotation",
    status: "backfilling",
    createdAt: "2026-03-21T11:00:00Z",
  },
];

// ─── Economic Events ──────────────────────────────────────────────────────────

export const MOCK_ECONOMIC_EVENTS: import("@/lib/types/data-service").EconomicEvent[] = [
  {
    id: "eco-001",
    eventType: "fomc",
    label: "FOMC Rate Decision",
    date: "2026-03-18",
    time: "19:00",
    importance: "high",
    country: "US",
    forecast: 5.25,
    actual: 5.25,
    previous: 5.25,
    unit: "%",
    surprise: 0,
    description: "Federal Open Market Committee interest rate decision",
    source: "Federal Reserve",
    period: "Mar 2026",
  },
  {
    id: "eco-002",
    eventType: "nfp",
    label: "Non-Farm Payrolls",
    date: "2026-03-06",
    time: "13:30",
    importance: "high",
    country: "US",
    forecast: 185000,
    actual: 228000,
    previous: 209000,
    unit: "K",
    surprise: 0.233,
    description: "US monthly employment report — non-farm payrolls",
    source: "BLS",
    period: "Feb 2026",
  },
  {
    id: "eco-003",
    eventType: "cpi",
    label: "CPI (YoY)",
    date: "2026-03-11",
    time: "13:30",
    importance: "high",
    country: "US",
    forecast: 2.9,
    actual: 2.8,
    previous: 3.0,
    unit: "%",
    surprise: -0.034,
    description: "US Consumer Price Index — year-over-year change",
    source: "BLS",
    period: "Feb 2026",
  },
  {
    id: "eco-004",
    eventType: "gdp",
    label: "GDP (QoQ)",
    date: "2026-03-27",
    time: "12:30",
    importance: "high",
    country: "US",
    forecast: 2.3,
    actual: null,
    previous: 2.1,
    unit: "%",
    surprise: null,
    description: "US Gross Domestic Product — quarter-over-quarter growth",
    source: "BEA",
    period: "Q4 2025",
  },
  {
    id: "eco-005",
    eventType: "fomc",
    label: "FOMC Rate Decision",
    date: "2026-05-06",
    time: "19:00",
    importance: "high",
    country: "US",
    forecast: 5.0,
    actual: null,
    previous: 5.25,
    unit: "%",
    surprise: null,
    description: "Federal Open Market Committee interest rate decision",
    source: "Federal Reserve",
    period: "May 2026",
  },
  {
    id: "eco-006",
    eventType: "nfp",
    label: "Non-Farm Payrolls",
    date: "2026-04-03",
    time: "13:30",
    importance: "high",
    country: "US",
    forecast: 190000,
    actual: null,
    previous: 228000,
    unit: "K",
    surprise: null,
    description: "US monthly employment report — non-farm payrolls",
    source: "BLS",
    period: "Mar 2026",
  },
  {
    id: "eco-007",
    eventType: "initial_claims",
    label: "Initial Jobless Claims",
    date: "2026-03-19",
    time: "13:30",
    importance: "medium",
    country: "US",
    forecast: 215000,
    actual: 224000,
    previous: 218000,
    unit: "K",
    surprise: 0.042,
    description: "Weekly initial unemployment insurance claims",
    source: "DOL",
    period: "Week ending Mar 14",
  },
  {
    id: "eco-008",
    eventType: "pce",
    label: "Core PCE (MoM)",
    date: "2026-02-28",
    time: "13:30",
    importance: "high",
    country: "US",
    forecast: 0.3,
    actual: 0.3,
    previous: 0.2,
    unit: "%",
    surprise: 0,
    description: "Fed's preferred inflation gauge — Personal Consumption Expenditures",
    source: "BEA",
    period: "Jan 2026",
  },
  {
    id: "eco-009",
    eventType: "election",
    label: "UK General Election",
    date: "2024-07-04",
    importance: "high",
    country: "UK",
    description: "United Kingdom general election — Labour party won majority",
    source: "UK Electoral Commission",
  },
];

// ─── Market Structure Events ──────────────────────────────────────────────────

export const MOCK_MARKET_STRUCTURE_EVENTS: import("@/lib/types/data-service").MarketStructureEvent[] = [
  {
    id: "mse-001",
    eventType: "options_expiry",
    label: "CME Monthly Options Expiry",
    date: "2026-03-21",
    time: "21:00",
    venue: "CME",
    description: "CME monthly options expiry — 3rd Friday",
    importance: "high",
    impactedSymbols: ["ES", "NQ", "CL", "GC"],
    openInterest: 2_480_000,
    notionalUsd: 1_120_000_000_000,
  },
  {
    id: "mse-002",
    eventType: "options_expiry",
    label: "Equity Options Expiry",
    date: "2026-03-21",
    time: "20:00",
    venue: "CBOE",
    description: "US equity options monthly expiry — 3rd Friday 4PM ET",
    importance: "high",
    impactedSymbols: ["SPY", "QQQ", "IWM", "AAPL", "MSFT", "NVDA"],
    openInterest: 18_900_000,
    notionalUsd: 3_400_000_000_000,
  },
  {
    id: "mse-003",
    eventType: "halving",
    label: "Bitcoin Halving",
    date: "2028-04-15",
    asset: "BTC",
    description: "Next estimated Bitcoin halving — block reward drops from 3.125 to 1.5625 BTC",
    importance: "high",
    blockHeight: 1_050_000,
  },
  {
    id: "mse-004",
    eventType: "halving",
    label: "Bitcoin Halving",
    date: "2024-04-20",
    asset: "BTC",
    description: "4th Bitcoin halving — block reward dropped from 6.25 to 3.125 BTC",
    importance: "high",
    blockHeight: 840_000,
  },
  {
    id: "mse-005",
    eventType: "futures_expiry",
    label: "CME BTC Futures Expiry",
    date: "2026-03-27",
    time: "16:00",
    venue: "CME",
    asset: "BTC",
    description: "CME Bitcoin futures last Friday expiry",
    importance: "medium",
    impactedSymbols: ["BTCH26"],
    openInterest: 24_800,
    notionalUsd: 8_600_000_000,
  },
  {
    id: "mse-006",
    eventType: "futures_expiry",
    label: "CME ETH Futures Expiry",
    date: "2026-03-27",
    time: "16:00",
    venue: "CME",
    asset: "ETH",
    description: "CME Ethereum futures last Friday expiry",
    importance: "medium",
    impactedSymbols: ["ETHH26"],
    openInterest: 18_200,
    notionalUsd: 1_200_000_000,
  },
  {
    id: "mse-007",
    eventType: "network_upgrade",
    label: "Ethereum Pectra Upgrade",
    date: "2025-05-07",
    asset: "ETH",
    description: "Ethereum Pectra hard fork — EIP-7702 account abstraction and validator UX improvements",
    importance: "high",
    blockHeight: 22_431_084,
    networkVersion: "Pectra",
  },
  {
    id: "mse-008",
    eventType: "liquidity_event",
    label: "Quarterly Rebalance",
    date: "2026-03-31",
    venue: "multiple",
    description: "End-of-quarter institutional rebalancing — elevated volume expected",
    importance: "medium",
    notionalUsd: 450_000_000_000,
  },
];

// ─── Calendar Holidays ────────────────────────────────────────────────────────

export const MOCK_CALENDAR_HOLIDAYS: import("@/lib/types/data-service").CalendarHoliday[] = [
  {
    id: "hol-001",
    date: "2026-01-01",
    label: "New Year's Day",
    region: "global",
    affectedVenues: ["nyse", "databento", "ibkr", "binance"],
    isPartialClose: false,
  },
  {
    id: "hol-002",
    date: "2026-01-19",
    label: "Martin Luther King Jr. Day",
    region: "US",
    affectedVenues: ["nyse", "databento", "ibkr"],
    isPartialClose: false,
  },
  {
    id: "hol-003",
    date: "2026-02-16",
    label: "Presidents' Day",
    region: "US",
    affectedVenues: ["nyse", "databento", "ibkr"],
    isPartialClose: false,
  },
  {
    id: "hol-004",
    date: "2026-04-03",
    label: "Good Friday",
    region: "US",
    affectedVenues: ["nyse", "databento", "ibkr"],
    isPartialClose: false,
  },
  {
    id: "hol-005",
    date: "2026-05-25",
    label: "Memorial Day",
    region: "US",
    affectedVenues: ["nyse", "databento", "ibkr"],
    isPartialClose: false,
  },
  {
    id: "hol-006",
    date: "2026-07-03",
    label: "Independence Day (early close)",
    region: "US",
    affectedVenues: ["nyse", "databento"],
    isPartialClose: true,
    closeTime: "17:00",
  },
  {
    id: "hol-007",
    date: "2026-12-24",
    label: "Christmas Eve (early close)",
    region: "US",
    affectedVenues: ["nyse", "databento"],
    isPartialClose: true,
    closeTime: "18:00",
  },
  {
    id: "hol-008",
    date: "2026-12-25",
    label: "Christmas Day",
    region: "global",
    affectedVenues: ["nyse", "databento", "ibkr", "lseg"],
    isPartialClose: false,
  },
  {
    id: "hol-009",
    date: "2026-01-26",
    label: "Australia Day",
    region: "AU",
    affectedVenues: ["asx"],
    isPartialClose: false,
  },
  {
    id: "hol-010",
    date: "2026-02-11",
    label: "National Foundation Day",
    region: "JP",
    affectedVenues: ["jpx"],
    isPartialClose: false,
  },
];
