...
export function generateSignals(
  ...
  for (let i = 0; i < count; i++) {
    ...
    signals.push({
      ...
      outcome: isClose ? "break_even" : isWin ? "win" : "loss",
      pnl_usd: isClose ? 0 : Math.round(pnl * 100) / 100,
      pnl_pct: isClose ? 0 : Math.round((pnl / 10000) * 10000) / 100,
      hold_duration_hours: isClose ? 0 : Math.round((1 + rng() * 24) * 10) / 10,
      bars_held: isClose ? 0 : Math.round(2 + rng() * 16),
      mfe_pct: isClose ? 0 : Math.round((rng() * 5 + 0.5) * 100) / 100,
      mae_pct: isClose ? 0 : Math.round((-rng() * 3 - 0.2) * 100) / 100,
