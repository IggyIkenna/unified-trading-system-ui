...
import { MOCK_CANDIDATES } from "@/lib/mocks/fixtures/promote-candidates";
...
export interface PromoteLifecycleState {
  ...
  recordWorkflow: (strategyId: string, stage: PromotionStage, payload: WorkflowPayload) => void;
  reset: () => void;
}

export const usePromoteLifecycleStore = create<PromoteLifecycleState>((set) => ({
  candidates: cloneInitialCandidates(),
  selectedId: null,
  setSelectedId: (id) => set({ selectedId: id }),
  recordWorkflow: (strategyId, stage, payload) =>
    set((s) => ({
      candidates: s.candidates.map((c) => (c.id === strategyId ? applyPromoteWorkflowRecord(c, stage, payload) : c)),
    })),
  reset: () =>
    set({
      candidates: cloneInitialCandidates(),
...
export function selectPromoteSelectedStrategy(state: PromoteLifecycleState): CandidateStrategy | null {
