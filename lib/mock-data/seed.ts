...
function pos(id: string, instrument: string, venue: string, side: "long" | "short", qty: number, entry: number, current: number, orgId: string, clientId: string, stratId: string, stratName: string): SeedPosition {
  const mult = side === "long" ? 1 : -1;
  return { id, instrument, venue, side, quantity: qty, entryPrice: entry, currentPrice: current, unrealisedPnl: Math.round((current - entry) * qty * mult * 100) / 100, realisedPnl: Math.round((Math.random() * 4000 - 1000) * 100) / 100, orgId, clientId, strategyId: stratId, strategyName: stratName };
}
...
function ord(id: string, inst: string, venue: string, side: "buy" | "sell", qty: number, price: number, filled: number, status: "open" | "filled" | "partially_filled" | "cancelled", type: "limit" | "market" | "stop", daysAgo: number, orgId: string, clientId: string, stratId: string): SeedOrder {
  const d = new Date(); d.setDate(d.getDate() - daysAgo); d.setHours(Math.floor(Math.random() * 14) + 8);
  return { id, instrument: inst, venue, side, quantity: qty, price, filledQty: filled, status, type, timestamp: d.toISOString(), orgId, clientId, strategyId: stratId };
}
...
function trd(id: string, inst: string, venue: string, side: "buy" | "sell", qty: number, price: number, type: "Exchange" | "OTC" | "DeFi" | "Manual", daysAgo: number, orgId: string, clientId: string, stratId: string, counterparty: string): SeedTrade {
  const d = new Date(); d.setDate(d.getDate() - daysAgo); d.setHours(Math.floor(Math.random() * 14) + 8);
  const sd = new Date(d); sd.setDate(sd.getDate() + 2);
  const fees = Math.round(qty * price * 0.001 * 100) / 100;
  return { id, instrument: inst, venue, side, quantity: qty, price, fees, total: Math.round((qty * price + (side === "buy" ? fees : -fees)) * 100) / 100, tradeType: type, status: daysAgo > 2 ? "settled" : "pending", counterparty, timestamp: d.toISOString(), settlementDate: sd.toISOString(), orgId, clientId, strategyId: stratId };
}
...
function alrt(id: string, msg: string, sev: "critical" | "high" | "medium" | "low", src: string, hoursAgo: number, orgId: string, stratId: string, ack: boolean): SeedAlert {
  const d = new Date(); d.setHours(d.getHours() - hoursAgo);
  return { id, message: msg, severity: sev, source: src, timestamp: d.toISOString(), orgId, strategyId: stratId, acknowledged: ack };
}
...
export const SEED_ALERTS: SeedAlert[] = [
  ...
  alrt("alert-014", "BTC Momentum: daily P&L target reached (+$12K)", "low", "strategy-health", 6, "atlas-ventures", "strat-btc-mom-atlas", true),
];
...
function generateDailyPnl(days: number, avgDaily: number, vol: number): SeedPnlDay[] {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    result.push({ date: d.toISOString().split("T")[0], pnl: Math.round((avgDaily + (Math.random() - 0.5) * 2 * vol) * 100) / 100 });
  }
  return result;
}
