...
const venueCategoryMap = (registries.venue_category_map ?? {}) as Record<
  string,
  string
>;
...
const allVenues =
  Object.keys(venueCategoryMap).length > 0
...
const instrumentTypesByVenue = (registries.instrument_types_by_venue ??
  {}) as Record<string, unknown>;
const instrumentSample = (data.representative_instrument_sample ?? {}) as {
  entries?: Record<string, unknown>;
};
const instrumentEntries =
  Object.keys(instrumentTypesByVenue).length > 0
...
export const PLATFORM_STATS = {
  ...
  dataVolumeTB: "100+",
  historyYears: "6+",
  uacEnumCount: Object.keys((data.uac_enums ?? {}) as Record<string, unknown>)
    .length,
  uicEnumCount: Object.keys((data.uic_enums ?? {}) as Record<string, unknown>)
    .length,
} as const;
