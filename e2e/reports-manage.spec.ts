...
test.describe('Reports Service', () => {
  test('P&L Attribution page renders summary cards and table', async ({ page }) => {
    await page.goto('/services/reports/overview')
    await page.waitForLoadState('networkidle')

    // Should have summary cards
    await expect(page.locator('text=Reporting')).toBeVisible({ timeout: 10000 })
  ...
  test('Reconciliation page renders drift panel and history', async ({ page }) => {
    await page.goto('/services/reports/reconciliation')
    await page.waitForLoadState('networkidle')

    // Should have summary cards
    await expect(page.locator('text=Total Breaks').or(page.locator('text=Reconciliation'))).toBeVisible({ timeout: 10000 })

    // DriftAnalysisPanel should render
    await expect(page.locator('text=Drift Analysis').or(page.locator('text=drift'))).toBeVisible()
  })

  test('Regulatory page renders report table with status badges', async ({ page }) => {
    await page.goto('/services/reports/regulatory')
    await page.waitForLoadState('networkidle')

    // Should have status summary cards
    await expect(page.locator('text=Submitted').or(page.locator('text=MiFID'))).toBeVisible({ timeout: 10000 })
...
test.describe('Manage Service', () => {
  test('Clients page renders client list', async ({ page }) => {
    ...
    await expect(page.locator('text=Client Management').or(page.locator('text=Clients'))).toBeVisible({ timeout: 10000 })

    // Should have Onboard Client button
    await expect(page.getByRole('button', { name: /onboard client/i })).toBeVisible()
  })
  ...
  test('Onboard Client workflow opens multi-step modal', async ({ page }) => {
    ...
    // Should show Step 1
    await expect(page.locator('text=Client Details')).toBeVisible({ timeout: 5000 })
    ...
    await expect(page.locator('text=Strategy Selection')).toBeVisible({ timeout: 5000 })
  })
  ...
  test('Users page renders user table', async ({ page }) => {
    ...
    await expect(page.locator('text=User Management').or(page.locator('text=Users'))).toBeVisible({ timeout: 10000 })
  ...
  test('Mandates page renders mandate table', async ({ page }) => {
    ...
    await expect(page.locator('text=Investment Mandates').or(page.locator('text=Mandates'))).toBeVisible({ timeout: 10000 })
  ...
  test('Compliance page renders rules and violations', async ({ page }) => {
    ...
    await expect(page.locator('text=Compliance').first()).toBeVisible({ timeout: 10000 })
