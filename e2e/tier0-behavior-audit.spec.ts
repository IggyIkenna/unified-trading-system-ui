...
async function loginAsAdmin(page: import("@playwright/test").Page) {
  ...
  await page
    .waitForURL(/\/(dashboard|admin|services)/, { timeout: 10000 })
    .catch(() => {});
}
...
test.describe("Tier 0 behavior audit", () => {
  ...
  test("admin access requests: pending row has Approve and approve mutates state", async ({
    page,
  }) => {
    ...
    await expect(
      approvedBadge,
      "TIER0_AUDIT: after Approve, request should show approved status",
    ).toBeVisible({
      timeout: 10000,
    });
  });

  test("trading alerts: active alert shows Ack control; Ack updates status", async ({
    page,
  }) => {
  ...
  test("reports reconciliation surface exposes reconciliation narrative", async ({
    page,
  }) => {
    ...
    await expect(
      main,
      "TIER0_AUDIT: reconciliation page should render main content",
    ).toBeVisible();
  ...
  test("strategy backtests surface exposes run / backtest primary action", async ({
    page,
  }) => {
    ...
    const hasBacktest = await page
      .getByRole("button", { name: /backtest/i })
      .count();
  ...
  test("manage user access request form is reachable from client manage flow", async ({
    page,
  }) => {
  ...
  test("footer FCA link does not 404", async ({ page }) => {
    ...
    await expect(
      fcaLink,
      "TIER0_AUDIT: footer should have FCA regulatory link",
    ).toBeVisible({ timeout: 5000 });

    const href = await fcaLink.getAttribute("href");
    expect(
      href,
      "TIER0_AUDIT: FCA link should not point to /compliance (404)",
    ).not.toBe("/compliance");
    ...
    if (href) {
      ...
      expect(
        resp?.status(),
        `TIER0_AUDIT: FCA link target ${href} should not 404`,
      ).toBeLessThan(400);
    }
  });

  test("dashboard shows non-zero venue count from PLATFORM_STATS", async ({
    page,
  }) => {
    ...
    await expect(
      venueKpi,
      "TIER0_AUDIT: dashboard should display venue count",
    ).toBeVisible({ timeout: 5000 });
  ...
  test("landing page and dashboard venue counts are consistent", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("no console warnings for unhandled API routes on key pages", async ({
    page,
  }) => {
  ...
  test("admin approve → deny round-trip preserves state", async ({ page }) => {
    ...
    expect(
      pendingCount,
      "TIER0_AUDIT: should have at least one pending request",
    ).toBeGreaterThan(0);

    const deny = page.getByRole("button", { name: /Deny/i }).first();
    await expect(
      deny,
      "TIER0_AUDIT: pending request should have Deny button",
    ).toBeVisible({ timeout: 5000 });
    ...
    await expect(
      deniedBadge,
      "TIER0_AUDIT: after Deny, request should show denied status",
    ).toBeVisible({ timeout: 10000 });
  });

  test("persona login sets correct role in authenticated shell", async ({
    page,
  }) => {
    await page.goto("/dashboard");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("Reset Demo button is visible in mock footer", async ({ page }) => {
    ...
    await expect(
      resetBtn,
      "TIER0_AUDIT: Reset Demo should be visible in mock mode footer",
    ).toBeVisible({ timeout: 5000 });
  });
});
