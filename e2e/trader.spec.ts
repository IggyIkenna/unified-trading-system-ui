...
test.describe("Overview Dashboard", () => {
  ...
  test.describe("Context Bar - Organization/Client/Strategy Filters", () => {
    test("org selector opens and allows selection", async ({ page }) => {
      const orgSelector = page.locator(
        'button:has-text("All Orgs"), button:has-text("Organization")',
      );
    ...
    test("client selector filters by selected org", async ({ page }) => {
      const clientSelector = page.locator(
        'button:has-text("All Clients"), button:has-text("Client")',
      );
    ...
    test("strategy multi-select works", async ({ page }) => {
      const strategySelector = page.locator(
        'button:has-text("All Strategies"), button:has-text("Strategies")',
      );
  ...
  test.describe("Scope Summary", () => {
    test("displays current filter state", async ({ page }) => {
      const scopeSummary = page.locator(
        '.scope-summary, [data-testid="scope-summary"]',
      );
      ...
      if ((await pauseBtn.count()) > 0) {
        ...
        expect(
          (await dialog.count()) > 0 ||
            buttonChanged?.includes("Resume") ||
            buttonChanged?.includes("Pause"),
        ).toBeTruthy();
      }
    });
    ...
    test("flatten button opens dialog with options", async ({ page }) => {
      ...
      if ((await flattenBtn.count()) > 0) {
        ...
        if ((await dialog.count()) > 0) {
          ...
          expect(
            (await confirmBtn.count()) + (await cancelBtn.count()),
          ).toBeGreaterThan(0);
        }
      }
    });
  ...
  test.describe("Kill Switch Panel", () => {
    test("kill switch can be armed", async ({ page }) => {
      const killSwitch = page.locator(
        'button:has-text("Arm Kill"), button:has-text("Kill Switch")',
      );
  ...
  test.describe("Value Format Toggle", () => {
    test("can switch between dollar and percent", async ({ page }) => {
      const dollarBtn = page.locator(
        'button[title*="dollar"], button:has-text("$")',
      );
      const percentBtn = page.locator(
        'button[title*="percent"], button:has-text("%")',
      );
  ...
  test.describe("Strategy Cards", () => {
    test("strategy cards are clickable and show details", async ({ page }) => {
      const strategyCards = page.locator(
        '[data-testid="strategy-card"], .strategy-card',
      );
