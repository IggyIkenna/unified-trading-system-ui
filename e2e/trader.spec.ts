...
test.describe('Trader Dashboard', () => {
    ...
    test('flatten button opens dialog with options', async ({ page }) => {
      ...
      if (await flattenBtn.count() > 0) {
        ...
        if (await dialog.count() > 0) {
          ...
          const confirmBtn = dialog.locator('button:has-text("Confirm"), button:has-text("Flatten"), button:has-text("Execute")')
  ...
  test.describe('Time Series Toggle', () => {
    test('time series can be expanded/collapsed', async ({ page }) => {
      const toggleBtn = page.locator('button:has-text("Time Series"), button:has-text("Show"), button:has-text("Hide")')
    ...
    test('promote button shows info or opens dialog', async ({ page }) => {
      ...
      if (await promoteBtn.count() > 0) {
        ...
        expect(
          ...
          true // Button clicked without error
        ).toBeTruthy()
      }
    })
  })
