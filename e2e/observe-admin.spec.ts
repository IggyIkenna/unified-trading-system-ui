...
test.describe('Observe > Risk Dashboard', () => {
  test('renders exposure data and risk cards', async ({ page }) => {
    ...
    await expect(page.getByText(/VaR|Value at Risk/i)).toBeVisible({ timeout: 10000 })
  })
  ...
  test('kill switch requires confirmation dialog', async ({ page }) => {
    ...
    if (await killButton.isVisible()) {
      ...
      await expect(page.getByText(/confirm|are you sure/i)).toBeVisible({ timeout: 5000 })
    }
  })
})
...
test.describe('Observe > Alerts', () => {
  ...
  test('empty state shows "All Clear" when no alerts', async ({ page }) => {
    ...
    if (await allClear.isVisible()) {
      await expect(page.getByText(/all systems operating normally/i)).toBeVisible()
    }
  })
})
...
test.describe('Observe > System Health', () => {
  ...
  test('tabs switch between services, freshness, resources, dependencies, logs', async ({ page }) => {
    await page.goto('/services/observe/health')
    await page.waitForLoadState('networkidle')

    // Click through tabs
    const tabs = ['Services', 'Feature Freshness', 'Resources', 'Dependencies', 'Logs']
