...
// Deduplicate by path
const UNIQUE_ROUTES = ALL_SMOKE_ROUTES.filter(
  (route, idx, arr) => arr.findIndex((r) => r.path === route.path) === idx,
);
...
test.describe("Tiered Smoke — Page Load + No Console Errors", () => {
  ...
  for (const route of UNIQUE_ROUTES) {
    test(`[Tier ${TIER}] ${route.name} (${route.path}) loads clean`, async ({
      page,
    }) => {
      const errors = await collectConsoleErrors(page, route.path);
      await takeScreenshot(
        page,
        `tier${TIER}-${route.name.replace(/\s+/g, "-").toLowerCase()}`,
      );
...
test.describe("Tiered Smoke — Protocol Badge", () => {
  ...
  for (const pagePath of BADGE_PAGES) {
    test(`[Tier ${TIER}] Protocol badge visible on ${pagePath}`, async ({
      page,
    }) => {
...
test.describe("Tiered Smoke — Navigation", () => {
  ...
  test(`[Tier ${TIER}] 404 route shows error or redirects`, async ({
    page,
  }) => {
...
test.describe("Tiered Smoke — Category Data Renders", () => {
  test(`[Tier ${TIER}] Trading terminal has price data`, async ({ page }) => {
    ...
    const priceElements = page.locator(
      '[data-testid*="price"], .price, .last-price, [class*="price"]',
    );
...
if (TIER >= 2) {
  test.describe("Tier 2 — SSE/WS Connectivity", () => {
    test("SSE connection established on positions page", async ({ page }) => {
      ...
      if (sseUrls.length === 0) {
        console.warn(
          "No SSE connections detected — verify SSE is wired on positions page",
        );
      }
    });
    ...
    test("WebSocket connection on trading terminal", async ({ page }) => {
      ...
      if (wsUrls.length === 0) {
        console.warn(
          "No WebSocket connections detected — verify WS is wired on terminal page",
        );
      }
    });
  });
