...
test.describe('Tier 2: Research Data', () => {
  test('research hub renders KPI cards or content', async ({ page }) => {
    ...
    const content = page.locator(
      '[data-testid*="kpi"], .kpi-card, .stat-card, .metric-card, table, [role="grid"]'
    )
    await expect(async () => {
  ...
  test('ML models page shows model list', async ({ page }) => {
    ...
    const modelRows = page.locator(
      'table tbody tr, [role="row"], [data-testid*="model"], .model-card'
    )
    await expect(async () => {
  ...
  test('strategies page shows backtest data', async ({ page }) => {
    ...
    const rows = page.locator(
      'table tbody tr, [role="row"], [data-testid*="strategy"], [data-testid*="backtest"]'
    )
    await expect(async () => {
  ...
  test('promote review queue shows candidates', async ({ page }) => {
    ...
    const candidates = page.locator(
      'table tbody tr, [role="row"], [data-testid*="candidate"], .candidate-card'
    )
    await expect(async () => {
      const bodyText = await page.textContent('body')
      // Either candidates exist or page content is substantial
      expect(
        (await candidates.count()) > 0 || (bodyText?.length ?? 0) > 100
      ).toBeTruthy()
    }).toPass({ timeout: 10000 })
  })
})
...
test.describe('Tier 3: Research Journey', () => {
  test('navigate research hub → ML models → features via tabs', async ({
    page,
  }) => {
    await page.goto('/services/research/overview')
    await page.waitForLoadState('networkidle')
    ...
    // Click ML Models tab
    const mlTab = page.locator(
      'a:has-text("ML"), a:has-text("Models"), [role="tab"]:has-text("ML")'
    )
    ...
    const featuresTab = page.locator(
      'a:has-text("Features"), [role="tab"]:has-text("Features")'
    )
  ...
  test('navigate to strategies and compare', async ({ page }) => {
    ...
    const compareTab = page.locator(
      'a:has-text("Compare"), [role="tab"]:has-text("Compare")'
    )
