...
test.describe("Lifecycle Navigation", () => {
  test("admin sees all 8 lifecycle stages", async ({ page }) => {
    ...
    const stages = ["Acquire", "Build", "Promote", "Run", "Execute", "Observe", "Manage", "Report"]
    for (const stage of stages) {
      await expect(page.locator(`button:has-text("${stage}")`)).toBeVisible()
    }
  })

  test("client-data-only sees limited stages with upgrade badges", async ({ page }) => {
  ...
  test("clicking lifecycle dropdown lands on first tab, not card landing", async ({ page }) => {
    await page.goto(`${BASE}/login?persona=admin`)
    await page.click('button:has-text("Sign In")')
    await page.waitForURL("**/dashboard")
...
test.describe("Service Tabs", () => {
  test("trading service shows correct tabs", async ({ page }) => {
    ...
    await expect(page.locator('a:has-text("Execution Analytics")')).not.toBeVisible()
  })

  test("execution service shows EXECUTE_TABS with Candidates and Handoff", async ({ page }) => {
    await page.goto(`${BASE}/login?persona=admin`)
    await page.click('button:has-text("Sign In")')
    await page.goto(`${BASE}/services/execution/overview`)

    const tabs = ["Analytics", "Algos", "Venues", "TCA", "Benchmarks", "Candidates", "Handoff"]
    for (const tab of tabs) {
      await expect(page.locator(`a:has-text("${tab}")`)).toBeVisible()
    }
  })
