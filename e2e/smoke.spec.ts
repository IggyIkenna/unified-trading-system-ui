...
test.describe("Smoke Tests - All Pages Load", () => {
  for (const page of PAGES) {
    test(`${page.name} page loads without console errors`, async ({
      page: browserPage,
    }) => {
...
test.describe("Interactive Elements - Buttons Have Handlers", () => {
  ...
  test("dialogs open and have action buttons", async ({ page }) => {
    ...
    const dialogTriggers = page.locator(
      'button:has-text("Flatten"), button:has-text("Kill"), button:has-text("Pause")',
    );
...
test.describe("Filters - Selection Propagates", () => {
  test("selecting a strategy updates the scope summary", async ({ page }) => {
    ...
    const strategySelector = page.locator(
      '[data-testid="strategy-filter"], .strategy-selector, button:has-text("Strategies")',
    );

    if ((await strategySelector.count()) > 0) {
      // Get initial scope text
      const scopeSummary = page.locator(
        '[data-testid="scope-summary"], .scope-summary',
      );
