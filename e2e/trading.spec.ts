...
test.describe("Trading Terminal Overview", () => {
  test("renders candlestick chart, order book, and order entry form", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("tab", { name: "Limit" }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("tab", { name: "Market" }).first(),
    ).toBeVisible();
  });
});
...
test.describe("Manual Trade Sheet", () => {
  test("clicking Manual Trade opens sheet drawer with trade form fields", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    await page.goto(`${BASE}/services/trading/overview`);
    ...
    const sheetContent = page.locator(
      '[class*="SheetContent"], [data-slot="sheet-content"], [role="dialog"]',
    );
    ...
    await expect(
      page.getByRole("button", { name: /Preview Order/i }),
    ).toBeVisible();
  });
});
...
test.describe("Batch Mode Banner", () => {
  test("switching to batch mode shows 'Viewing Batch Data' banner", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    await page.goto(`${BASE}/services/trading/overview`);
    ...
    await expect(page.locator("text=Viewing Batch Data")).toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe("Execution Analytics", () => {
  test("execution overview renders platform heading and metric cards", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    ...
    // Page heading
    await expect(page.locator("h1:has-text('Execution Platform')")).toBeVisible(
      { timeout: 10000 },
    );
...
test.describe("Execution Venues", () => {
  test("venues page renders Venue Matrix heading and venue data", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    ...
    // Page heading
    await expect(page.locator("h1:has-text('Venue Matrix')")).toBeVisible({
      timeout: 10000,
    });
