...
test.describe("Regulatory Onboarding Runbook", () => {
  test.describe("Stage 1: Signup — Account Creation", () => {
    test("navigates to regulatory signup from landing page", async ({
      page,
    }) => {
    ...
    test("validates Step 1 — catches invalid email", async ({ page }) => {
      ...
      await page.fill(
        'input[placeholder="Acme Capital"]',
        TEST_USER.company,
      );
      await page.fill(
    ...
    test("validates Step 1 — catches name without surname", async ({
      page,
    }) => {
      await page.goto(`${BASE}/signup?service=regulatory`);
      ...
      await page.click('button:has-text("Continue")');
      await expect(
        page.locator("text=Enter first and last name"),
      ).toBeVisible();
    });
    ...
    test("validates Step 1 — catches short password", async ({ page }) => {
      ...
      await page.click('button:has-text("Continue")');
      await expect(
        page.locator("text=At least 6 characters"),
      ).toBeVisible();
    });

    test("validates Step 1 — catches phone without country code", async ({
      page,
    }) => {
      await page.goto(`${BASE}/signup?service=regulatory`);
      ...
      await page.fill(
        'input[placeholder="+44 7XXX XXX XXX"]',
        "07700900000",
      ); // no +
      await page.fill(
        'input[placeholder="Min 6 characters"]',
      ...
      await page.click('button:has-text("Continue")');
      await expect(
        page.locator("text=Start with + and country code"),
      ).toBeVisible();
    });

    test("Step 1 succeeds with valid data — account created", async ({
      page,
    }) => {
      await page.goto(`${BASE}/signup?service=regulatory`);
      ...
      await expect(
        page.locator("text=Configure Your Engagement"),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Stage 2: Configure Engagement", () => {
    test("Step 2 — select AR engagement and activities", async ({ page }) => {
      ...
      const dealingLabel = page.locator(
        "text=Dealing in Investments as Agent",
      );
      await dealingLabel.hover();
      await expect(
        page.locator("text=Execute trades on behalf of your clients"),
      ).toBeVisible();
  ...
  test.describe("Stage 3: Document Upload", () => {
    test("shows required documents based on applicant type", async ({
      page,
    }) => {
      await page.goto(`${BASE}/signup?service=regulatory`);
      // Quick fill Step 1 + 2
      ...
      await expect(
        page.locator("text=Source of Funds Declaration"),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Stage 4: Login & Forgot Password", () => {
    ...
    test("forgot password shows demo mode message", async ({ page }) => {
      ...
      await expect(
        page.locator("text=Password reset").or(page.locator("text=demo")),
