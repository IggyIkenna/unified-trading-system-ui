...
function clearMockState(page: Page) {
  return page.evaluate(() => localStorage.removeItem('mock-provisioning-state'))
}
...
test.describe('User Management: Pages', () => {
  ...
  test('users list renders with internal and external sections', async ({ page }) => {
  ...
  test('users list shows real email addresses', async ({ page }) => {
    await page.goto(`${BASE}/admin/users`)
    await expect(page.getByText('admin@odum.internal')).toBeVisible({ timeout: 10000 })
  ...
  test('admin tabs show Users / Access Requests / Onboard only', async ({ page }) => {
    await page.goto(`${BASE}/admin/users`)
    await expect(page.locator('a[href="/admin/users"]').first()).toBeVisible({ timeout: 10000 })
    await expect(page.locator('a[href="/admin/users/requests"]').first()).toBeVisible()
    await expect(page.locator('a[href="/admin/users/onboard"]').first()).toBeVisible()
  })
...
test.describe('User Management: Detail View', () => {
  ...
  test('external user detail shows portal only', async ({ page }) => {
    ...
    await expect(page.getByText('pm@alphacapital.com')).toBeVisible({ timeout: 10000 })
...
test.describe('User Management: Lifecycle', () => {
  ...
  test('onboard new internal user and verify in list', async ({ page }) => {
    ...
    const domainBtn = page.locator('button:has-text("Platform"), button:has-text("Data Access")').first()
    if (await domainBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await domainBtn.click()
      await page.waitForTimeout(300)
      // Click first category to expand
      const catBtn = page.locator('button:has-text("Service"), button:has-text("Venues")').first()
...
test.describe('User Management: Access Requests', () => {
  ...
  test('pending requests visible with approve/deny buttons', async ({ page }) => {
    await page.goto(`${BASE}/admin/users/requests`)
    await page.waitForTimeout(1000)
...
test.describe('User Management: Permissions', () => {
  test('non-admin redirected away from admin pages', async ({ page }) => {
    ...
    const url = page.url()
    const body = await page.textContent('body')
    const blocked = url.includes('/dashboard') || (body?.includes('Access Denied') ?? false)
...
test.describe('User Management: Permission Catalogue', () => {
  ...
  test('catalogue tab visible in admin nav', async ({ page }) => {
    await page.goto(`${BASE}/admin/users`)
    await page.waitForTimeout(500)
    await expect(page.locator('a[href="/admin/users/catalogue"]').first()).toBeVisible({ timeout: 5000 })
  })
})
