...
test.describe("Tier 1: Gateway mock mode readiness", () => {
  ...
  test("API /readiness returns ready with mock mode info", async ({
    request,
  }) => {
    ...
    expect(
      body.mock_mode === true || body.mock_domain_service === true,
    ).toBeTruthy();
  });
...
test.describe("Full stack smoke — all 9 services", () => {
  ...
  for (const svc of SERVICE_ROUTES) {
    test(`${svc.name} service page loads`, async ({ page }) => {
      ...
      expect(
        status === 200 || status === 304 || status === 307 || status === 308,
      ).toBeTruthy();
  ...
  test("POST /admin/reset restores clean state", async ({ request }) => {
    ...
    const ordersRes = await request.get(
      `${API}/execution/orders?venue=test-smoke`,
    );
