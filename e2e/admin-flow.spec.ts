...
test.describe('Tier 2: Admin Data', () => {
  ...
  test('config page has configuration controls', async ({ page }) => {
    ...
    const controls = page.locator(
      'input, select, textarea, button:visible, [role="switch"]'
    )
    await expect(async () => {
...
test.describe('Tier 2: Manage Data', () => {
  test('clients page shows client list', async ({ page }) => {
    ...
    const clientRows = page.locator(
      'table tbody tr, [role="row"], [data-testid*="client"], .client-card'
    )
    await expect(async () => {
      const bodyText = await page.textContent('body')
      expect(
        (await clientRows.count()) > 0 || (bodyText?.length ?? 0) > 200
      ).toBeTruthy()
    }).toPass({ timeout: 10000 })
  })
  ...
  test('users page shows user list', async ({ page }) => {
    ...
    const userRows = page.locator(
      'table tbody tr, [role="row"], [data-testid*="user"], .user-card'
    )
    await expect(async () => {
      const bodyText = await page.textContent('body')
      expect(
        (await userRows.count()) > 0 || (bodyText?.length ?? 0) > 200
      ).toBeTruthy()
    }).toPass({ timeout: 10000 })
  })
...
test.describe('Tier 3: Admin Journey', () => {
  test('navigate admin → config → devops', async ({ page }) => {
    ...
    const configLink = page.locator(
      'a[href="/config"], a:has-text("Config"), [role="tab"]:has-text("Config")'
    )
    ...
    // Navigate to devops
    const devopsLink = page.locator(
      'a[href="/devops"], a:has-text("DevOps"), [role="tab"]:has-text("DevOps")'
    )
  ...
  test('navigate manage clients → users via tabs', async ({ page }) => {
    ...
    const usersTab = page.locator(
      'a:has-text("Users"), [role="tab"]:has-text("Users")'
    )
  ...
  test('reset demo button exists in admin or debug footer', async ({
    page,
  }) => {
    await page.goto('/admin')
    await page.waitForLoadState('networkidle')
    ...
    const debugFooter = page.locator(
      '[data-testid="debug-footer"], footer, .debug-footer'
    )

    // Reset may be in admin page or debug footer
    const hasReset =
      (await resetBtn.count()) > 0 || (await debugFooter.count()) > 0
