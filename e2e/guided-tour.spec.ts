...
test.describe("Guided Tour", () => {
  test("tour overlay appears on fresh visit (no localStorage flag)", async ({
    page,
  }) => {
    // Clear the tour-completed flag to simulate first visit
    await page.goto("/dashboard");
    await page.evaluate(() =>
      localStorage.removeItem("unified-tour-completed"),
    );
    ...
    await expect(async () => {
      const joyrideTooltip = page.locator(
        '[role="alertdialog"], [class*="joyride"], [class*="__tooltip"]',
      );
  ...
  test("clicking Skip Tour closes the tour and sets localStorage", async ({
    page,
  }) => {
    await page.goto("/dashboard");
    await page.evaluate(() =>
    ...
    const skipBtn = page.locator(
      'button:has-text("Skip Tour"), button:has-text("Skip")',
    );
    ...
    const flagValue = await page.evaluate(() =>
      localStorage.getItem("unified-tour-completed"),
    );
    expect(flagValue).toBe("true");
  });

  test("tour does not appear when localStorage flag is set", async ({
    page,
  }) => {
    await page.goto("/dashboard");
    // Set the flag to simulate returning user
    await page.evaluate(() =>
      localStorage.setItem("unified-tour-completed", "true"),
    );
    await page.reload();
    await page.waitForLoadState("networkidle");
