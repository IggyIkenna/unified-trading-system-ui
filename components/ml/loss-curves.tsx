...
function generateLossCurve(epochs: number, startLoss: number, endLoss: number, noise: number = 0.02): { epoch: number; trainLoss: number; valLoss: number }[] {
  const data = []
...
const mockModelStrategyLinks: ModelStrategyLink[] = [
  {
    ...
    modelVersion: "3.2.1",
    strategies: [
      { strategyId: "CEFI_BTC_ML_DIR_HUF_4H", strategyName: "ML Directional BTC", featureUsed: "btc_direction_signal", signalType: "direction", lastPrediction: "2s ago", accuracy7d: 0.72 },
      { strategyId: "CEFI_BTC_BASIS_SCE_1H", strategyName: "BTC Basis Trade", featureUsed: "btc_regime", signalType: "timing", lastPrediction: "15s ago", accuracy7d: 0.68 },
    ],
  },
  {
    ...
    modelVersion: "2.1.0",
    strategies: [
      { strategyId: "CEFI_ETH_OPT_MM_EVT_TICK", strategyName: "ETH Options MM", featureUsed: "eth_vol_surface", signalType: "sizing", lastPrediction: "1s ago", accuracy7d: 0.65 },
      { strategyId: "CEFI_ETH_MOM_HUF_4H", strategyName: "ETH Momentum", featureUsed: "eth_vol_regime", signalType: "risk", lastPrediction: "4h ago", accuracy7d: 0.71 },
    ],
  },
  {
    ...
    modelVersion: "1.0.0",
    strategies: [
      { strategyId: "CEFI_ETH_MOM_HUF_4H", strategyName: "ETH Momentum", featureUsed: "momentum_signal", signalType: "direction", lastPrediction: "30m ago", accuracy7d: 0.74 },
      { strategyId: "CEFI_SOL_MOM_HUF_4H", strategyName: "SOL Momentum", featureUsed: "momentum_signal", signalType: "direction", lastPrediction: "30m ago", accuracy7d: 0.69 },
    ],
  },
]
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <div>
            ...
            <span className="font-mono font-semibold">{latestTrainLoss.toFixed(4)}</span>
          </div>
          <div>
            <span className="text-muted-foreground">Val Loss: </span>
            <span className="font-mono font-semibold">{latestValLoss.toFixed(4)}</span>
          </div>
          <div>
            <span className="text-muted-foreground">Best (epoch {bestEpoch}): </span>
            <span className="font-mono font-semibold text-[var(--status-live)]">{bestValLoss.toFixed(4)}</span>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={lossData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
              ...
              <ReferenceLine 
                ...
                strokeDasharray="5 5" 
                label={{ value: "Best", position: "right", fill: "var(--status-live)", fontSize: 10 }}
              />
              <Line 
                type="monotone" 
                dataKey="trainLoss" 
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="font-medium flex items-center gap-2">
                    {link.modelName}
                    <Badge variant="outline" className="font-mono text-[10px]">v{link.modelVersion}</Badge>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div 
                        ...
                        style={{ backgroundColor: signalTypeColors[strategy.signalType] }}
                      />
                      <div>
                        <EntityLink
                          type="strategy"
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          <Badge variant="secondary" className="text-[10px]">{strategy.signalType}</Badge>
                        </div>
                      </div>
                    </div>
