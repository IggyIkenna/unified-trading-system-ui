...
const LANES = [
  { id: "data", label: "Data", sub: "Feeds, normalisation, entitlements", color: "sky" },
  { id: "ml", label: "ML", sub: "Features, models, signals, inference", color: "violet" },
  { id: "strategy", label: "Strategy", sub: "Research, simulation, decision logic", color: "amber" },
  { id: "execution", label: "Execution", sub: "Algos, routing, transaction cost", color: "emerald" },
  { id: "capital", label: "Capital", sub: "Mandates, allocations, oversight", color: "rose" },
  { id: "compliance", label: "Compliance", sub: "Audit trail, regulatory controls", color: "slate" },
] as const
...
const COLOR_MAP: Record<string, { bg: string; glow: string; dim: string; text: string; rail: string }> = {
  sky:     { bg: "bg-sky-400",     glow: "shadow-[0_0_12px_rgba(56,189,248,0.6)]",  dim: "bg-sky-400/20",     text: "text-sky-400",     rail: "bg-sky-400" },
  violet:  { bg: "bg-violet-400",  glow: "shadow-[0_0_12px_rgba(167,139,250,0.6)]", dim: "bg-violet-400/20",  text: "text-violet-400",  rail: "bg-violet-400" },
  amber:   { bg: "bg-amber-400",   glow: "shadow-[0_0_12px_rgba(251,191,36,0.6)]",  dim: "bg-amber-400/20",   text: "text-amber-400",   rail: "bg-amber-400" },
  emerald: { bg: "bg-emerald-400", glow: "shadow-[0_0_12px_rgba(52,211,153,0.6)]",  dim: "bg-emerald-400/20", text: "text-emerald-400", rail: "bg-emerald-400" },
  rose:    { bg: "bg-rose-400",    glow: "shadow-[0_0_12px_rgba(251,113,133,0.6)]", dim: "bg-rose-400/20",    text: "text-rose-400",    rail: "bg-rose-400" },
  slate:   { bg: "bg-slate-400",   glow: "shadow-[0_0_12px_rgba(148,163,184,0.6)]", dim: "bg-slate-400/20",   text: "text-slate-400",   rail: "bg-slate-400" },
}
...
function getDotSize(intensity: number, isHovered: boolean): string {
  ...
  switch (intensity) {
    ...
    case 2: return "size-2.5"
    case 1: return "size-1.5"
...
function getDotOpacity(intensity: number): string {
  switch (intensity) {
    ...
    case 1: return "opacity-30"
    default: return "opacity-15"
  }
}

export function PlatformArchitectureGrid() {
  const [hovered, setHovered] = React.useState<{ lane: string; stage: string } | null>(null)
                ...
                {intensities.map((intensity, i) => (
                  <div
                    ...
                    className={cn(
                      ...
                      colors.rail,
                      intensity >= 3 ? "opacity-60" :
                      intensity >= 2 ? "opacity-40" :
                      intensity >= 1 ? "opacity-20" : "opacity-10",
