...
const NODES = [
  { label: "TradFi",      sub: "CME · ICE",          color: "#22d3ee", fill: "#051318", angle: -126 },
  { label: "CeFi",        sub: "Binance · OKX",       color: "#4ade80", fill: "#05130a", angle: -54  },
  { label: "DeFi",        sub: "Uniswap · Aave",      color: "#a78bfa", fill: "#0c0818", angle:  18  },
  { label: "Sports",      sub: "Betfair · Pinnacle",  color: "#fbbf24", fill: "#141003", angle:  90  },
  { label: "Predictions", sub: "Polymarket · Kalshi", color: "#fb7185", fill: "#140508", angle: 162  },
]
const CONNS = [
...
export function MarketGalaxy({ className, showStats = true, animate }: MarketGalaxyProps) {
  ...
  const W = 520, H = 340, cx = W * 0.45, cy = H * 0.5, R = 120, nr = 46
    ...
    <div className="w-full flex items-center gap-8 px-4">
          ...
          return (
            <g key={i}>
              <line
                ...
                stroke={c.color} strokeOpacity={0.55} strokeWidth={1.8}
                strokeDasharray="9 7"
                strokeDashoffset={-offset}
              />
              <rect
                x={mx - 18} y={my - 9} width={36} height={18} rx={9}
                fill={c.color + "28"} stroke={c.color + "99"} strokeWidth={1}
              />
              <text
                x={mx} y={my + 4.5}
                textAnchor="middle"
      ...
      {showStats && (
        <div className="flex flex-col gap-6 flex-shrink-0">
          {STATS.map(s => (
            <div key={s.label}>
              <div className="text-3xl font-bold tabular-nums leading-none" style={{ color: s.color }}>{s.value}</div>
