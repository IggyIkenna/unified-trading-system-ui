...
function QuantOverview() {
  const [backtestRuns, setBacktestRuns] =
    React.useState<BacktestRun[]>(initialBacktestRuns);
  const [promotingBacktest, setPromotingBacktest] =
    React.useState<BacktestRun | null>(null);
  const [allocationAmount, setAllocationAmount] =
    React.useState<string>("250000");

  const completedBacktests = backtestRuns.filter(
    (b) => b.status === "completed",
  ).length;
  const promotedBacktests = backtestRuns.filter(
    (b) => b.status === "promoted",
  ).length;
  const runningJobs =
      ...
      prev.map((bt) =>
        bt.id === promotingBacktest.id
          ? {
              ...
              promotedAt: "just now",
              liveAllocation: parseInt(allocationAmount),
            }
          : bt,
      ),
    );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <Card className="w-[500px] shadow-xl">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    Deploy {promotingBacktest.name} to production
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-3 p-3 rounded-lg bg-muted/50">
                <div className="text-center">
                  ...
                  <div className="font-mono font-medium text-[var(--accent-blue)]">
                    {promotingBacktest.sharpe}
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-muted-foreground">Win Rate</div>
                  <div className="font-mono font-medium">
                    {promotingBacktest.winRate}%
                  </div>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  ...
                  <div className="font-mono font-medium text-negative">
                    {promotingBacktest.maxDrawdown}%
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setAllocationAmount(e.target.value.replace(/[^0-9]/g, ""))
                    }
              ...
              <div className="flex items-start gap-2 p-3 rounded-lg bg-[var(--status-warning)]/10 border border-[var(--status-warning)]/30">
                ...
                <div className="text-xs">
                  <span className="font-medium">Risk Warning:</span> This will
                  deploy the strategy with real capital. Simulated performance
                  does not guarantee live results. Position sizing and risk
            ...
            <div className="flex justify-end gap-2 px-6 pb-4">
              <Button
                variant="outline"
                onClick={() => setPromotingBacktest(null)}
              >
                Cancel
              </Button>
              <Button
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Research, backtesting, and ML training
          </p>
        </div>
                        ...
                        <div className="text-right">
                          <div
                            className={cn(
                              ...
                              bt.pnl >= 0 ? "text-positive" : "text-negative",
                            )}
                          >
                            {bt.pnl >= 0 ? "+" : ""}$
...
function BacktestPage() {
  const [selectedBacktest, setSelectedBacktest] = React.useState<string | null>(
    "bt-001",
  );
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Historical strategy testing and analysis
          </p>
        </div>
        <Button size="sm">
          <Plus className="h-3.5 w-3.5 mr-1.5" />
                ...
                {backtestRuns.map((bt) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {bt.status === "running" && (
                        <Progress value={bt.progress} className="mt-1.5 h-1" />
                      )}
                    </div>
                  </button>
                ))}
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Generate parameter grids for backtesting
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <Select
                  value={gridDimensions.toString()}
                  onValueChange={(v) => setGridDimensions(parseInt(v))}
                >
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Model training jobs and results
          </p>
        </div>
        <Button size="sm">
          <Plus className="h-3.5 w-3.5 mr-1.5" />
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Feature catalog and freshness monitoring
          </p>
        </div>
        <Button variant="outline" size="sm">
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Signal vs execution performance analysis
          </p>
        </div>
        <Button variant="outline" size="sm">
          <Download className="h-3.5 w-3.5 mr-1.5" />
