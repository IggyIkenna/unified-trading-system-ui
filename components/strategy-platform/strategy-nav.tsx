...
export function StrategyPlatformNav({ className }: { className?: string }) {
  ...
  return (
    <WidgetScroll axes="horizontal" scrollbarSize="thin" className={cn("shrink-0 border-b bg-card/50", className)}>
      <nav className="flex items-center gap-1 px-4 py-2">
        <div className="flex items-center gap-1 mr-4">
          <div className="size-2 rounded-full" style={{ backgroundColor: "var(--surface-strategy)" }} />
          ...
          return (
            ...
            >
              ...
              {item.badge && (
                <Badge
                  variant="secondary"
                  className={cn("h-5 min-w-[20px] px-1.5 text-[10px]", isActive && "bg-[var(--surface-strategy)]/20")}
                >
                  {item.badge}
                </Badge>
              )}
