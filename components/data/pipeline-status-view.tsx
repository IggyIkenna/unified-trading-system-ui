...
function VenueRow({
  ...
  const shardData = MOCK_SHARD_AVAILABILITY.find(
    (s) =>
      s.venue === venue || s.venue.toLowerCase().includes(venue.toLowerCase()),
  );

  const completionPct =
    job?.progressPct ??
    (historyEntry
      ? Math.round(historyEntry.successRate)
      : stableFallbackPct(venue, category));
  ...
  const tfData =
    config.timeframeData?.filter(
      (t) => t.venue === venue && t.category === category,
    ) ?? [];
    ...
    <div>
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span
            className={cn("size-2 rounded-full flex-shrink-0", statusDot)}
          />
            ...
            <div className="text-xs text-muted-foreground flex flex-wrap gap-4">
              <span>
                ...
                <span className="text-foreground font-mono">
                  {formatMs(historyEntry.avgDurationMs)}
                </span>
              </span>
              <span>
                p90:{" "}
                <span className="text-foreground font-mono">
                  {formatMs(historyEntry.p90DurationMs)}
                </span>
              </span>
              <span>
                Success rate:{" "}
                <span className="text-emerald-400 font-mono">
                  {historyEntry.successRate.toFixed(1)}%
                </span>
              </span>
              <span>
                Runs:{" "}
                <span className="text-foreground font-mono">
                  {historyEntry.runCount}
                </span>
              </span>
            </div>
          )}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="capitalize">
                      {DATA_CATEGORY_LABELS[job.category]}
                    </span>
                    <span className="text-muted-foreground">·</span>
                    <span className="capitalize">
                      {job.venue.replace(/_/g, " ")}
                    </span>
          ...
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm">
              {selectedCategory === "all"
                ? "All Venues"
                : DATA_CATEGORY_LABELS[selectedCategory]}
            </CardTitle>
            <Button
              type="button"
              variant="ghost"
          ...
          {visibleCategories
            ...
            .map((cat) => {
              ...
              return venues.map((venue) => {
                const job = jobs.find(
                  (j) => j.venue === venue && j.category === cat,
                );
                const history = jobHistory.find(
                  (h) => h.venue === venue && h.category === cat,
                );
                return (
                  <VenueRow
                    key={`${cat}:${venue}`}
