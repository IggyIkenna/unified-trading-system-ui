...
export function CloudPricingSelector({
  ...
  const hasCrossCloudSurcharge =
    selectedMode === "download" && org.linkedCloudAccount !== undefined && selectedCloud !== org.cloudPreference;

  const estimatedCostIn = estimatedGb ? estimatedGb * inSystemPricing.pricePerGb : null;
  const estimatedCostDown = estimatedGb
    ? estimatedGb * downloadPricing.pricePerGb + (hasCrossCloudSurcharge ? estimatedGb * 0.08 : 0)
    : null;
    ...
    <div className={cn("space-y-4", className)}>
          ...
          {(["gcp", "aws", "both"] as CloudProvider[]).map((cloud) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Cloud
                    className={cn(
                      "size-4",
                      cloud === "gcp" ? "text-blue-400" : cloud === "aws" ? "text-orange-400" : "text-sky-400",
                    )}
                  />
              ...
              {cloud === org.cloudPreference && (
                <div className="mt-1 text-[10px] text-muted-foreground text-left">Your preference</div>
              )}
              {cloud !== "both" && org.linkedCloudAccount?.startsWith(cloud) && (
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="text-xs text-muted-foreground mb-2">Query via our API. Data stays in Odum cloud.</div>
            <div className="text-lg font-bold font-mono text-emerald-400">
              ${formatNumber(inSystemPricing.pricePerGb, 2)}
              <span className="text-xs font-normal text-muted-foreground">/GB</span>
            </div>
            {estimatedCostIn !== null && (
              <div className="mt-1 text-xs text-emerald-400">≈ ${formatNumber(estimatedCostIn, 2)} estimated</div>
            )}
            <Badge variant="outline" className="mt-2 text-[10px] text-emerald-500 border-emerald-500/20">
              Recommended
            </Badge>
          </button>
          ...
          <button
            ...
            className={cn(
              "rounded-md border p-4 text-left transition-colors",
              selectedMode === "download" ? "border-amber-500/60 bg-amber-500/10" : "border-border hover:bg-accent",
            )}
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
            ...
            <div className="text-xs text-muted-foreground mb-2">
              Export to your {selectedCloud === "aws" ? "S3" : "GCS"} bucket. You own the copy.
            </div>
            <div className="text-lg font-bold font-mono text-amber-400">
              ${formatNumber(downloadPricing.pricePerGb, 2)}
              <span className="text-xs font-normal text-muted-foreground">/GB</span>
            </div>
            {hasCrossCloudSurcharge && <div className="text-xs text-amber-400">+ $0.08/GB cross-cloud</div>}
            {estimatedCostDown !== null && (
              <div className="mt-1 text-xs text-amber-400">≈ ${formatNumber(estimatedCostDown, 2)} estimated</div>
            )}
          </button>
        </div>
      </div>
      ...
      {selectedMode === "download" && !org.linkedCloudAccount && (
        <div className="flex items-center gap-2 rounded-md border border-border bg-accent/30 px-3 py-2 text-xs text-muted-foreground">
          <Info className="size-3.5 shrink-0" />
          Link your {selectedCloud === "aws" ? "AWS account" : "GCP project"} to enable direct egress.
          <Button variant="link" className="h-auto p-0 text-xs text-sky-400 ml-auto">
