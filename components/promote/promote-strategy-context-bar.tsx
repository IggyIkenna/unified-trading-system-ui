...
import { PROMOTE_PIPELINE_HREF, promoteHrefForStage } from "@/lib/config/services/promote.config";
import { selectPromoteSelectedStrategy, usePromoteLifecycleStore } from "@/lib/stores/promote-lifecycle-store";
...
export function PromoteStrategyContextBar() {
  ...
  return (
    <WidgetScroll axes="horizontal" scrollbarSize="thin" className="shrink-0 border-b border-border/60 pb-3 mb-1">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 px-1">
        ...
        <div className="flex items-center gap-1 shrink-0 pb-1 sm:pb-0">
          {STAGE_ORDER.map((stage, idx) => {
            ...
            const title = locked ? `${STAGE_META[stage].label} (locked)` : STAGE_META[stage].label;
            const inner = locked ? (
