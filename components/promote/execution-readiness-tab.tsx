...
export function ExecutionReadinessTab({
  ...
  const avgLatencyMs =
    nConn > 0 ? connected.reduce((s, v) => s + v.latencyMs, 0) / nConn : 0;
  const avgFillPct =
    nConn > 0 ? connected.reduce((s, v) => s + v.fillRate, 0) / nConn : 0;
  const maxLatencyMs =
    nConn > 0 ? Math.max(...connected.map((v) => v.latencyMs)) : 0;
  const minFillPct =
    nConn > 0 ? Math.min(...connected.map((v) => v.fillRate)) : 0;
  ...
  const summaryMetrics: ExecMetricCard[] = [
    ...
    {
      label: "Mkt impact",
      value: `${(exec.marketImpact * 10000).toFixed(1)} bps`,
      color:
        exec.marketImpact <= 0.0005 ? "text-emerald-400" : "text-amber-400",
    ...
    },
    {
      ...
      value:
        nVenues > 0
          ? fmtUsd(Math.round(exec.capacityUsd / nVenues))
          : fmtUsd(0),
    ...
    },
    {
      ...
      color:
        exec.marketImpact * 10000 <= 5 ? "text-emerald-400" : "text-rose-400",
      hint: "% of gate threshold",
    },
  ];
  ...
  const executionGates: {
    ...
    },
    {
      ...
      status: (connected.every((v) => v.fillRate >= FILL_FLOOR_PCT)
        ? "passed"
    ...
    },
    {
      ...
      status: (connected.every((v) => v.latencyMs <= LATENCY_CAP_MS)
        ? "passed"
        : "warning") as GateStatus,
          ...
          <Table className="table-fixed w-full min-w-[720px]">
            ...
            <TableBody>
              {exec.venues.map((venue) => {
                const latencyHeadroom = venue.connected
                  ? LATENCY_CAP_MS - venue.latencyMs
                  : null;
                const fillCushion = venue.connected
                  ? venue.fillRate - FILL_FLOOR_PCT
                  : null;
                const gateStatus =
                  venue.connected &&
                  venue.latencyMs <= LATENCY_CAP_MS &&
                  venue.fillRate >= FILL_FLOOR_PCT
                    ? ("passed" as const)
                    : venue.connected
                      ? ("warning" as const)
