...
import { PORTFOLIO_PRE_ADD_MAX_CORRELATION } from "@/lib/mocks/fixtures/promote-fixtures";
import { fmtNum, fmtPct } from "./helpers";
import type { CandidateStrategy } from "./types";

export function PortfolioImpactPanel({ strategy }: { strategy: CandidateStrategy }) {
  ...
  const divBenefit = r.correlationToPortfolio < 0.25 ? "Net diversification benefit" : "Marginal crowding risk";
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg border border-border/60 p-3">
            <p className="text-xs uppercase text-muted-foreground">
              Marginal VaR {marginalFromSizing !== undefined ? "(from sizing)" : "(est.)"}
            </p>
            <p className="text-lg font-mono font-bold mt-1">{fmtPct(marginal)}</p>
          </div>
          <div className="rounded-lg border border-border/60 p-3">
            <p className="text-xs uppercase text-muted-foreground">Correlation to book</p>
            <p className="text-lg font-mono font-bold mt-1">{fmtNum(r.correlationToPortfolio)}</p>
          </div>
        </div>
        <div className="rounded-lg border border-border/50 p-3">
          <p className="text-xs text-muted-foreground mb-2">Before / after (illustrative)</p>
