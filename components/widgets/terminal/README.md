...
| id                   | Role                                                                             |
| -------------------- | -------------------------------------------------------------------------------- |
| `terminal-watchlist` | Watchlist — instrument list grouped by category with search and one-click select |
...
| `order-entry`        | Order Entry — buy/sell form with account picker + strategy linking               |
...
`instrument-bar` was archived on 2026-04-22 (WU-4). Instrument selection moved to `terminal-watchlist` (reuses the shared `WatchlistPanel` from `components/trading/watchlist-panel.tsx`). Account picker moved into `order-entry`. Live price + %Δ + LIVE/BATCH badge were dropped — the chart already shows price and order-entry has a price field. Dead stub buttons (Refresh / Settings / Maximize) were removed per finding #4. See `docs/audits/live-review-findings.md` rows #4 and #17.
...
| -------------------- | ---------------------------------------------- | ----------------------------------------------------------- |
| `terminal-watchlist` | All other terminal widgets (anchors context)   | **positions** `positions-table` · **orders** `orders-table` |
...
| `calendar-events`    | `price-chart`, `terminal-watchlist`            | **overview** `pnl-chart` (event risk)                       |
