...
export function useValueFormat(initial: ValueFormat = "dollar") {
  ...
  const formatValue = React.useCallback((dollarValue: number, baseValue: number): string => {
    if (format === "percent") {
    ...
    // Dollar format
    if (Math.abs(dollarValue) >= 1000000) return `$${(dollarValue / 1000000).toFixed(2)}M`
    if (Math.abs(dollarValue) >= 1000) return `$${(dollarValue / 1000).toFixed(0)}k`
...
export function FormattedValue({
  ...
  const primary = format === "dollar" ? formatDollar(value) : formatPercent(value, baseValue)
  const secondary = format === "dollar" ? formatPercent(value, baseValue) : formatDollar(value)
