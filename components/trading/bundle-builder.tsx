...
const TEMPLATES: BundleTemplate[] = [
  {
    name: "Flash Loan Arb",
    description:
      "Borrow via flash loan, swap on DEX A, swap back on DEX B, repay",
    category: "DeFi",
    steps: [
      {
...
function getOperationColor(opType: string): string {
  ...
  if (["LEND", "BORROW", "REPAY", "WITHDRAW"].includes(opType))
    return "text-violet-400";
  if (["STAKE", "UNSTAKE"].includes(opType)) return "text-emerald-400";
  if (["ADD_LIQUIDITY", "REMOVE_LIQUIDITY", "COLLECT_FEES"].includes(opType))
                    ...
                    <div className="flex items-center gap-2 text-[10px]">
                      ...
                      {template.estimatedProfit > 0 && (
                        <span className="text-emerald-400 font-mono">
                          +${template.estimatedProfit.toFixed(0)}
                        </span>
                      )}
                    </div>
                  </div>
                    ...
                    {template.steps.map((s, i) => (
                      <React.Fragment key={i}>
                        <Badge
                          ...
                          className={cn(
                            "text-[8px] px-1 py-0",
                            getOperationBadgeClass(s.operationType),
                          )}
                        >
                          {s.operationType}
                        </Badge>
          ...
          <div className="space-y-2">
                  ...
                  <div className="flex flex-col items-center gap-0.5 shrink-0">
                    ...
                    <span
                      className={cn(
                        ...
                        getOperationColor(step.operationType),
                      )}
                    >
                      {step.operationType}
                    </span>
            ...
            {steps.map((step, index) => (
              <div key={step.id} className="p-3 rounded-lg border space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateStep(step.id, "operationType", v)
                      }
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateStep(step.id, "instrument", v)
                      }
                    >
                      <SelectTrigger className="h-7 text-xs">
                        <SelectValue />
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      value={step.quantity}
                      onChange={(e) =>
                        updateStep(step.id, "quantity", e.target.value)
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      value={step.price}
                      onChange={(e) =>
                        updateStep(step.id, "price", e.target.value)
                      }
                      className="h-7 text-xs font-mono"
                    />
                  </div>
                ...
                {/* Notional */}
                {(parseFloat(step.quantity) || 0) > 0 &&
                  (parseFloat(step.price) || 0) > 0 && (
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground pt-1">
                      ...
                      >
                        {step.side === "BUY" ? "-" : "+"}$
                        {(
                          (parseFloat(step.quantity) || 0) *
                          (parseFloat(step.price) || 0)
          ...
          <>
            ...
            <div className="p-3 rounded-lg border space-y-2">
              ...
              <div className="grid grid-cols-2 gap-1 text-xs">
                ...
                >
                  {netPnl >= 0 ? "+" : ""}$
                  {netPnl.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                </span>
              </div>
