...
export function PnLWaterfall({
  ...
  const maxAbsValue = Math.max(
    ...data.components.map((c) => Math.abs(c.value)),
    Math.abs(data.total),
  );
  ...
  return (
    <div className={cn("space-y-3", className)}>
          ...
          <div className="flex-1 flex items-center gap-4 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span
                className={cn(
                  ...
                  data.realized >= 0 ? "pnl-positive" : "pnl-negative",
                )}
              >
                {data.realized >= 0 ? "+" : ""}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span
                className={cn(
                  "font-mono tabular-nums",
                  data.unrealized >= 0 ? "pnl-positive" : "pnl-negative",
                )}
              >
                {data.unrealized >= 0 ? "+" : ""}
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-border">
        {showLabels && <div className="w-32 text-xs font-semibold">Total</div>}
        <div className="flex-1 h-6 bg-muted/30 rounded relative overflow-hidden">
          <div
            className="absolute top-0 bottom-0 w-px bg-border"
            style={{ left: "50%" }}
          ...
          <div
            ...
            style={{
              left:
                data.total >= 0
                  ? "50%"
                  : `${50 - (Math.abs(data.total) * scale) / 2}%`,
...
export function PnLBarChart({ data, className }: PnLBarChartProps) {
  const positiveTotal = data.components
    .filter((c) => c.value > 0)
...
export function PnLDonut({ data, size = 120, className }: PnLDonutProps) {
  ...
  const negativeTotal = Math.abs(
    negativeComponents.reduce((sum, c) => sum + c.value, 0),
  );
