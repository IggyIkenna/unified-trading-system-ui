...
export function StrategyInstructionViewer({
  className,
}: StrategyInstructionViewerProps) {
  ...
  const filtered = React.useMemo(() => {
    return MOCK_INSTRUCTIONS.filter((inst) => {
      if (strategyFilter !== "ALL" && inst.strategyType !== strategyFilter)
        return false;
      if (
        opTypeFilter !== "ALL" &&
        inst.instruction.operationType !== opTypeFilter
      )
        return false;
      return true;
    });
    ...
    <Card className={cn("overflow-hidden", className)}>
      <CardHeader className="pb-3">
        ...
        <div className="flex items-center gap-4 py-2 px-3 bg-muted/30 rounded mt-2 text-xs">
          ...
          <span className="text-muted-foreground">
            Filled:{" "}
            <span className="font-mono text-emerald-400">{summary.filled}</span>
          </span>
          <span className="text-muted-foreground">
            Partial:{" "}
            <span className="font-mono text-amber-400">{summary.partial}</span>
          </span>
          <span className="text-muted-foreground">
            Pending:{" "}
          ...
          <span className="text-muted-foreground">
            Avg Slippage:{" "}
            <span
              className={cn("font-mono", getSlippageColor(summary.avgSlippage))}
            >
      ...
      <CardContent className="pt-0 px-0">
        ...
        <div className="max-h-[600px] overflow-y-auto">
          {filtered.map((inst) => {
            ...
            const fillQtyPct = inst.fill
              ? (inst.fill.fillQty / inst.instruction.quantity) * 100
              : 0;
                ...
                >
                  ...
                  <div className="px-3 py-2.5 space-y-1">
                    <div className="flex items-center gap-1.5">
                      {inst.signal.direction === "LONG" ||
                      inst.signal.direction === "BACK" ||
                      inst.signal.direction === "YES" ? (
                      ...
                      <span
                        className={cn(
                          "text-[10px] font-mono",
                          getConfidenceColor(inst.signal.confidence),
                        )}
                      >
                      ...
                      <>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            className={cn(
                              "text-xs font-medium",
                              getStatusColor(inst.fill.status),
                            )}
                          >
                            {inst.fill.status}
                          </span>
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={cn(
                              "text-[10px] font-mono",
                              getSlippageColor(inst.fill.slippageBps),
                            )}
                          >
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
                          ...
                          <span className="font-mono">
                            {formatTimestamp(inst.signal.timestamp)}
                          </span>
                          <span className="text-muted-foreground">
                            Direction
                          </span>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
                          ...
                          <span className="font-mono">
                            $
                            {(
                              inst.instruction.quantity * inst.instruction.price
                        ...
                        {inst.fill ? (
                          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
                            ...
                            >
                              $
                              {(
                                inst.fill.fillPrice - inst.instruction.price
                              ).toFixed(2)}
                            </span>
                            <span className="text-muted-foreground">
                              Qty Diff
                            ...
                            <span
                              className={cn(
                                "font-mono",
                                inst.fill.fillQty < inst.instruction.quantity
                                  ? "text-amber-400"
                                  : "text-emerald-400",
                              )}
                            >
