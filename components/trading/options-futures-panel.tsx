...
type ComboType = "vertical-spread" | "straddle" | "strangle" | "calendar" | "butterfly" | "risk-reversal"
...
function generateSpreadMatrix(asset: SpreadAsset): (SpreadCell | null)[][] {
  ...
  const marks = SPREAD_EXPIRIES.map((_, i) =>
    spot * (1 + i * 0.008 + seededRandom(i + 50) * 0.003)
  )
...
function generateVolSurface(asset: Asset): { strikes: number[]; expiries: string[]; ivs: number[][] } {
  const spot = SPOT_PRICES[asset]
  const inc = STRIKE_INCREMENTS[asset]
  const baseIv = IV_INDEX[asset]
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 border-b bg-muted/20 overflow-x-auto">
      ...
      <div className="flex items-center gap-0.5 rounded-lg border p-0.5 bg-muted/30">
        {ASSETS.map((a) => (
          <Button
            ...
            size="sm"
            className={cn("h-7 px-3 text-xs font-mono", asset === a && "shadow-sm")}
            onClick={() => setAsset(a)}
          >
            {a}
          </Button>
      ...
      <div className="flex items-center gap-1.5 text-xs whitespace-nowrap">
        ...
        <span className="font-mono font-bold">{formatUsd(spot)} {asset}</span>
      </div>
      ...
      <div className="flex items-center gap-1.5 text-xs whitespace-nowrap">
        ...
        <span className="text-[10px] text-muted-foreground">(VoV: {(iv * 0.15).toFixed(1)}%)</span>
      </div>
    </div>
  )
}
...
function OptionsChainTab({
  ...
  const handleCellClick = (row: OptionRow, side: "C" | "P", useAsk: boolean) => {
    const price = side === "C"
      ? (useAsk ? row.callAsk : row.callBid)
      : (useAsk ? row.putAsk : row.putBid)
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs px-1">
          <span className="text-muted-foreground">
            Underlying future: <span className="font-mono font-medium text-foreground">{formatUsd(spot + spot * 0.001, 2)}</span>
          </span>
          <span className="text-muted-foreground">
            Time to Expiry: <span className="font-mono text-foreground">94d 18h 13m</span>
            ...
            {EXPIRY_DATES.map((exp) => (
              ...
              >
                ...
                {EXPIRIES_WITH_POSITIONS.has(exp) && (
                  <Badge variant="secondary" className="ml-1 text-[8px] px-1 py-0 h-3.5">POS</Badge>
                )}
              </Button>
            ))}
          </div>
      ...
      <div className="flex items-center gap-3 px-1">
        <div className="flex items-center gap-1.5">
          ...
          />
          <label htmlFor="around-atm" className="text-xs text-muted-foreground cursor-pointer">
            Around ATM
          </label>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          />
          <label htmlFor="expected-move" className="text-xs text-muted-foreground cursor-pointer">
            Expected move
          </label>
        </div>
              ...
              {filteredChain.map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className={cn(
                        row.callDelta > 0.7 ? "text-emerald-500" :
                        row.callDelta > 0.3 ? "text-emerald-400/70" :
                    ...
                    >
                      <span className={cn(
                        row.putDelta < -0.7 ? "text-rose-500" :
                        row.putDelta < -0.3 ? "text-rose-400/70" :
                        "text-muted-foreground"
                      )}>
              ...
              {filteredFutures.map((row) => (
                ...
                >
                  ...
                  <td className="py-1.5 px-2 font-mono font-medium">
                    {row.contract}
                    {row.isPerpetual && (
                      <Badge variant="outline" className="ml-1.5 text-[8px] px-1 py-0 h-3.5">PERP</Badge>
                    )}
                  </td>
                  ...
                  <td className={cn(
                    ...
                    row.fundingRate !== null
                      ? (row.fundingRate >= 0 ? "text-emerald-400" : "text-rose-400")
                      : "text-muted-foreground"
                  )}>
                    {row.fundingRate !== null
                        ...
                        <div className="rounded border bg-card/80 p-1.5 min-w-[90px] space-y-0.5">
                          ...
                          <div className="flex items-center justify-center gap-2">
                            <span className="font-mono text-emerald-400 font-medium">
                              {cell.bid >= 0 ? "+" : ""}{cell.bid.toFixed(1)}
                            </span>
                            <span className="font-mono text-muted-foreground">
                              {cell.ask >= 0 ? "+" : ""}{cell.ask.toFixed(1)}
                            </span>
                          </div>
    ...
    return (
      <div className="space-y-3">
        ...
        <p className="text-[10px] text-muted-foreground px-1">
          ...
          Lower triangle shows valid combinations. Click a cell to trade.
        </p>
        <div className="border rounded-lg overflow-hidden">
          <ScrollArea className="w-full">
            <table className="w-full text-[10px]">
                        ...
                        >
                          <div className="rounded border bg-card/80 p-1 min-w-[80px] space-y-0.5">
                            <div className="flex items-center justify-center gap-1.5">
                              <span className={cn("font-mono font-medium", netDebit > 0 ? "text-rose-400" : "text-emerald-400")}>
              ...
              <thead>
                <tr className="border-b bg-muted/40">
                  ...
                  {CALENDAR_EXPIRIES.map((exp) => (
                    <th key={exp} className="py-2 px-1.5 text-center font-mono font-medium text-[10px] whitespace-nowrap">
                      {exp}
                    </th>
                  ))}
                </tr>
                  ...
                  return (
                    <tr key={strike} className={cn("border-b", isAtm && "bg-amber-500/10")}>
                      ...
                      {CALENDAR_EXPIRIES.map((exp, eIdx) => {
                        ...
                        const legs: ComboLeg[] = [
                          { strike, type: "call", direction: "buy", price: farPrice, delta: baseG.delta * 0.95, gamma: baseG.gamma * 0.9, theta: baseG.theta * 0.7, vega: baseG.vega * 1.2 },
                          { strike, type: "call", direction: "sell", price: nearPrice, delta: baseG.delta, gamma: baseG.gamma, theta: baseG.theta, vega: baseG.vega },
                        ]
                    ...
                    <tr key={center} className={cn("border-b", isAtm && "bg-amber-500/10")}>
                      ...
                      {wingWidths.map((width) => {
                        ...
                        const legs: ComboLeg[] = [
                          ...
                          { strike: center, type: "call", direction: "sell", price: centerG.price, delta: centerG.delta * -2, gamma: centerG.gamma * -2, theta: centerG.theta * -2, vega: centerG.vega * -2 },
                        ...
                        legs[1] = { strike: center, type: "call", direction: "sell", price: centerG.price, delta: centerG.delta, gamma: centerG.gamma, theta: centerG.theta, vega: centerG.vega }

                        // Sell 2x center: duplicate the sell leg
                        const allLegs: ComboLeg[] = [
                          { strike: lower, type: "call", direction: "buy", ...lowerG },
                          { strike: center, type: "call", direction: "sell", price: centerG.price, delta: centerG.delta, gamma: centerG.gamma, theta: centerG.theta, vega: centerG.vega },
                          { strike: center, type: "call", direction: "sell", price: centerG.price, delta: centerG.delta, gamma: centerG.gamma, theta: centerG.theta, vega: centerG.vega },
                          { strike: upper, type: "call", direction: "buy", ...upperG },
                        ]
                ...
                <tr key={callStrike} className="border-b">
                  ...
                  {rrPutStrikes.map((putStrike) => {
                    ...
                    return (
                      ...
                      >
                        <div className="rounded border bg-card/80 p-1 min-w-[70px] space-y-0.5">
                          <span className={cn("font-mono font-medium", netCredit >= 0 ? "text-emerald-400" : "text-rose-400")}>
...
function TradePanel({
  ...
  const liqPrice = instrument?.type === "future" && priceNum > 0 && leverageVal > 1
    ? direction === "buy"
      ? priceNum * (1 - 1 / leverageVal)
      : priceNum * (1 + 1 / leverageVal)
    : null

  const marginRequired = amountNum > 0 && priceNum > 0
    ? (amountNum * priceNum) / (instrument?.type === "future" ? leverageVal : 1)
    : 0

  // Spread margin: combined margin for both legs
  const spreadMargin = instrument?.type === "spread" && amountNum > 0
          ...
          {legs.map((leg, idx) => (
            <div key={idx} className="flex items-center justify-between text-xs">
              <span className={cn(
                ...
                leg.direction === "buy" ? "text-emerald-400" : "text-rose-400"
              )}>
                {leg.direction === "buy" ? "BUY" : "SELL"} {leg.strike.toLocaleString()}{leg.type === "call" ? "C" : "P"}
              </span>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div>
                    ...
                    <p className={cn("font-mono text-xs font-medium", netDelta >= 0 ? "text-emerald-400" : "text-rose-400")}>
                      {netDelta >= 0 ? "+" : ""}{netDelta.toFixed(3)}
                    </p>
                  </div>
                </TooltipTrigger>
                <TooltipContent><p className="text-xs">Net delta across all legs</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent><p className="text-xs">Net gamma across all legs</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent><p className="text-xs">Net vega across all legs</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent><p className="text-xs">Net theta across all legs (USD/day)</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
        </div>
    ...
    <div className="space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent><p className="text-xs">Delta: price sensitivity</p></TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent><p className="text-xs">Gamma: delta sensitivity</p></TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent><p className="text-xs">Theta: time decay (USD/day)</p></TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent><p className="text-xs">Vega: IV sensitivity</p></TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          </div>
      ...
      <div className="p-3 rounded-lg border bg-muted/30 space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-mono">
            {amountNum > 0 && priceNum > 0 ? formatUsd(amountNum * priceNum) : "--"}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-muted-foreground">
...
function GreeksSurfacePanel({ asset }: { asset: Asset }) {
    ...
    return surface.strikes.map((strike, sIdx) => {
      ...
      return surface.expiries.map((_, eIdx) => {
        ...
        switch (selectedGreek) {
          case "delta": return normCdf
          case "gamma": return (0.01 / (iv / 100 * sqrtT * spot)) * Math.exp(-d1 * d1 / 2)
          case "vega": return spot * sqrtT * Math.exp(-d1 * d1 / 2) * 0.01
          case "theta": return -(spot * (iv / 100) * Math.exp(-d1 * d1 / 2)) / (2 * sqrtT) / 365
        }
      })
    })
  ...
  const formatGreek = (val: number): string => {
    switch (selectedGreek) {
      ...
      case "vega": return val.toFixed(2)
