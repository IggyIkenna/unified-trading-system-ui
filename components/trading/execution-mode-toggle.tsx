        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className={cn(
              ...
              >
                <Radio className={cn(size === "sm" ? "size-3" : "size-3.5", isLive && "animate-pulse")} />
                Live
              </Button>
              <Button
                variant="ghost"
              ...
              >
                <Database className={cn(size === "sm" ? "size-3" : "size-3.5")} />
                Batch
              </Button>
            </div>
          </TooltipTrigger>
          <TooltipContent side="bottom" className="max-w-xs">
            <div className="space-y-1">
              <p className="font-semibold">{isLive ? "Live Mode" : "Batch Mode"}</p>
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <Badge 
              ...
              className={cn(
                ...
                isLive ? "border-[var(--status-live)] text-[var(--status-live)]" : ""
              )}
            >
              {config.dataSource.toUpperCase()}
            </Badge>
...
export function ExecutionModeIndicator({ className }: ExecutionModeIndicatorProps) {
    ...
    <div className={cn("flex items-center gap-1.5", className)}>
      {isLive ? (
        <>
          ...
          <span className="text-xs font-medium text-[var(--status-live)] uppercase">Live</span>
        </>
      ) : (
        <>
          <Clock className="size-3 text-muted-foreground" />
          <span className="text-xs font-medium text-muted-foreground uppercase">Batch</span>
        </>
      )}
    </div>
  )
...
export function ExecutionModeSelector({ className }: ExecutionModeSelectorProps) {
    ...
    <div className={cn("grid grid-cols-2 gap-4", className)}>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          <Radio className={cn("size-5", mode === "live" && "text-[var(--status-live)] animate-pulse")} />
          <span className="font-semibold">Live Mode</span>
          {mode === "live" && (
            <Badge className="bg-[var(--status-live)] text-white text-[10px]">Active</Badge>
          )}
        </div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          <Database className={cn("size-5", mode === "batch" && "text-primary")} />
