...
export function VolSurfaceChart({
  ...
  const avgAtmIv =
    surface.smiles.length > 0
      ? surface.smiles.reduce((sum, s) => sum + s.atmIv, 0) / surface.smiles.length
      : 0

  const avgSkew =
    surface.smiles.length > 0
      ? surface.smiles.reduce((sum, s) => sum + s.skew25Delta, 0) / surface.smiles.length
      : 0
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
      <CardHeader className="pb-2">
        ...
        {isLoading && (
          <div className="text-xs text-muted-foreground mt-1">Loading vol surface...</div>
        )}
      </CardHeader>
