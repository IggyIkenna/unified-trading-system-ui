 ...
 */
export function LockedItemDialog({ href, label, sectionTitle, accessType, children }: LockedItemDialogProps) {
  const [open, setOpen] = React.useState(false);
  const [code, setCode] = React.useState("");
  const [error, setError] = React.useState("");
  const router = useRouter();
  const signInHref = `/login?redirect=${encodeURIComponent(href)}`;
  const hint = accessType === "code" ? "Enter access code or contact us" : "Sign in or contact us";
  ...
  return (
    <>
      ...
      >
        <span className={cn("relative flex w-full cursor-default items-center gap-2 pr-7 text-muted-foreground")}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{sectionTitle} is restricted. Here&apos;s how to get in.</DialogDescription>
          </DialogHeader>
